...
try {
  ...
  # Create friendly name 'Sora' beside the published artifact
  $published = Get-ChildItem -Path $outPath -File | Where-Object { $_.BaseName -eq 'Sora.Orchestration.Cli' -or $_.Name -eq 'Sora.Orchestration.Cli' } | Select-Object -First 1
  if (-not $published) {
    # fallback: best-effort search for cli exe in folder
  $published = Get-ChildItem -Path $outPath -File | Where-Object { $_.Name -like 'Sora.Orchestration.Cli*' -or $_.Name -like 'Sora.Orchestration.*Cli*' } | Select-Object -First 1
  }
  if ($published) {
    $targetName = if ($published.Extension) { 'Sora' + $published.Extension } else { 'Sora' }
  $target = Join-Path -Path $outPath -ChildPath $targetName
    ...
    Write-Warning "[cli-publish] Could not locate main binary to alias as 'Sora'."
  }
