...
internal sealed class ProductSurfaceCompiler(string repositoryRoot)
{
    ...
    {
        if (source.SchemaVersion != PackagingConstants.ProductSurface.ClaimsSchema)
        {
            throw new InvalidOperationException(
                $"Product claims schema {source.SchemaVersion} is unsupported; expected " +
                $"{PackagingConstants.ProductSurface.ClaimsSchema}.");
        }

        var releaseTrain = ReadRootReleaseTrain();
        ...
        ValidateSupportedDependencyClosure(graph, claimsById.Values);
    ...
    public static string ToMarkdown(ProductSurface surface)
    {
        ...
        markdown.Append("framework_version: v").AppendLine(surface.ReleaseTrain);
        ...
        markdown.AppendLine();
        markdown.Append("All inventoried active packages ship together on release train `").Append(surface.ReleaseTrain).AppendLine("`.");
        markdown.AppendLine();
        markdown.AppendLine("Every package carries the stable train's 1.x compatibility contract. Claims below state the strength of each behavioral guarantee; packages without a claim are explicitly unassessed.");
        markdown.AppendLine();
        markdown.AppendLine("## Maturity vocabulary");
        markdown.AppendLine();
        markdown.AppendLine("These labels are evidence and support dispositions, not one linear ranking. Release membership comes from the evaluated package inventory, not claim maturity.");
        markdown.AppendLine();
        ...
        {
            markdown.Append("| `").Append(definition.Name).Append("` | ")
                .Append(Escape(definition.Meaning)).Append(" | ")
                .Append(Escape(definition.Contract)).AppendLine(" |");
        }
        markdown.AppendLine();
        ...
        markdown.AppendLine();
        markdown.AppendLine("| Package | Project | Shape / target | Product claim | Package docs |");
        markdown.AppendLine("|---|---|---|---|---|");
        foreach (var package in surface.Packages)
        {
            ...
            markdown.Append("| `").Append(Escape(package.PackageId)).Append("` — ")
                ...
                .Append(Escape(package.ProjectPath)).Append("` | ")
    ...
    private ProductClaim CompileClaim(ProductClaimInput input, PackageGraph graph)
    {
        ...
        foreach (var packageId in packages)
        {
            ...
            if (PackagingConstants.ProductSurface.SupportedMaturities.Contains(maturity) && !package.OwnsReadme)
            {
                throw new InvalidOperationException(
                    $"Product claim '{id}' marks '{package.PackageId}' as '{maturity}', but the package has no " +
    ...
    private static void ValidateSupportedDependencyClosure(
        PackageGraph graph,
        IEnumerable<ProductClaim> claims)
    {
        var supportedPackages = claims
            .Where(claim => PackagingConstants.ProductSurface.SupportedMaturities.Contains(claim.Maturity))
    ...
    private string ReadRootReleaseTrain()
    {
        var path = Path.Combine(repositoryRoot, ReleaseTrain.FileName);
        if (!File.Exists(path))
        {
            throw new InvalidOperationException(
                $"The repository root has no '{ReleaseTrain.FileName}' release-train authority.");
        }

        try
        {
            return ReleaseTrain.ParseJson(File.ReadAllText(path)).ToString();
        }
        catch (Exception error) when (error is JsonException or InvalidOperationException)
        {
            throw new InvalidOperationException(
                $"The root '{ReleaseTrain.FileName}' release train is invalid: {error.Message}",
                error);
        }
    }
