...
(function(){
  ...
  async function rebuildFromCache(){ try{ const u='/admin/cache/list'; const r = await fetch(u); if(!r.ok){ window.showToast && showToast('No cache available','error'); return; } const data = await r.json(); if(!data.caches || data.caches.length === 0){ window.showToast && showToast('No cached imports found','error'); return; } const cache = data.caches[0]; window.showToast && showToast(`Rebuilding from cache: ${cache.source}/${cache.mediaType}...`); const rebuildUrl='/admin/rebuild-db-from-cache'; const rebuildResp = await fetch(rebuildUrl, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ source: cache.source, mediaType: cache.mediaType }) }); if(rebuildResp.ok){ const result = await rebuildResp.json(); window.showToast && showToast(`Database rebuilt: ${result.imported} items from ${result.jobsProcessed} cache job(s)`, 'success'); setTimeout(()=>{ loadStats(); loadTagsCount(); }, 1500); }else{ window.showToast && showToast('Rebuild from cache failed','error'); }}catch{ window.showToast && showToast('Rebuild error','error'); } }
