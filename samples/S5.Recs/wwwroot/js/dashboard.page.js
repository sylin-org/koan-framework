...
(function(){
  ...
  async function vectorUpsert(){ try{ window.showToast && showToast('Rebuilding ALL vectors…'); const u=(window.S5Const?.ENDPOINTS?.ADMIN_SEED_VECTORS)||'/admin/seed/vectors'; const r = await fetch(u, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({}) }); window.showToast && showToast(r.ok?'Vector rebuild job started':'Vector rebuild failed', r.ok?'success':'error'); }catch{ window.showToast && showToast('Vector rebuild error','error'); } }
  ...
  function bindSettings(){
    ...
    const ctpw = document.getElementById('ctpw');
    const ctpwNum = document.getElementById('ctpwNum');
    if(!ptw||!ptwNum||!mpt||!mptNum||!dw||!dwNum||!ctpw||!ctpwNum) return;
    const link = (a,b) => { a.addEventListener('input', ()=> b.value = a.value); b.addEventListener('input', ()=> a.value = b.value); };
    link(ptw, ptwNum); link(mpt, mptNum); link(dw, dwNum); link(ctpw, ctpwNum);
