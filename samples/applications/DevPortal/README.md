# DevPortal — publish through named provider channels

DevPortal is a business-aligned provider-composition sample. Editors draft articles in a local SQLite store,
...
```powershell
dotnet run --project samples/applications/DevPortal
```
...
The application module also states how an article should appear when shared:
...
Referencing `Koan.Web.OpenGraph` contributes the request-pipeline behavior automatically. The application
declares only its entity-to-card meaning; without a configured shell or matching card the capability is inert.

## Optional external channels

DevPortal also references Mongo and Postgres connectors. Named source configuration maps business channels to
their mechanics:
...
- article navigations render entity-backed OpenGraph metadata without host middleware ceremony;
