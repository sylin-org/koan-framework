...
- GET /api/devices - pages `DynamicCanonEntity<Device>` (Id = ULID, CanonicalId = business key, Model = nested JSON)
- GET /api/sensors - pages `DynamicCanonEntity<Sensor>`
...
- Ingestion uses the normalized sender (`IFlowSender`) with server-side stamping and reserved key prefixes (plain bag):
  ...
  - Use `CanonEntitySendExtensions.Send()` for entities and `CanonValueObjectSendExtensions.Send()` for value objects with the messaging-first pattern.
