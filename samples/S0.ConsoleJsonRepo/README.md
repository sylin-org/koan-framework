# S0.ConsoleJsonRepo — one local checklist

S0 is Koan's smallest console sample: one Entity model, one standard host, and one durable JSON provider selected by reference.
...
```powershell
Set-Location samples/S0.ConsoleJsonRepo
dotnet run
```
...
- `S0.ConsoleJsonRepo.csproj` — the single foundation capability reference
