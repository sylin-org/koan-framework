...
class FlowApiClient {
  ...
  async getReadingsByReference(referenceId, params = {}) {
    ...
    return this.fetchJson(
      `/api/readings/${referenceId}${query ? "?" + query : ""}`
    );
  }
  ...
  async getReadingsByCanonicalId(canonicalId, params = {}) {
    const query = new URLSearchParams(params).toString();
    return this.fetchJson(
      `/api/readings/by-cid/${canonicalId}${query ? "?" + query : ""}`
    );
  }
  ...
  async getModelView(model, view, params = {}) {
    const query = new URLSearchParams(params).toString();
    return this.fetchJson(
      `/models/${model}/views/${view}${query ? "?" + query : ""}`
    );
  }
  ...
  async getRecentActivity(limit = 20) {
    try {
      ...
      return (readings?.items || []).map((reading) => ({
        ...
        subtitle: `Sensor: ${
          reading.payload?.SensorId || reading.correlationId || "Unknown"
        }`,
        level: "success",
        data: reading,
      }));
