...
$sourcePrompts = @(
    @{ Prompt = "Meeting notes capturing enterprise architecture discussions and vendor evaluation decisions."; TargetFields = @("$.notes") },
    @{ Prompt = "Vendor questionnaire responses describing revenue, staffing, certifications, and service offerings."; TargetFields = @("$.revenue","$.employees") }
)

$sourceTypes = @()
foreach ($entry in $sourcePrompts) {
    $created = Ensure-MeridianSourceTypeAi -BaseUrl $BaseUrl -Prompt $entry.Prompt -TargetFields $entry.TargetFields -SkipCertificateCheck:$SkipCertificateCheck
    $sourceTypes += $created
