...
public sealed class MeridianModule : KoanModule
{
    ...
    public override Task Start(IServiceProvider services, CancellationToken ct) => Task.WhenAll(
        new SourceTypeSeeder().Seed(ct),
        new OrganizationProfileSeeder().Seed(ct),
        new DocumentStyleSeeder().Seed(ct),
        new AnalysisTypeSeeder().Seed(ct));

    public override void Report(ProvenanceModuleWriter module, IConfiguration cfg, IHostEnvironment env)
    {
        module.Describe(Version, "Evidence-backed narrative pipeline sample.")
            .SetStatus("enabled");
    }
}
