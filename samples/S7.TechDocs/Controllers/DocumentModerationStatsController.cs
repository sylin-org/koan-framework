...
public class DocumentModerationStatsController : ControllerBase
{
    ...
    /// </summary>
    [Authorize]
    [RequireCapability(Sora.Web.Extensions.Capabilities.CapabilityActions.Moderation.Queue)]
    [HttpGet("moderation/stats")]
    public async Task<IActionResult> Stats(CancellationToken ct)
    {
        // Submitted count: items in Submitted set
        int submittedCount = 0;
    try { submittedCount = await Document.CountAll(Sora.Web.Infrastructure.SoraWebConstants.Sets.Moderation.Submitted, ct); } catch { submittedCount = 0; }
        ...
        int deniedTotal = 0;
    try { deniedTotal = await Document.CountAll(Sora.Web.Infrastructure.SoraWebConstants.Sets.Moderation.Denied, ct); } catch { deniedTotal = 0; }
