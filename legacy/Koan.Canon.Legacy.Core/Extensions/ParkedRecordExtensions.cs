...
{
    /// <summary>
    /// Heals a parked record by re-injecting the resolved model data into the Canon pipeline
    /// and removing the parked record. This is the proper Koan.Canon pattern for 
    ...
    /// <param name="CanonActions">The Canon actions service for re-injection</param>
    /// <param name="healedModel">The resolved/healed model to re-inject into Canon</param>
        ...
        CancellationToken ct = default)
    {
        ...
        ArgumentNullException.ThrowIfNull(CanonActions);
        ...
        // For now, we'll rely on Canon's logging and tracing for audit
        
        // Extract the model name from the type
        var modelName = typeof(TModel).Name.ToLowerInvariant();
        
        // Re-inject the healed model into Canon's intake pipeline
        await CanonActions.SeedAsync(modelName, effectiveCorrelationId, healedModel, effectiveCorrelationId, ct);
        ...
        where TModel : new()
    {
        ...
        await HealAsync(parkedRecord, CanonActions, healedModel, healingReason, correlationId, ct);
    }
