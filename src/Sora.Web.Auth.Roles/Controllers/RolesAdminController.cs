...
namespace Sora.Web.Auth.Roles.Controllers;
...
public sealed class RolesAdminController : ControllerBase
{
    ...
    // Roles
    [HttpGet]
    public async Task<ActionResult<IReadOnlyList<ISoraAuthRole>>> GetRoles(CancellationToken ct)
    ...
    public async Task<ActionResult<IReadOnlyList<ISoraAuthRoleAlias>>> GetAliases(CancellationToken ct)
    ...
    public async Task<ActionResult<IReadOnlyList<ISoraAuthRolePolicyBinding>>> GetPolicyBindings(CancellationToken ct)
    ...
    public async Task<ActionResult<object>> Import([FromQuery] bool? dryRun = null, [FromQuery] bool? force = null, CancellationToken ct = default)
    {
        ...
        bool allowMagic = Sora.Core.SoraEnv.AllowMagicInProduction || opt.AllowSeedingInProduction;
        if (isProd && !allowMagic)
            return StatusCode(403, new { error = "Import is disabled in Production. Set SoraEnv.AllowMagicInProduction or Roles.AllowSeedingInProduction to enable." });
    ...
    public sealed class RoleDto : ISoraAuthRole
    {
    ...
    public sealed class RoleAliasDto : ISoraAuthRoleAlias
    {
        public string Id { get; set; } = string.Empty;
        public string TargetRole { get; set; } = string.Empty;
        public byte[]? RowVersion { get; set; }
    }
    public sealed class RolePolicyBindingDto : ISoraAuthRolePolicyBinding
    {
        public string Id { get; set; } = string.Empty;
