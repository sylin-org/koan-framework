...
public class OptimizedModelAssociationWorker : BackgroundService
{
    ...
    private async Task<int> ProcessModelWithBatchOperations(Type modelType, int batchSize, CancellationToken ct)
    {
        try
        {
            ...
            var setName = CanonSets.StageShort(CanonSets.Standardized);
            ...
            if (processedRecords.Count > 0)
            {
                await BatchTransitionRecords(processedRecords, modelType,
                    CanonSets.StageShort(CanonSets.Standardized),
                    CanonSets.StageShort(CanonSets.Keyed), ct);
            }
        ...
        CancellationToken ct)
    {
        ...
        var tasks = recordGroups.Select(async group =>
        {
            ...
            try
            {
                ...
                {
                    if (keyIndexLookup.TryGetValue(candidate, out var ReferenceId))
                    {
                        owners.Add(ReferenceId);
                    }
                }
    ...
    private List<string> ExtractCandidatesFromRecord(object record, Type modelType)
    {
        ...
        try
        {
            ...
            var modelInfo = CanonRegistry.GetEntityIdStructure(modelType);
    ...
    private string GetKeyIndexReferenceId(object keyIndex)
    {
        return keyIndex.GetType().GetProperty("ReferenceId")?.GetValue(keyIndex) as string ?? "";
    }
