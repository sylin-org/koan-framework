...
// All Canon entities are automatically optimized for storage efficiency
[OptimizeStorage(Reason = "Canon entity base class - all Canon entities use optimized storage")]
public abstract class CanonEntity<TModel> : Entity<TModel> where TModel : CanonEntity<TModel>, new() { }

// Value-object base marker; derives from Entity<T> so standard EntityController<> can be used.
// Unlike CanonEntity<>, value-objects are not treated as Canonical roots and should not participate
...
public abstract class CanonValueObject<TVo> : Entity<TVo> where TVo : CanonValueObject<TVo>, new() { }

// Base interface for DynamicCanonEntity types
public interface IDynamicCanonEntity
{
    JObject? Model { get; set; }
}

// Normalized transport/delta for a model: Id + dynamic data (JObject or dictionary)
public class DynamicCanonEntity<TModel> : Entity<DynamicCanonEntity<TModel>>, IDynamicCanonEntity
{
