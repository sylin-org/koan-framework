# Koan.Canon.Core - Developer Usage Guide

A comprehensive guide for building data processing pipelines with Koan.Canon, from basic concepts to advanced enterprise scenarios.
...
Koan.Canon is designed to solve common challenges in modern data processing:
...
Koan.Canon processes data through a series of stages:
...
- **CanonEntity**: Your canonical data model (e.g., Customer, Device, Order)
...
Add the Koan.Canon.Core package to your project:
...
- Inherit from `CanonEntity<T>` where T is your model
...
public async Task<IActionResult> GetCustomer(string referenceId)
{
    ...
    var customer = await DynamicCanonEntity<Customer>.Get(referenceId);
    if (customer == null)
        return NotFound();
...
```csharp
public sealed class Contact : CanonEntity<Contact>
{
    // Primary identifiers
    [AggregationTag("email")]
    public string? Email { get; set; }
...
Koan.Canon follows strict rules when associating records:
...
public async Task<IActionResult> HealParkedRecord(string parkedId,
    [FromServices] ICanonActions flowActions)
{
    using (DataSetContext.With(CanonSets.StageShort(CanonSets.Parked)))
    {
...
{
    private readonly ICanonActions _flowActions;
...
public sealed class Order : CanonEntity<Order>
{
...
public sealed class OrderItem : CanonEntity<OrderItem>
{
...
public sealed class EnterpriseCustomer : CanonEntity<EnterpriseCustomer>
{
    [AggregationTag("taxId")]
...
[FlowModel("sensor")]
public sealed class SensorData : CanonEntity<SensorData>
{
...
public class FlowManagementController : ControllerBase
{
    ...
    public FlowManagementController(ICanonActions flowActions)
    {
...
```csharp
public class CustomerMonitor : ICanonMonitor<Customer>
{
    ...
    public Task OnProjectedAsync(CanonMonitorContext context, CancellationToken ct = default)
    {
...
{
    var intakeCount = await GetStageCount<Customer>(CanonSets.Intake);
    var keyedCount = await GetStageCount<Customer>(CanonSets.Keyed);
    var parkedCount = await GetStageCount<Customer>(CanonSets.Parked);
...
{
    using (DataSetContext.With(CanonSets.StageShort(stage)))
    {
...
```csharp
public class ErrorHandlingService : ICanonMonitor<Customer>
{
    private readonly ILogger<ErrorHandlingService> _logger;
    private readonly IMessageBus _messageBus;

    public async Task OnProjectedAsync(CanonMonitorContext context, CancellationToken ct = default)
    {
        try
        {
...
{
    services.Configure<CanonOptions>(o =>
    {
...
public async Task<IActionResult> ReprojectEntity(string referenceId)
{
    ...
    // Trigger reproject
    var runtime = HttpContext.RequestServices.GetRequiredService<ICanonRuntime>();
...
var model = CanonRegistry.GetAggregationTags(typeof(Customer));
if (model.Length == 0)
{
...
public async Task<IActionResult> GetStageSizes()
{
    ...
    foreach (var stage in new[] { CanonSets.Intake, CanonSets.Keyed, CanonSets.Parked })
    {
        using (DataSetContext.With(CanonSets.StageShort(stage)))
        {
...
public class FlowMetricsService : BackgroundService
{
    ...
    private async Task LogMetrics()
    {
        ...
        {
            Timestamp = DateTimeOffset.UtcNow,
            IntakeSize = await GetStageSize<Customer>(CanonSets.Intake),
            KeyedSize = await GetStageSize<Customer>(CanonSets.Keyed),
            ParkedSize = await GetStageSize<Customer>(CanonSets.Parked),
...
```csharp
public sealed class OrderCanonActionHandler : IMessageHandler<FlowAction>
{
    private readonly ICommandDispatcher _commands;
    private readonly ILogger<OrderCanonActionHandler> _log;

    public OrderCanonActionHandler(ICommandDispatcher commands, ILogger<OrderCanonActionHandler> log)
            ...
            try
            {
                using (DataSetContext.With(CanonSets.StageShort(CanonSets.Keyed)))
                {
                    ...
                    foreach (var rec in page)
                    {
                        ...
                        await Data<OrderOrchestrationView, string>.UpsertAsync(doc, CanonSets.ViewShort(ViewName), stoppingToken);
                    }
                }
            }
            catch (Exception ex)
...
public void ConfigureServices(IServiceCollection services)
{
    ...
    services.Replace(ServiceDescriptor.Singleton<IMessageHandler<FlowAction>, OrderCanonActionHandler>());
...
public interface IBusinessRule<TModel>
{
    string Name { get; }
    int Priority { get; }
    Task<RuleResult> EvaluateAsync(CanonEntity<TModel> entity, RuleContext context);
}
...
public class CustomerValidationRule : IBusinessRule<Customer>
{
    ...
    public async Task<RuleResult> EvaluateAsync(CanonEntity<Customer> entity, RuleContext context)
    {
...
public class RuleBasedProcessor<TModel> : IFlowProcessor<TModel>
{
    ...
    public async Task<ProcessingResult<TModel>> ProcessAsync(CanonEntity<TModel> entity, string stage)
    {
...
public record ProcessingResult<TModel>(
    bool Success,
    CanonEntity<TModel> Entity,
    string? ErrorMessage = null,
    object? Metadata = null)
{
    public static ProcessingResult<TModel> Success(CanonEntity<TModel> entity, object? metadata = null)
        => new(true, entity, null, metadata);
    public static ProcessingResult<TModel> Rejected(CanonEntity<TModel> entity, string error, object? metadata = null)
...
public sealed class CustomerRulesMonitor : ICanonMonitor<Customer>
{
    ...
    public async Task OnProjectedAsync(CanonMonitorContext context, CancellationToken ct)
    {
    ...
    var shim = new DynamicCanonEntity<Customer> { ReferenceId = context.ReferenceId, Model = (ExpandoObject)exp };
...
This comprehensive guide provides developers with everything needed to build sophisticated data processing pipelines with Koan.Canon. Start with the basic concepts and gradually adopt the advanced patterns as your requirements grow.

For additional resources, see:

- [S8.Canon Sample Application](https://github.com/sylin-labs/Koan-framework/tree/dev/samples/S8.Canon)
