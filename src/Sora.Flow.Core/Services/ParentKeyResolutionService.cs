...
public class ParentKeyResolutionService : BackgroundService
{
        ...
        CancellationToken ct)
    {
        ...
        var lookupTasks = parentKeys.Select(async parentKey =>
        {
            ...
            if (link != null)
            {
                var refUlid = identityLinkType.GetProperty("ReferenceUlid")?.GetValue(link) as string;
    ...
    private static object CopyModel(object original)
    {
        ...
        return Newtonsoft.Json.JsonConvert.DeserializeObject(json, original.GetType(), Sora.Core.Json.JsonDefaults.Settings) ?? new object();
    }
