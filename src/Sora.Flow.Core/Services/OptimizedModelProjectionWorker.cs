...
public class OptimizedModelProjectionWorker : BackgroundService
{
    ...
    private async Task<int> ProcessTaskTypeWithBulkOperations(Type taskType, int batchSize, CancellationToken ct)
    {
        try
        {
            ...
            // Group records by ReferenceUlid for batch projection processing
    ...
    private Dictionary<string, List<object>> GroupRecordsByReference(List<object> records)
    {
        ...
        {
            var referenceUlid = GetRecordReferenceUlid(record);
            if (!string.IsNullOrWhiteSpace(referenceUlid))
            {
                if (!groups.TryGetValue(referenceUlid, out var group))
                {
                    group = new List<object>();
                    groups[referenceUlid] = group;
                }
                group.Add(record);
            }
        }
    ...
    private async Task<(bool success, List<object> records, object? canonicalProjection, object? lineageProjection)> 
        ProcessReferenceProjection(string referenceUlid, List<object> records, Type modelType, CancellationToken ct)
    {
        try
        {
            // Create canonical projection by merging all records for this reference
            var canonicalProjection = await CreateCanonicalProjection(referenceUlid, records, modelType, ct);
            
            // Create lineage projection with full history
            var lineageProjection = await CreateLineageProjection(referenceUlid, records, modelType, ct);

            return (true, records, canonicalProjection, lineageProjection);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[optimized-projection] Error processing projection for reference {ReferenceUlid}", referenceUlid);
            return (false, records, null, null);
        }
    }

    private async Task<object> CreateCanonicalProjection(string referenceUlid, List<object> records, Type modelType, CancellationToken ct)
    {
        ...
        canonicalType.GetProperty("ReferenceUlid")?.SetValue(canonical, referenceUlid);
        canonicalType.GetProperty("Id")?.SetValue(canonical, referenceUlid);
    ...
    private async Task<object> CreateLineageProjection(string referenceUlid, List<object> records, Type modelType, CancellationToken ct)
    {
        var lineageType = typeof(LineageProjection<>).MakeGenericType(modelType);
        var lineage = Activator.CreateInstance(lineageType)!;

        // Set basic properties
        lineageType.GetProperty("ReferenceUlid")?.SetValue(lineage, referenceUlid);
        lineageType.GetProperty("Id")?.SetValue(lineage, referenceUlid);
    ...
    private string GetRecordReferenceUlid(object record)
    {
        return GetRecordProperty(record, "ReferenceUlid") ?? "";
    }
