...
public static class ParkedRecordExtensions
{
    ...
    /// and removing the parked record. This is the proper Sora.Flow pattern for 
        ...
        CancellationToken ct = default)
    {
        ...
        var effectiveCorrelationId = correlationId ?? parkedRecord.CorrelationId ?? Guid.NewGuid().ToString();
    ...
    private static TModel CopyModel<TModel>(TModel original) where TModel : new()
    {
        ...
        var json = Newtonsoft.Json.JsonConvert.SerializeObject(original, Sora.Core.Json.JsonDefaults.Settings);
        return Newtonsoft.Json.JsonConvert.DeserializeObject<TModel>(json, Sora.Core.Json.JsonDefaults.Settings) ?? new TModel();
    }
}