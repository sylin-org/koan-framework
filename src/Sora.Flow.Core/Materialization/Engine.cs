...
internal sealed class FlowMaterializer : IFlowMaterializer
{
    ...
    private IRecordMaterializationTransformer? GetRecordTransformer(string modelName)
    {
        return _recordCache.GetOrAdd(modelName, name =>
        {
            ...
            if (t is null)
            {
                WarnOncePolicy($"record:{modelName}", $"Sora.Flow materialization: record transformer type not found: {typeName} for model {modelName}");
                return null;
            }
            if (!typeof(IRecordMaterializationTransformer).IsAssignableFrom(t))
            {
                WarnOncePolicy($"record:{modelName}", $"Sora.Flow materialization: record transformer does not implement {nameof(IRecordMaterializationTransformer)}: {typeName}");
                return null;
            }
    ...
    private IPropertyMaterializationTransformer GetPropertyTransformer(string policyName)
    {
        return _policyCache.GetOrAdd(policyName, name =>
        {
            ...
            WarnOncePolicy(name, $"Sora.Flow materialization: unknown policy '{name}', falling back to '{BuiltInPolicies.Last}'");
    ...
    private void WarnOnceRecord(string modelName)
    {
        ...
        {
            _log.LogWarning("Sora.Flow materialization: record-level transformer configured for model {Model}. Property-level policies will be ignored for this model.", modelName);
        }
    }
