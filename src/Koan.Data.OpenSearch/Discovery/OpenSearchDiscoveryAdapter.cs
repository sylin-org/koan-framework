...
namespace Koan.Data.OpenSearch.Discovery;
...
internal sealed class OpenSearchDiscoveryAdapter : ServiceDiscoveryAdapterBase
{
    ...
    /// <summary>OpenSearch-specific discovery candidates with proper container-first priority</summary>
    protected override IEnumerable<DiscoveryCandidate> BuildDiscoveryCandidates(Koan.Orchestration.Attributes.KoanServiceAttribute attribute, DiscoveryContext context)
    {
        ...
        // Add OpenSearch-specific candidates from environment variables (highest priority)
        ...
        if (KoanEnv.InContainer)
        {
            ...
            if (!string.IsNullOrWhiteSpace(attribute.Host))
            {
                ...
                _logger.LogDebug("OpenSearch adapter: Added container candidate {ContainerUrl} (in container environment)", containerUrl);
            }
            ...
            if (!string.IsNullOrWhiteSpace(attribute.LocalHost))
            {
                ...
                _logger.LogDebug("OpenSearch adapter: Added local fallback {LocalUrl}", localhostUrl);
            }
        }
        else
        {
            ...
            if (!string.IsNullOrWhiteSpace(attribute.LocalHost))
            {
                ...
                _logger.LogDebug("OpenSearch adapter: Added local candidate {LocalUrl} (standalone environment)", localhostUrl);
            }
        }
        ...
        if (context.OrchestrationMode == OrchestrationMode.AspireAppHost)
        {
            ...
            if (!string.IsNullOrWhiteSpace(aspireUrl))
            {
                ...
                _logger.LogDebug("OpenSearch adapter: Added Aspire candidate {AspireUrl}", aspireUrl);
            }
        }

        // Apply OpenSearch-specific connection parameters if provided
        if (context.Parameters != null)
        {
            for (int i = 0; i < candidates.Count; i++)
            {
                if (!string.IsNullOrWhiteSpace(candidates[i].Url))
                {
                    ...
                    {
                        Url = BuildOpenSearchConnectionString(candidates[i].Url, context.Parameters)
                    };
                }
            }
        }
