...
internal static class EndpointFormatter
{
    ...
    {
        var host = string.IsNullOrWhiteSpace(p.Address) || p.Address == "0.0.0.0" || p.Address == "::" || p.Address == "[::]"
            ? "localhost"
        ...
        var (scheme, pattern) = _endpointResolver?.Invoke(p.Service, p.Container)
            ?? (_resolver is null
                ...
                : (_resolver.Invoke(p.Service, p.Container), (string?)null));
    ...
    public static string GetPlanHint(string serviceIdOrImage, int containerPort, int hostPort)
    {
        var (scheme, pattern) = _endpointResolver?.Invoke(serviceIdOrImage, containerPort)
            ?? (_resolver is null
                ...
                : (_resolver.Invoke(serviceIdOrImage, containerPort), (string?)null));
    ...
    public static void UseSchemeResolver(Func<string, int, string> resolver) => _resolver = resolver;
    ...
    public static void UseEndpointResolver(Func<string, int, (string Scheme, string? Pattern)> resolver) => _endpointResolver = resolver;
