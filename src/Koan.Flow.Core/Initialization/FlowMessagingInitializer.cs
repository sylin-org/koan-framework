...
namespace Koan.Flow.Initialization;
...
public static class FlowMessagingInitializer
{
    ...
    private static async Task ProcessFlowTransportEnvelope(string json)
    {
        try
        {
            ...
            try
            {
                var modelType = Koan.Flow.Infrastructure.FlowRegistry.ResolveModel(model);
                if (modelType != null)
                {
    ...
    private static async Task ProcessWithOrchestrator(Type modelType, string model, string referenceId, object payload, string source, string? correlationId)
    {
        try
        {
            ...
            var hasHandler = Koan.Flow.Core.Orchestration.Flow.HasHandler(modelType);
            ...
            if (hasHandler)
            {
                ...
                if (typedPayload is Koan.Data.Abstractions.IEntity<string> entity)
                {
                    ...
                    var handler = Koan.Flow.Core.Orchestration.Flow.GetHandler(modelType);
                    if (handler != null)
                    {
                        // Create metadata for handler
                        var updateMetadata = new Koan.Flow.Core.Orchestration.UpdateMetadata
                        {
                        ...
                        var result = await (Task<Koan.Flow.Core.Orchestration.UpdateResult>)method.Invoke(null, new object[] { handler, entity, null, updateMetadata })!;

                        if (result.Action == Koan.Flow.Core.Orchestration.UpdateAction.Skip)
                        {
    ...
    /// </summary>
    private static async Task<Koan.Flow.Core.Orchestration.UpdateResult> InvokeOrchestrator<T>(object handler, T proposed, T? current, Koan.Flow.Core.Orchestration.UpdateMetadata metadata) where T : class, Koan.Data.Abstractions.IEntity<string>
    {
        var typedHandler = (Koan.Flow.Core.Orchestration.UpdateHandler<T>)handler;
