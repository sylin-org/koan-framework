...
[ServiceEvent(Koan.Core.Events.KoanServiceEvents.Flow.EntityProcessed, EventArgsType = typeof(FlowEntityProcessedEventArgs))]
[ServiceEvent(Koan.Core.Events.KoanServiceEvents.Flow.EntityParked, EventArgsType = typeof(FlowEntityParkedEventArgs))]
[ServiceEvent(Koan.Core.Events.KoanServiceEvents.Flow.EntityFailed, EventArgsType = typeof(FlowEntityFailedEventArgs))]
public abstract class FlowOrchestratorBase : KoanFluentServiceBase, IFlowOrchestrator
{
    ...
    [ServiceAction(Koan.Core.Actions.KoanServiceActions.Flow.ProcessFlowEntity, RequiresParameters = true, ParametersType = typeof(object))]
    ...
    [ServiceAction(Koan.Core.Actions.KoanServiceActions.Flow.TriggerProcessing)]
    ...
    public virtual async Task ProcessFlowEntity(object transportEnvelope)
    {
        try
        {
            ...
            await EmitEventAsync(Koan.Core.Events.KoanServiceEvents.Flow.EntityProcessed, new FlowEntityProcessedEventArgs
            {
                Type = type,
                Model = model,
                Source = source,
        ...
        catch (Exception ex)
        {
            ...
            await EmitEventAsync(Koan.Core.Events.KoanServiceEvents.Flow.EntityFailed, new FlowEntityFailedEventArgs
            {
                Error = ex.Message,
    ...
    protected virtual async Task WriteToParked(Type modelType, string model, object payload, string source, string reasonCode, dynamic? metadata = null)
    {
        try
        {
            ...
            if (upsertMethod != null)
            {
                ...
                await EmitEventAsync(Koan.Core.Events.KoanServiceEvents.Flow.EntityParked, new FlowEntityParkedEventArgs
                {
                    Model = model,
