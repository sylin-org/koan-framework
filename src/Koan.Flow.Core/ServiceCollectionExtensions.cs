...
public static class ServiceCollectionExtensions
{
    ...
    private static IDictionary<string, object?>? ExtractDict(object? payload)
    {
        ...
        {
            return Koan.Flow.Model.DynamicFlowEntityExtensions.FlattenJObject(jObjectPayload);
        }
        ...
        if (payload is IDynamicFlowEntity dynamicEntity)
        {
            if (dynamicEntity.Model is JObject jObjectModel)
            {
                ...
                var flattened = Koan.Flow.Model.DynamicFlowEntityExtensions.FlattenJObject(jObjectModel);
                return flattened;
            }
            else
            {
    ...
    /// <summary>
    /// Registers Koan Flow core services and background workers.
    /// </summary>
    public static IServiceCollection AddKoanFlow(this IServiceCollection services)
    {
        // Options
        ...
        services.AddKoanFlowNaming();

        // Initialize Flow interceptor registry manager
        var provider = services.BuildServiceProvider();
        Koan.Flow.Core.Interceptors.FlowInterceptorRegistryManager.Initialize(provider);
        ...
        // Register FlowEntity message handler for "Koan.Flow.FlowEntity" queue
    ...
    /// </summary>
    public static IServiceCollection AddKoanFlowNaming(this IServiceCollection services)
    {
        services.OverrideStorageNaming((type, defaults) =>
        {
            ...
            if (def == typeof(Koan.Flow.Model.IdentityLink<>)) return modelFull + "#flow.identityLink";
            if (def == typeof(Koan.Flow.Model.KeyIndex<>)) return modelFull + "#flow.keyIndex";
            if (def == typeof(Koan.Flow.Model.ReferenceItem<>)) return modelFull + "#flow.reference";
            if (def == typeof(Koan.Flow.Model.ProjectionTask<>)) return modelFull + "#flow.tasks";
            if (def == typeof(Koan.Flow.Model.PolicyState<>)) return modelFull + "#flow.policies";
            // Root container should be just the model name (no suffix)
            if (def == typeof(Koan.Flow.Model.DynamicFlowEntity<>)) return modelFull;

            // Stage/View docs (set suffix appended via DataSetContext -> StorageNameRegistry)
            if (def == typeof(Koan.Flow.Model.StageRecord<>)) return modelFull;
            if (def == typeof(Koan.Flow.Model.ParkedRecord<>)) return modelFull;
            if (def == typeof(Koan.Flow.Model.CanonicalProjection<>)) return modelFull;
            if (def == typeof(Koan.Flow.Model.LineageProjection<>)) return modelFull;
                    ...
                    foreach (var modelType in models)
                    {
                        ...
                        while (!stoppingToken.IsCancellationRequested)
                        {
                            ...
                            foreach (var t in tasks)
                            {
                                ...
                                var ctx = new Koan.Flow.Monitoring.FlowMonitorContext(modelName, refId, mutableModel, mutablePolicies);
    ...
    internal sealed class ModelAssociationWorkerHostedService : BackgroundService
    {
        ...
        private static async Task<string?> TryResolveIdentityAsync(Type modelType, IDictionary<string, object?> dict, CancellationToken ct)
        {
            ...
            var idType = typeof(Koan.Flow.Model.IdentityLink<>).MakeGenericType(modelType);
        ...
        private async Task SaveRejectAndDrop(string code, object evidence, object rec, Type modelType, string intakeSet, CancellationToken ct)
        {
            ...
            await new Koan.Flow.Diagnostics.RejectionReport { Id = Guid.CreateVersion7().ToString("n"), ReasonCode = code, EvidenceJson = json, PolicyVersion = (string?)rec.GetType().GetProperty("PolicyVersion")?.GetValue(rec) }.Save(ct);
    ...
    private static async Task<string?> TryResolveParentViaExternalId(Type parentType, string sourceSystem, string parentExternalId, CancellationToken ct)
    {
        ...
        var idType = typeof(Koan.Flow.Model.IdentityLink<>).MakeGenericType(parentType);
        var idData = typeof(Data<,>).MakeGenericType(idType, typeof(string));
        var getM = idData.GetMethod("GetAsync", System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Static, new[] { typeof(string), typeof(CancellationToken) });
    ...
    private static bool HasFlowOrchestrators()
    {
        try
        {
            var orchestratorAttributeType = typeof(Koan.Flow.Attributes.FlowOrchestratorAttribute);
    ...
    /// Use this instead of AddKoanFlow for high-performance scenarios.
    /// </summary>
    public static IServiceCollection AddKoanFlowOptimized(this IServiceCollection services)
    {
        ...
        services.TryAddSingleton<Koan.Flow.Core.Services.AdaptiveBatchProcessor>();
        services.TryAddSingleton<Koan.Flow.Core.Monitoring.FlowPerformanceMonitor>();
        ...
        if (HasFlowOrchestrators())
        {
            ...
            services.TryAddEnumerable(ServiceDescriptor.Singleton<IHostedService, Koan.Flow.Core.Services.OptimizedModelAssociationWorker>());
            services.TryAddEnumerable(ServiceDescriptor.Singleton<IHostedService, Koan.Flow.Core.Services.OptimizedModelProjectionWorker>());

            // Start performance monitoring
            services.AddSingleton<IHostedService>(provider =>
            {
                var monitor = provider.GetRequiredService<Koan.Flow.Core.Monitoring.FlowPerformanceMonitor>();
    ...
    /// </summary>
    public static IServiceCollection AddKoanFlowWithOptimizations(
...
{
    private readonly Koan.Flow.Core.Monitoring.FlowPerformanceMonitor _monitor;

    public PerformanceMonitoringService(Koan.Flow.Core.Monitoring.FlowPerformanceMonitor monitor)
    {
        _monitor = monitor;
    }
