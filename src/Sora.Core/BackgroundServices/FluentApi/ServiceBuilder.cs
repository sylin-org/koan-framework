...
public class ServiceBuilder<T> : IServiceBuilder<T>, IServiceActionBuilder, IServiceEventBuilder
    where T : class, ISoraBackgroundService
{
    ...
    var service = serviceRegistry.GetService<T>();
        ...
        if (service is not SoraFluentServiceBase fluentService)
        {
            throw new InvalidOperationException($"Service {typeof(T).Name} must inherit from SoraFluentServiceBase for fluent API");
        }
...
/// </summary>
/// <typeparam name="T">The background service type</typeparam>
public class ServiceQueryBuilder<T> : IServiceQueryBuilder where T : class, ISoraBackgroundService
{
    ...
    public Task<ServiceInfo> GetInfoAsync()
    {
        ...
        var info = new ServiceInfo
        {
            ...
            SupportedCommands = service is ISoraPokableService pokable ? pokable.SupportedCommands.Select(c => c.Name).ToArray() : Array.Empty<string>()
        };

        return Task.FromResult(info);
    }

    private string[] GetSupportedActions(ISoraBackgroundService service)
    {
    ...
    private string[] GetSupportedEvents(ISoraBackgroundService service)
    {
        var type = service.GetType();
