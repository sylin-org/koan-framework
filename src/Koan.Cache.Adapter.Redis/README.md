...
Connection ownership currently comes from `Sylin.Koan.Data.Connector.Redis`, using
`Koan:Data:Redis:ConnectionString`. This shared-backend boundary is explicit and is being assessed for a future
backend-neutral Redis contract; applications should not register a second `IConnectionMultiplexer`.
...
See [TECHNICAL.md](TECHNICAL.md) for the current activation boundary.
