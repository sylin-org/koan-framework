...
public interface IAiModelManager
{
    ...
    Task<AiModelOperationResult> EnsureInstalled(AiModelOperationRequest request, CancellationToken ct = default);

    /// <summary>Forces a re-pull/update of a model regardless of current availability.</summary>
    Task<AiModelOperationResult> Refresh(AiModelOperationRequest request, CancellationToken ct = default);

    /// <summary>Removes a model from the provider.</summary>
    Task<AiModelOperationResult> Flush(AiModelOperationRequest request, CancellationToken ct = default);

    /// <summary>Lists models managed by this adapter (may be a subset of reported capabilities).</summary>
    Task<IReadOnlyList<AiModelDescriptor>> ListManagedModels(CancellationToken ct = default);
}
