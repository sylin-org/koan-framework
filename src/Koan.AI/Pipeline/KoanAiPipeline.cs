...
/// </summary>
internal sealed class KoanAiPipeline(
    IChatClient chatClient,
    IEmbeddingGenerator<string, Embedding<float>> embeddingGenerator,
    ILogger<KoanAiPipeline> logger) : IAiPipeline
{
    ...
    public async Task<AiChatResponse> Prompt(AiChatRequest request, CancellationToken ct = default)
    {
        ...
        logger.LogDebug("Koan AI pipeline prompt complete (model: {Model}, tokens out: {TokensOut})", mapped.Model ?? "unknown", mapped.TokensOut);
        return mapped;
    }
    ...
    public async IAsyncEnumerable<AiChatChunk> Stream(AiChatRequest request, [EnumeratorCancellation] CancellationToken ct = default)
    {
        ...
        logger.LogDebug("Koan AI pipeline streamed {ChunkCount} chunks", index);
    }
    ...
    public async Task<AiEmbeddingsResponse> Embed(AiEmbeddingsRequest request, CancellationToken ct = default)
    {
        ...
        var embeddings = await embeddingGenerator.GenerateAsync(request.Input, options, ct).ConfigureAwait(false);
        var mapped = ChatResponseMapper.ToAiEmbeddingsResponse(embeddings);

        logger.LogDebug("Koan AI pipeline generated embeddings (model: {Model}, count: {Count})", mapped.Model ?? "unknown", mapped.Vectors.Count);
        return mapped;
    }
