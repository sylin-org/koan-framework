...
internal sealed class HealthAnnouncements : IHealthAnnouncer, IHealthAnnouncementsStore
{
    ...
    private readonly Koan.Core.Observability.Health.HealthAggregatorOptions _opt;
    public HealthAnnouncements(Koan.Core.Observability.Health.IHealthAggregator agg, Koan.Core.Observability.Health.HealthAggregatorOptions opt)
    { _agg = agg; _opt = opt; }

    public void Healthy(string name)
    => _agg.Push(name, Koan.Core.Observability.Health.HealthStatus.Healthy, message: null, ttl: null, facts: null);

    public void Degraded(string name, string? description = null, IReadOnlyDictionary<string, object?>? data = null, TimeSpan? ttl = null)
    => _agg.Push(name, Koan.Core.Observability.Health.HealthStatus.Degraded, description, ttl, ToFacts(data));

    public void Unhealthy(string name, string? description = null, IReadOnlyDictionary<string, object?>? data = null, TimeSpan? ttl = null)
    => _agg.Push(name, Koan.Core.Observability.Health.HealthStatus.Unhealthy, description, ttl, ToFacts(data));
    ...
    private static HealthState Map(Koan.Core.Observability.Health.HealthStatus s) => s switch
    {
