...
public class WorkflowOrchestrator : KoanFluentServiceBase
{
    ...
    await KoanServices.On<TranslationService>(Koan.Core.Events.KoanServiceEvents.Translation.Started).Do<TranslationEventArgs>(async args =>
            {
                Logger.LogInformation("Workflow: Translation started for {FileId}", args.FileId);
                // Could trigger other services here
                await Task.CompletedTask;
            })
            .On(Koan.Core.Events.KoanServiceEvents.Translation.Completed).Do<TranslationEventArgs>(async args =>
            {
...
/// </summary>
[KoanPeriodicService(IntervalSeconds = 1800)] // 30 minutes
public class AdvancedPeriodicService : KoanPokablePeriodicServiceBase
{
