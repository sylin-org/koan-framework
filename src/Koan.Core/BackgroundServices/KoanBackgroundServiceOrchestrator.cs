...
/// <summary>
/// Orchestrates all Koan background services
/// </summary>
public class KoanBackgroundServiceOrchestrator : BackgroundService, IHealthContributor
{
    private readonly IServiceProvider _serviceProvider;
    private readonly ILogger<KoanBackgroundServiceOrchestrator> _logger;
    private readonly KoanBackgroundServiceOptions _options;
    private readonly List<ServiceExecutionContext> _runningServices = new();

    public string Name => "Koan-background-services";
    public bool IsCritical => true;

    public KoanBackgroundServiceOrchestrator(
        IServiceProvider serviceProvider,
        ILogger<KoanBackgroundServiceOrchestrator> logger,
        IOptionsMonitor<KoanBackgroundServiceOptions> options)
    {
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        _logger.LogKoanServices("starting...");
        ...
        try
        {
            ...
            var startupServices = backgroundServices.OfType<IKoanStartupService>().ToList();
            ...
            var regularServices = backgroundServices.Where(s => s is not IKoanStartupService).ToList();
    ...
    private async Task<List<string>> StartStartupServices(IEnumerable<IKoanStartupService> startupServices, CancellationToken cancellationToken)
    {
        ...
        foreach (var service in orderedServices)
        {
            ...
            catch (Exception ex)
            {
                ...
                var attr = service.GetType().GetCustomAttribute<KoanStartupServiceAttribute>();
    ...
    private async Task<List<string>> StartBackgroundServices(IEnumerable<IKoanBackgroundService> services, CancellationToken cancellationToken)
    {
        var startedServices = new List<string>();
    ...
    private bool ShouldStartService(IKoanBackgroundService service)
    {
        ...
        var attr = service.GetType().GetCustomAttribute<KoanBackgroundServiceAttribute>();
        if (attr != null)
        {
            if (!attr.Enabled)
            {
            ...
            var envName = KoanEnv.EnvironmentName;
    ...
    {
        public IKoanBackgroundService Service { get; init; } = null!;
