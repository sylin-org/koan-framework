...
/// <summary>
/// Deterministic topological sort of <see cref="KoanModule"/> types
...
/// whose target type doesn't derive from <see cref="KoanModule"/> throw
...
public static class ModuleOrdering
{
    ...
    /// <see cref="KoanModule"/>, or when the constraint graph contains
    /// a cycle. The message names the offending edges / cycle path.
    /// </exception>
    public static Type[] Sort(IEnumerable<Type> types) => Sort(
        types,
        static type => type.AssemblyQualifiedName ?? type.FullName ?? type.Name,
        Array.Empty<(Type From, Type To)>());

    internal static Type[] Sort(
        IEnumerable<Type> types,
        Func<Type, string> stableIdentity,
        IEnumerable<(Type From, Type To)> additionalEdges)
    {
        ArgumentNullException.ThrowIfNull(types);
        ArgumentNullException.ThrowIfNull(stableIdentity);
        ArgumentNullException.ThrowIfNull(additionalEdges);
        ...
        foreach (var edge in additionalEdges)
        {
            ...
            if (edge.From == edge.To) ThrowSelfReference(edge.From, "semantic ordering");
            edges.Add(edge);
        }
        ...
        if (!StableTopologicalOrder.TrySort(
                nodes,
                edges,
                new StableIdentityTypeComparer(stableIdentity),
                out var sorted,
                out var residual))
        {
    ...
    private static void ValidateTarget(Type source, Type target, string attributeName)
    {
        ...
        if (!typeof(KoanModule).IsAssignableFrom(target))
        {
            throw new InvalidOperationException(
                $"Koan ordering: {attributeName}({target.FullName}) on '{source.FullName}' is invalid — " +
                $"target does not derive from {nameof(KoanModule)}.");
        }
    }
    ...
    private sealed class StableIdentityTypeComparer(Func<Type, string> stableIdentity) : IComparer<Type>
    {
        public int Compare(Type? x, Type? y)
        {
            ...
            var byIdentity = string.Compare(stableIdentity(x), stableIdentity(y), StringComparison.Ordinal);
            return byIdentity != 0
                ? byIdentity
