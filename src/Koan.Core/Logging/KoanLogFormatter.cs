...
public class KoanLogFormatter : ConsoleFormatter
{
    private static readonly Regex KoanContextPattern = new(@"^\[(Koan:[^]]+|flow:[^]]+)\]", RegexOptions.Compiled);
    private const int ContextColumnWidth = 15;
    private readonly KoanLogContextRegistry? _contextRegistry;
    
    public KoanLogFormatter() : base("Koan") { }
    
    public KoanLogFormatter(KoanLogContextRegistry contextRegistry) : base("Koan") 
    {
    ...
    public override void Write<TState>(in LogEntry<TState> logEntry, IExternalScopeProvider? scopeProvider, TextWriter textWriter)
    {
        ...
        // Check if this is already a Koan-formatted message with context tags
        else if (KoanContextPattern.Match(message) is { Success: true } match)
        {
            FormatKoanMessage(logEntry, message, match, textWriter);
        }
        else
        {
    ...
    private static void FormatWithRegisteredContext<TState>(in LogEntry<TState> logEntry, string message, KoanLogContext context, TextWriter textWriter)
    {
    ...
    private static void FormatKoanMessage<TState>(in LogEntry<TState> logEntry, string message, Match contextMatch, TextWriter textWriter)
    {
