...
internal sealed class HealthAggregator : Koan.Core.Observability.Health.IHealthAggregator
{
    private readonly Koan.Core.Observability.Health.HealthAggregatorOptions _options;
    private readonly ConcurrentDictionary<string, Koan.Core.Observability.Health.HealthSample> _samples = new(StringComparer.OrdinalIgnoreCase);
    ...
    public HealthAggregator(Koan.Core.Observability.Health.HealthAggregatorOptions options)
    {
        _options = options;
    }
    ...
    public void Push(string component, Koan.Core.Observability.Health.HealthStatus status, string? message = null, TimeSpan? ttl = null, IReadOnlyDictionary<string, string>? facts = null)
    {
        ...
        var sample = new Koan.Core.Observability.Health.HealthSample(component, status, message, now, effectiveTtl, facts);
        _samples.AddOrUpdate(component, sample, (_, _) => sample);
        _lastSnapshotAt = now;
    }

    public Koan.Core.Observability.Health.HealthSnapshot GetSnapshot()
    {
        var now = DateTimeOffset.UtcNow;
        var list = new List<Koan.Core.Observability.Health.HealthSample>();
        foreach (var kv in _samples.ToArray())
        {
            ...
            if (s.Ttl is not null && (s.TimestampUtc + s.Ttl.Value) <= now)
            {
                ...
                s = s with { Status = Koan.Core.Observability.Health.HealthStatus.Unknown };
        ...
        Koan.Core.Observability.Health.HealthStatus overall = Koan.Core.Observability.Health.HealthStatus.Healthy;
        foreach (var s in list)
        {
            ...
            var status = s.Status;
            if (status == Koan.Core.Observability.Health.HealthStatus.Unknown)
            {
                ...
                if (_options.Policy.TreatUnknownAsDegradedForRequired && required)
                    status = Koan.Core.Observability.Health.HealthStatus.Degraded;
                else
                    continue;
            }
        ...
        return new Koan.Core.Observability.Health.HealthSnapshot(overall, list.OrderBy(x => x.Component).ToList(), now);
    }
