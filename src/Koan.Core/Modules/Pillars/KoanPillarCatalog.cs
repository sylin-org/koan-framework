...
public static class KoanPillarCatalog
{
    ...
    /// <summary>
    /// Declares a pillar. Every caller is a manifest stating what it owns.
    ///
    /// <para>Nothing registers a pillar it merely guessed at, and that is load-bearing rather than incidental.
    /// Provenance used to invent a descriptor for any pillar no manifest had declared and register it here;
    /// when the real manifest arrived its colour and icon differed, this refused it, and every host boot in the
    /// process failed from that point on. A description is not a declaration — see
    /// <c>ProvenanceRegistry.ResolveDescriptor</c>, which now describes without declaring.</para>
    /// </summary>
    ...
    private static PillarDescriptor MergeDescriptors(PillarDescriptor existing, PillarDescriptor incoming)
    {
        ...
        {
            // Two manifests describing one pillar differently is a genuine contradiction: both are claims of
            // ownership and the framework cannot pick. This is reachable only from declarations now, which is
            // why it can refuse without stopping a boot that had nothing wrong with it.
