...
using Koan.Services.Pillars;
using Koan.Services.ServiceMesh;
using Microsoft.Extensions.Configuration;
using Microsoft.Extensions.DependencyInjection;
using Microsoft.Extensions.Hosting;
using Microsoft.Extensions.Logging;

namespace Koan.Services.Initialization;
...
{
    public string ModuleName => "Koan.Services";
    ...
    private static void DescribeService(KoanServiceDescriptor descriptor, IConfiguration cfg, IHostEnvironment env)
    {
        ...
        var moduleName = $"Koan.Services.{ToPascalCase(descriptor.ServiceId)}";
        ...
        serviceWriter.SetSetting("Port", setting => setting
            ...
            .Source(ProvenanceSettingSource.AppSettings, $"Koan:Services:{descriptor.ServiceId}:Port"));

        serviceWriter.SetSetting("HeartbeatInterval", setting => setting
            .Value(descriptor.HeartbeatInterval.ToString())
            .Source(ProvenanceSettingSource.AppSettings, $"Koan:Services:{descriptor.ServiceId}:HeartbeatInterval"));

        serviceWriter.SetSetting("StaleThreshold", setting => setting
            .Value(descriptor.StaleThreshold.ToString())
            .Source(ProvenanceSettingSource.AppSettings, $"Koan:Services:{descriptor.ServiceId}:StaleThreshold"));
        ...
        if (!string.IsNullOrEmpty(descriptor.ContainerImage))
        {
            serviceWriter.SetSetting("ContainerImage", setting => setting
                ...
                .Source(ProvenanceSettingSource.AppSettings, $"Koan:Services:{descriptor.ServiceId}:ContainerImage"));
        }
    ...
    private static KoanServiceDescriptor BuildDescriptor(Type serviceType)
    {
        ...
        // Configuration path: Koan:Services:{ServiceId}:Port, etc.
