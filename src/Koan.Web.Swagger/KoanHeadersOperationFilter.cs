...
public sealed class KoanHeadersOperationFilter : IOperationFilter
{
    public void Apply(OpenApiOperation operation, OperationFilterContext context)
    {
        ...
        foreach (var resp in operation.Responses.Values)
        {
            ...
            if (!resp.Headers.ContainsKey(Koan.Web.Infrastructure.KoanWebConstants.Headers.KoanTraceId))
            {
                resp.Headers.Add(Koan.Web.Infrastructure.KoanWebConstants.Headers.KoanTraceId, new OpenApiHeader
                {
            ...
            if (!resp.Headers.ContainsKey("Koan-InMemory-Paging") && context.ApiDescription.HttpMethod?.ToUpperInvariant() == "GET")
            {
                resp.Headers.Add("Koan-InMemory-Paging", new OpenApiHeader
                {
