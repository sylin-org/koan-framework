...
public sealed class ViewsController : ControllerBase
{
    ...
    public async Task<IActionResult> GetOne([FromRoute] string view, [FromRoute] string referenceId, CancellationToken ct)
    {
        ...
        var set = Koan.Flow.Infrastructure.FlowSets.ViewShort(view);
        ...
        if (string.Equals(view, Koan.Flow.Infrastructure.Constants.Views.Canonical, StringComparison.OrdinalIgnoreCase))
        {
            // First try direct id fetch for each model
            foreach (var modelType in DiscoverModels())
            {
                var canType = typeof(Koan.Flow.Model.CanonicalProjection<>).MakeGenericType(modelType);
                var raw = await TryGetByIdAsync(canType, $"{Koan.Flow.Infrastructure.Constants.Views.Canonical}::{referenceId}", set, ct);
                if (raw is not null)
                {
        ...
        if (string.Equals(view, Koan.Flow.Infrastructure.Constants.Views.Lineage, StringComparison.OrdinalIgnoreCase))
        {
            foreach (var modelType in DiscoverModels())
            {
                var linType = typeof(Koan.Flow.Model.LineageProjection<>).MakeGenericType(modelType);
                var raw = await TryGetByIdAsync(linType, $"{Koan.Flow.Infrastructure.Constants.Views.Lineage}::{referenceId}", set, ct);
                if (raw is not null)
                {
    ...
    public async Task<IActionResult> GetPage([FromRoute] string view, [FromQuery] string? q, [FromQuery] int? page = 1, [FromQuery] int? size = 50, CancellationToken ct = default)
    {
        ...
        if (string.Equals(view, Koan.Flow.Infrastructure.Constants.Views.Canonical, StringComparison.OrdinalIgnoreCase))
        {
            var canonicalAll = await QueryCanonicalAcrossModels(null, Koan.Flow.Infrastructure.FlowSets.ViewShort(view), ct);
        ...
        if (string.Equals(view, Koan.Flow.Infrastructure.Constants.Views.Lineage, StringComparison.OrdinalIgnoreCase))
        {
            var lineageAll = await QueryLineageAcrossModels(null, Koan.Flow.Infrastructure.FlowSets.ViewShort(view), ct);
        ...
        var genericAll = await QueryGenericAcrossModels(view, null, Koan.Flow.Infrastructure.FlowSets.ViewShort(view), ct);
    ...
    private static async Task<List<Koan.Flow.Model.ProjectionView<object>>> QueryGenericAcrossModels(string viewName, string? q, string set, CancellationToken ct)
    {
        var acc = new List<Koan.Flow.Model.ProjectionView<object>>();
        foreach (var modelType in DiscoverModels())
        {
            var genViewType = typeof(Koan.Flow.Model.ProjectionView<,>).MakeGenericType(modelType, typeof(object));
            ...
            foreach (var it in result)
            {
                var doc = new Koan.Flow.Model.ProjectionView<object>
                {
