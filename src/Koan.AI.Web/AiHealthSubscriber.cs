...
internal sealed class AiHealthSubscriber : IHostedService
{
    ...
    public AiHealthSubscriber(Koan.Core.Observability.Health.IHealthAggregator agg, IAiAdapterRegistry registry, ILogger<AiHealthSubscriber> log)
    { _agg = agg; _registry = registry; _log = log; }

    public Task StartAsync(CancellationToken cancellationToken)
    {
        // Targeted subscription for component "ai"
        _sub = _agg.Subscribe("ai", e => { _ = Push(); });
    ...
    private Task Push()
    {
        try
        {
            ...
            _agg.Push(
                component: "ai",
                status: Koan.Core.Observability.Health.HealthStatus.Healthy,
                message: "ok",
        ...
        catch (Exception ex)
        {
            ...
            _agg.Push("ai", Koan.Core.Observability.Health.HealthStatus.Degraded, ex.Message, ttl: TimeSpan.FromSeconds(15));
        }
        return Task.CompletedTask;
    }
