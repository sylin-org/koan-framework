...
namespace Koan.Web.Auth.TestProvider.Hosting;
...
internal sealed class KoanTestProviderStartupFilter : IStartupFilter
{
    public Action<IApplicationBuilder> Configure(Action<IApplicationBuilder> next)
    {
        return app =>
        {
            const string appliedKey = "Koan.Web.Auth.TestProvider.Applied";
            if (!app.Properties.ContainsKey(appliedKey))
            {
                ...
                if (!enabled)
                {
                    logger?.LogDebug("Koan.Web.Auth.TestProvider disabled (env={Env}). Set '{Section}:{Key}=true' to enable outside Development.", env?.EnvironmentName, TestProviderOptions.SectionPath, nameof(TestProviderOptions.Enabled));
                }
                else
                {
                    ...
                    try
                    {
                        ...
                        logger?.LogInformation("Koan.Web.Auth.TestProvider endpoints mapped at '{Base}' (env={Env})", basePath, env?.EnvironmentName);
                    }
                    catch (Exception ex)
                    {
                        logger?.LogWarning(ex, "Koan.Web.Auth.TestProvider requested but endpoint mapping failed. Ensure routing is configured and MVC is registered.");
                    }
                }
            }
