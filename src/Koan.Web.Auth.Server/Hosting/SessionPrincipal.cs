...
internal static class SessionPrincipal
{
    public static TrustClaims ToTrustClaims(ClaimsPrincipal user, string clientId)
    {
        ...
        var roles = user.FindAll(ClaimTypes.Role).Select(c => c.Value).Distinct(StringComparer.Ordinal).ToArray();
        var permissions = user.FindAll(JwtClaimFactory.PermissionClaimType).Select(c => c.Value).Distinct(StringComparer.Ordinal).ToArray();
