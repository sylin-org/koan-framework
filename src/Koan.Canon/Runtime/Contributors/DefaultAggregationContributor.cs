    ...
    where TModel : CanonEntity<TModel>, new()
{
    ...
    public async ValueTask<CanonizationEvent?> Execute(CanonPipelineContext<TModel> context, CancellationToken cancellationToken)
    {
        ...
        {
            var index = CopyIndex(entry.Value) ?? new CanonIndex
            {
    ...
    private static CanonIndex? CopyIndex(CanonIndex? source)
    {
        if (source is null)
        {
            return null;
        ...
        if (!string.IsNullOrWhiteSpace(source.CanonicalId))
        {
            copy.Update(source.CanonicalId, source.Origin, source.Attributes);
        }

        return copy;
    }

    private static void QueueIndex(CanonPipelineContext<TModel> context, CanonIndex index)
    {
        if (!context.TryGetItem(PendingIndexesContextKey, out List<CanonIndex>? pending) || pending is null)
        {
            pending = [];
            context.SetItem(PendingIndexesContextKey, pending);
        }
