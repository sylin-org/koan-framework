...
namespace Koan.AI.Connector.LlamaCpp.Discovery;

internal sealed class LlamaCppDiscoveryAdapter(
    IConfiguration configuration,
    ILogger<LlamaCppDiscoveryAdapter> logger)
    : ServiceDiscoveryAdapterBase(configuration, logger)
{
    public override string ServiceName => Constants.Adapter.Type;
    public override string[] Aliases => ["llama-cpp","llama-server"];

    protected override Type GetFactoryType() => typeof(LlamaCppServiceDescriptor);

    protected override string? ReadExplicitConfiguration() =>
        _configuration.GetConnectionString("LlamaCpp");
