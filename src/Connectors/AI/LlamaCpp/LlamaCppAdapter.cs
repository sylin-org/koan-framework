...
internal sealed class LlamaCppAdapter :
    IChatAdapter,
    IEmbedAdapter,
    IAiSourceInspector,
    IAdapterReadiness,
    ...
    IDisposable
{
    ...
    public string DisplayName => "llama.cpp (llama-server) AI Provider";
    ...
    /// <summary>AI-level capabilities declared for AdapterResolver routing. llama-server's portable
    /// surface is the OpenAI-compatible one; tools stay undeclared (template-dependent), and there
    /// is no model manager to claim Pull/ModelRemove.</summary>
    ...
    public async Task<AiChatResponse> Chat(AiChatRequest request, CancellationToken ct = default)
    {
        ...
        {
            _logger.LogWarning("llama.cpp chat request failed ({Status}): {Body}", (int)response.StatusCode, body);
            response.EnsureSuccessStatusCode();
        }

        var doc = JsonConvert.DeserializeObject<ChatCompletionResponse>(body)
                  ?? throw new InvalidOperationException("llama-server returned an empty response.");
    ...
    public async Task<AiEmbeddingsResponse> Embed(AiEmbeddingsRequest request, CancellationToken ct = default)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            // llama-server only serves embeddings when started with --embedding; that refusal must
            // surface loudly here because /health stays green without the flag.
            _logger.LogWarning("llama.cpp embeddings request failed ({Status}): {Body}", (int)response.StatusCode, body);
            response.EnsureSuccessStatusCode();
        }

        var doc = JsonConvert.DeserializeObject<EmbeddingResponse>(body)
                  ?? throw new InvalidOperationException("llama-server returned an empty embedding response.");

        var vectors = doc.data?.Select(d => d.embedding?.ToArray() ?? []).ToList() ?? new List<float[]>();
        if (vectors.Count == 0)
            throw new InvalidOperationException("llama-server returned no embedding vectors.");
    ...
    public async Task WaitForReadiness(TimeSpan? timeout = null, CancellationToken ct = default)
    {
        ...
        {
            throw new AdapterNotReadyException(AdapterId, _stateManager.State, "llama.cpp readiness initialization failed.", initTask.Exception?.GetBaseException());
        }

        var effective = timeout ?? ReadinessTimeout;
        if (effective <= TimeSpan.Zero)
        {
            throw new AdapterNotReadyException(AdapterId, _stateManager.State, "llama.cpp readiness timeout is zero; readiness gating cannot wait.");
        }
        ...
        {
            throw new AdapterNotReadyException(AdapterId, _stateManager.State, $"Timed out waiting for llama.cpp readiness after {effective}.", ex);
        }
        ...
        {
            throw new AdapterNotReadyException(AdapterId, _stateManager.State, "llama.cpp adapter failed during readiness initialization.", initTask.Exception?.GetBaseException() ?? ex);
        }

        if (!_stateManager.IsReady)
        {
            throw new AdapterNotReadyException(AdapterId, _stateManager.State, "llama.cpp adapter is not ready after waiting for readiness.");
        }
    }
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogWarning(ex, "[{AdapterId}] llama.cpp adapter not ready after initialization (state={State})", AdapterId, ReadinessState);
        }
    }
    ...
    private async Task InitializeReadiness()
    {
        ...
        try
        {
            ...
            // /health answers 503 while the model loads, so it is the honest ready probe;
            // /v1/models then decides Ready (model listed) vs Degraded (health up, model absent).
            await VerifyHealthEndpoint(timeoutCts.Token);
        ...
        catch (OperationCanceledException ex)
        {
            ...
            _logger.LogError(ex, "[{AdapterId}] llama.cpp readiness timed out after {Timeout}", AdapterId, ReadinessTimeout);
            throw new InvalidOperationException($"llama.cpp readiness timed out after {ReadinessTimeout}.", ex);
        }
        catch (Exception ex)
        {
            _stateManager.TransitionTo(AdapterReadinessState.Failed);
            _logger.LogError(ex, "[{AdapterId}] llama.cpp readiness failed", AdapterId);
            throw;
        }
    }

    private async Task VerifyHealthEndpoint(CancellationToken ct)
    {
        using var request = new HttpRequestMessage(HttpMethod.Get, Constants.Discovery.HealthPath);
        AttachAuth(request);
    ...
    private void ApplyConfiguredBaseAddress()
    {
        ...
        {
            _logger.LogWarning("llama.cpp adapter: Ignoring invalid base URL '{BaseUrl}'", configured);
            return;
        }
    ...
    private string ResolveModel(string? requested)
    {
        ...
        throw new InvalidOperationException("llama.cpp adapter requires a model name when no default is configured.");
    }
    ...
    private async IAsyncEnumerable<ChatCompletionChunk?> ReadServerSentEvents(HttpResponseMessage response, [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            if (line.StartsWith("data:", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    _logger.LogDebug(ex, "Failed to deserialize llama.cpp stream payload: {Payload}", payload);
                }
