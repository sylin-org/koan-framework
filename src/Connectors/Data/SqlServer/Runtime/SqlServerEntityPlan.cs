    ...
    where TKey : notnull
{
    ...
    public SqlServerEntityPlan(MappingPlan mapping, SqlServerRepositoryOptions options, DataSegmentationPlan segmentation)
    {
        Mapping = mapping;
        Commands = new RelationalCommandPlanner(mapping);
        Dialect = new SqlServerDialect();
        Schema = mapping.Container.Namespace.LastOrDefault() ?? options.Schema;
        Table = mapping.Container.Name;
        QualifiedTable = $"{SqlServerDialect.Quote(Schema)}.{SqlServerDialect.Quote(Table)}";
    ...
    public SqlServerDialect Dialect { get; }
