...
namespace Koan.Data.Connector.Cockroach.Runtime;

internal sealed class CockroachInspector(CockroachRoute route) : IDataSourceInspectorAdapter
{
    ...
    public async Task<StorageContainerReference> Resolve(StorageAddress address, CancellationToken ct = default)
    {
        ...
        await using var connection = await Open(ct).ConfigureAwait(false);
        await using var command = new NpgsqlCommand(
            "SELECT table_type FROM information_schema.tables WHERE table_schema=@schema AND table_name=@table",
            connection);
        ...
        var kind = await command.ExecuteScalarAsync(ct).ConfigureAwait(false) as string
            ?? throw new KeyNotFoundException(
                $"CockroachDB container '{address}' does not exist on source '{route.Source}'.");
    ...
    private StorageContainerDescriptor Descriptor(
        string schema,
        string name,
        string kind,
        IReadOnlyList<DataField>? shape)
    {
        ...
        return new StorageContainerDescriptor(
            ...
            StorageContainerOperations.Query | (view ? StorageContainerOperations.None : StorageContainerOperations.Write),
            shape);
    }
    ...
    private static int Continuation(string? value) => string.IsNullOrWhiteSpace(value)
        ...
        : int.TryParse(value, out var offset) && offset >= 0
            ? offset
            : throw new ArgumentException("CockroachDB container continuation is invalid.", nameof(value));
}
