...
OpenSearch vector storage and similarity search for Koan Entities. Reference the package, keep the application's
...
this page owns OpenSearch setup and limits.
...
With OpenSearch reachable, no provider registration or repository scaffold is required:
...
When OpenSearch is the intended vector provider, it participates in the shared automatic election policy. If more
than one durable provider is referenced, pin business-critical placement with `[VectorAdapter("opensearch")]` or
...
{
  "ConnectionStrings": {
    "OpenSearch": "https://search.example.net:9200"
  },
  "Koan": {
    "Data": {
      "OpenSearch": {
        "Username": "koan-app",
        "Password": "use-your-secret-provider",
        "IndexPrefix": "catalog",
        "Dimension": 1536
      }
    }
...
`Koan:Data:OpenSearch:Endpoint` is the exact endpoint alternative to the connection-string entry. API-key
authentication is also available through `ApiKey`. Credentials belong in the platform's secret store.

## Capabilities

- automatic OpenSearch `knn_vector` mapping and index provisioning;
- single and bulk vector upsert/delete, clear, count, scroll export, and index statistics;
- native OpenSearch kNN search with caller-owned `topK`;
- metadata filtering pushed into the native kNN query, with unsupported operators rejected rather than ignored;
- source, partition, container, and tenant-aware index naming;
- selection-aware readiness and redacted startup reporting.

## Boundaries

- This connector stores vector representations and metadata; it is not a general OpenSearch Entity data adapter.
...
- OpenSearch index mappings fix the vector dimension at creation. Changing models normally requires a new index or
  a controlled reindex.
