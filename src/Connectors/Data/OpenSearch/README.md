...
OpenSearch vector storage for Koan Entities. The application declares one vector space; the connector realizes that
plan through OpenSearch `knn_vector`, native kNN search, and native metadata pre-filtering.
...
own access and storage lifecycle. OpenSearch configuration owns only placement, credentials, timeouts, and bounded
work limits.
...
HTTP Basic authentication uses `Username` and `Password`. `ConnectionStrings:OpenSearch` remains an endpoint
...
- Cosine, Euclidean, and unrestricted DotProduct spaces use explicit native mappings and normalized
  higher-is-closer scores;
...
- This is a Vector adapter, not a general OpenSearch Entity adapter.
- Search accuracy is approximate because OpenSearch uses indexed approximate kNN.
