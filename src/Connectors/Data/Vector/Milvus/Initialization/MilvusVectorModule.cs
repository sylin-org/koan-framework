...
public sealed class MilvusVectorModule : KoanModule
{
    public override void Register(IServiceCollection services)
    {
        services.AddKoanOptions<MilvusOptions>(Infrastructure.Constants.Configuration.Section);
        ...
        services.AddSingleton<MilvusVectorAdapterFactory>();
        services.AddSingleton<IVectorAdapterFactory>(static services =>
            services.GetRequiredService<MilvusVectorAdapterFactory>());
    ...
    public override void Report(ProvenanceModuleWriter module, IConfiguration configuration, IHostEnvironment environment)
    {
        ...
        var defaults = new MilvusOptions();
        ...
        module.AddSetting("Endpoint", Redaction.DeIdentify(endpoint.Value), source: endpoint.Source,
            consumers: [typeof(MilvusVectorAdapterFactory).FullName!], sourceKey: endpoint.ResolvedKey);
        module.AddSetting("TimeoutSeconds", timeout.Value.ToString(CultureInfo.InvariantCulture), source: timeout.Source,
            consumers: [typeof(MilvusClient).FullName!], sourceKey: timeout.ResolvedKey);
        module.AddNote("Vector shape and visibility come from the immutable Koan VectorSpacePlan; Milvus options own placement and bounds only.");
    }
}
