# Koan.Secrets.Connector.Vault - TECHNICAL
...
- Auto-registrar (`Initialization/KoanAutoRegistrar.cs`):
  ...
  - Registers typed `HttpClient` named `Koan.Secrets.Connector.Vault` with base address, `X-Vault-Token`, and optional `X-Vault-Namespace`.
...
- Manual registration (when not using StartKoan):
  ...
  - `services.AddHttpClient("Koan.Secrets.Connector.Vault", ...)` with headers.
