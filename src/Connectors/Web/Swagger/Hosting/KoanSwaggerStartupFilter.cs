...
internal sealed class KoanSwaggerStartupFilter : IStartupFilter
{
    public Action<IApplicationBuilder> Configure(Action<IApplicationBuilder> next)
    {
        return app =>
        {
            const string appliedKey = "Koan.Web.Connector.Swagger.Applied";
            if (!app.Properties.ContainsKey(appliedKey))
            {
                ...
                {
                    logger?.LogInformation("Koan.Web.Connector.Swagger disabled (env={Env}). Set 'Koan:Web:Swagger:Enabled=true' or 'Koan:AllowMagicInProduction=true' to enable.", env?.EnvironmentName);
                }
            }
