...
namespace Koan.Orchestration.Renderers.Connector.Compose;
    ...
    static Dictionary<string, List<string>> DiscoverHostMounts()
    {
        try
        {
            ...
            var assemblies = AppDomain.CurrentDomain.GetAssemblies()
                .Where(a =>
                {
                    try { var n = a.GetName().Name; return n is not null && n.StartsWith("Koan.", StringComparison.OrdinalIgnoreCase); }
                    catch { return false; }
                })
                .ToArray();
