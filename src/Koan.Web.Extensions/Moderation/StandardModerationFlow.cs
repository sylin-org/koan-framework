...
public sealed class StandardModerationFlow<TEntity> : IModerationFlow<TEntity>, IModerationValidator<TEntity>
{
	...
	public ValueTask<FlowResult> ValidateApprove(TransitionContext<TEntity> ctx, CancellationToken ct)
	{
		...
		return ValueTask.FromResult(ctx.SubmittedSnapshot is null
			? FlowResult.Fail(StatusCodes.Status404NotFound, KoanWebConstants.Codes.Moderation.NotFound, "Submitted snapshot not found for approval.")
			: FlowResult.Success);
	}
	...
	public ValueTask<FlowResult> ValidateReject(TransitionContext<TEntity> ctx, CancellationToken ct)
	{
		if (ctx.SubmittedSnapshot is null)
			return ValueTask.FromResult(FlowResult.Fail(StatusCodes.Status404NotFound, KoanWebConstants.Codes.Moderation.NotFound, "Submitted snapshot not found for rejection."));
		if (ctx.Options is RejectOptions ro && string.IsNullOrWhiteSpace(ro.Reason))
			return ValueTask.FromResult(FlowResult.Fail(StatusCodes.Status400BadRequest, KoanWebConstants.Codes.Moderation.ReasonRequired, "A reason is required to reject."));
	...
	{
		if (ctx.SubmittedSnapshot is null)
			return ValueTask.FromResult(FlowResult.Fail(StatusCodes.Status404NotFound, KoanWebConstants.Codes.Moderation.NotFound, "Submitted snapshot not found for return."));
		if (ctx.Options is RejectOptions ro && string.IsNullOrWhiteSpace(ro.Reason))
			return ValueTask.FromResult(FlowResult.Fail(StatusCodes.Status400BadRequest, KoanWebConstants.Codes.Moderation.ReasonRequired, "A reason is required to return to draft."));
		return ValueTask.FromResult(FlowResult.Success);
	}
