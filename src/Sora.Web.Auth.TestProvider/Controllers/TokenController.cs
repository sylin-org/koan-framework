...
[ApiController]
public sealed class TokenController(IOptionsSnapshot<TestProviderOptions> opts, DevTokenStore store, IHostEnvironment env) : ControllerBase
{
    ...
    public IActionResult Token([FromForm] TokenRequest req)
    {
        ...
        if (!store.TryRedeemCode(req.code, out var profile, out var challenge)) return BadRequest(new { error = "invalid_grant" });
