...
- Pebble (Pond CA keypair) is replicated to all stones
...
#### Zen Syntax
```bash
garden-rake place pebble                     # Initialize pond security
garden-rake lift pebble                      # Remove pond entirely
...
#### API
```http
POST /api/v1/pond/init                       # Initialize pond (creates pebble)
...
Creating pebble (encrypted with passphrase)...

Enter passphrase (20+ characters): **********************
Confirm passphrase: **********************

✓ Pond initialized on stone-01 (Cornerstone)
✓ Pebble stored: /var/lib/zen-garden/pebble
...
stone-01 responded with pebble
...
You can re-secure later with: garden-rake place pebble
```
...
#### Pond Security
```http
POST   /api/v1/pond/init                     # Initialize pond (create pebble)
DELETE /api/v1/pond                          # Remove pond from all stones
POST   /api/v1/pond/invite                   # Generate TOTP invitation code
POST   /api/v1/pond/join                     # Join pond with code
DELETE /api/v1/pond/stones/:stone_name       # Remove stone from pond
...
{
  "cornerstone": "stone-01",
  "pebble_path": "/var/lib/zen-garden/pebble",
  "certificate_expires": "2026-01-17T11:30:00Z",
  "status": "active",
...
| Initialize pond | `place pebble` | `pond init` | `POST /api/v1/pond/init` |
| Remove pond | `lift pebble` | `pond remove` | `DELETE /api/v1/pond` |
...
COMMANDS:
    Zen Verbs (natural syntax):
      ...
      place          Place pebble (init pond) or stone (join pond)
      lift           Lift pebble (remove pond) or stone (untrust)
...
- [ ] `place pebble` / `pond init` - Initialize pond with pebble creation
...
- [ ] `lift pebble` / `pond remove` - Remove pond from all stones
