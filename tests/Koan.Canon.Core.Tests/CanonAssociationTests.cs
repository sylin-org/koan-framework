...
public class CanonAssociationTests
{
    ...
    // Wire Koan AppHost for DataService resolution in AggregateExtensions
    ...
    [Fact(Skip = "Legacy Canon tests disabled in greenfield runtime; update to typed model tests later.")]
    ...
    var sp = await CreateHostAsync(dir, Koan.Testing.Canon.CanonTestConstants.UbiquitousAggregationTags);
        var ct = CancellationToken.None;

        using (DataSetContext.With(Constants.Sets.Intake))
        {
            await new Koan.Canon.Model.Record
            {
                SourceId = "src1",
    ...
    await new KeyIndex { AggregationKey = Koan.Testing.Canon.CanonTestConstants.Samples.EmailA, ReferenceUlid = "refA" }.Save(ct);
    await new KeyIndex { AggregationKey = Koan.Testing.Canon.CanonTestConstants.Samples.EmailB, ReferenceUlid = "refB" }.Save(ct);
        ...
        using (DataSetContext.With(Constants.Sets.Intake))
        {
            await new Koan.Canon.Model.Record
            {
                SourceId = "src1",
                OccurredAt = DateTimeOffset.UtcNow,
                Data = new Dictionary<string, object> { { Koan.Testing.Canon.CanonTestConstants.Keys.Email, new object[] { Koan.Testing.Canon.CanonTestConstants.Samples.EmailA, Koan.Testing.Canon.CanonTestConstants.Samples.EmailB } } }
            }.Save(ct);
        }
    ...
    var rejects = await RejectionReport.All(ct);
        ...
        (await Koan.Canon.Model.Record.All(Constants.Sets.Keyed, ct)).Should().BeEmpty();
    }
    ...
    var sp = await CreateHostAsync(dir, Koan.Testing.Canon.CanonTestConstants.UbiquitousAggregationTags);
        ...
        using (DataSetContext.With(Constants.Sets.Intake))
        {
            await new Koan.Canon.Model.Record
            {
                SourceId = "src1",
                OccurredAt = DateTimeOffset.UtcNow,
                Data = new Dictionary<string, object> { { Koan.Testing.Canon.CanonTestConstants.Keys.Email, Koan.Testing.Canon.CanonTestConstants.Samples.EmailA }, { "name", "Ann" } }
            }.Save(ct);
        }
    ...
    var allKeyed = await Koan.Canon.Model.Record.All(Constants.Sets.Keyed, ct);
