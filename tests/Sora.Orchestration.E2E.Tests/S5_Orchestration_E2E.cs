...
public class S5_Orchestration_E2E
{
    ...
    public void Export_Compose_For_S5_Should_Succeed()
    {
        ...
        // Act: run `dotnet run --project src/Sora.Orchestration.Cli -- export compose --profile Local`
    ...
    {
        // Gate running containers in CI or dev machines. Opt-in via SORA_E2E_RUN_CONTAINERS=true
        var enabled = Environment.GetEnvironmentVariable("SORA_E2E_RUN_CONTAINERS");
    ...
    {
        // Writes repo-root/sora.orchestration.json and cleans it up on dispose
        var path = Path.Combine(repoRoot, "sora.orchestration.json");
    ...
    {
        // Run as `dotnet run --project src/Sora.Orchestration.Cli -- <args>`
        var project = Path.Combine(repoRoot, "src", "Sora.Orchestration.Cli", "Sora.Orchestration.Cli.csproj");
        ...
        psi.Environment["SORA_PORT_PROBE_MAX"] = "50";
