...
public sealed class PostgresAutoFixture : IRelationalTestFixture<PostgresSchemaGovernanceSharedTests.Todo, string>, IAsyncLifetime, IDisposable
{
    ...
    public async Task InitializeAsync()
    {
        ...
        var envCxn = Environment.GetEnvironmentVariable("Koan_POSTGRES__CONNECTION_STRING")
        ...
        else
        {
            ...
            try
            {
                ...
                var builder = new TestcontainersBuilder<TestcontainersContainer>()
                    ...
                    .WithEnvironment("POSTGRES_DB", "Koan")
                    .WithPortBinding(54329, 5432)
                    .WithWaitStrategy(Wait.ForUnixContainer().UntilPortIsAvailable(5432));

                _container = builder.Build();
                await _container.StartAsync();
                ConnectionString = $"Host=localhost;Port=54329;Database=Koan;Username=postgres;Password={password};Timeout=3";
            }
            catch (Exception ex)
            {
                SkipTests = true;
    ...
    // Docker probing moved to Koan.Testing.DockerEnvironment
