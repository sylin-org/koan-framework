...
/// <summary>
/// Comprehensive count tests for Postgres adapter.
/// Verifies CountRequest/CountResult contract, CountStrategy behavior,
/// IsEstimate flag, fast count optimization, and edge cases.
/// </summary>
public class PostgresCountTests : KoanTestBase
{
    ...
    private IServiceProvider BuildPostgresServices()
    {
        AggregateConfigs.Reset();
        var connString = Environment.GetEnvironmentVariable("POSTGRES_CONNECTION_STRING")
            ?? "Host=localhost;Database=koan_test;Username=postgres;Password=postgres";
        ...
        return BuildServices(services =>
        {
            var cfg = new ConfigurationBuilder()
                ...
                {
                    new KeyValuePair<string, string?>("PostgresOptions:ConnectionString", connString),
                    new KeyValuePair<string, string?>("Koan_DATA_PROVIDER", "postgresql")
                })
                .Build();

            services.AddSingleton<IConfiguration>(cfg);
            services.AddPostgresAdapter(o => o.ConnectionString = connString);
    ...
    public async Task EntityCount_Fast_UsesMetadataWhenAvailable()
    {
        ...
        // Assert
        count.Should().BeGreaterThan(0, "Entity.Count.Fast() should use pg_stat metadata");
    ...
    public async Task FastCount_SetsIsEstimateTrue()
    {
        ...
        // Assert
        result.IsEstimate.Should().BeTrue("Fast count using pg_stat should set IsEstimate = true");
    ...
    public async Task CountStrategy_Fast_UsesMetadataWhenPossible()
    {
        ...
        // Assert
        result.Value.Should().BeGreaterThan(0, "Fast strategy should use pg_stat");
        result.IsEstimate.Should().BeTrue("pg_stat provides estimates");
    }
    ...
    [Fact]
    public async Task Postgres_FastCount_UsesPgStatUserTables()
    {
        // Arrange
        using var _ = EntityContext.Partition("pgstat-" + Guid.NewGuid().ToString("N")[..8]);
        var sp = BuildPostgresServices();
        var data = sp.GetRequiredService<IDataService>();
        var repo = data.GetRepository<CountTestEntity, string>();
        await data.Execute<CountTestEntity, int>(new Abstractions.Instructions.Instruction("data.clear"));

        // Insert data to ensure pg_stat has entries
        for (int i = 0; i < 20; i++)
        {
            await new CountTestEntity { Name = $"Item{i}", Value = i }.Save();
        }

        // Act - Fast count should query pg_stat_user_tables
        var result = await repo.CountAsync(new CountRequest<CountTestEntity>
        {
            Options = new DataQueryOptions { CountStrategy = CountStrategy.Fast }
        });
        ...
        // Assert
        result.Value.Should().BeGreaterThan(0, "pg_stat should report some rows");
        result.IsEstimate.Should().BeTrue("pg_stat provides estimates, not exact counts");
    }
    ...
    public async Task FastCount_WhenMetadataUnavailable_FallbacksToExact()
    {
        ...
        // Act - Even if pg_stat fails, should fallback gracefully
        var result = await repo.CountAsync(new CountRequest<CountTestEntity>
        {
            Options = new DataQueryOptions { CountStrategy = CountStrategy.Fast }
        });
    ...
    public async Task Count_EmptyTable_ReturnsZero()
    {
        ...
        // Assert
        exactResult.Value.Should().Be(0, "Exact count on empty table should be 0");
        fastResult.Value.Should().BeGreaterThanOrEqualTo(0, "Fast count on empty table should be >= 0");
    }
