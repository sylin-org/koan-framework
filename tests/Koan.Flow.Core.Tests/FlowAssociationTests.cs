...
public class FlowAssociationTests
{
    ...
    var sp = await CreateHostAsync(dir, Koan.Testing.Flow.FlowTestConstants.UbiquitousAggregationTags);
        var ct = CancellationToken.None;

        using (DataSetContext.With(Constants.Sets.Intake))
        {
            await new Koan.Flow.Model.Record
            {
                SourceId = "src1",
    ...
    await new KeyIndex { AggregationKey = Koan.Testing.Flow.FlowTestConstants.Samples.EmailA, ReferenceUlid = "refA" }.Save(ct);
    await new KeyIndex { AggregationKey = Koan.Testing.Flow.FlowTestConstants.Samples.EmailB, ReferenceUlid = "refB" }.Save(ct);
        ...
        using (DataSetContext.With(Constants.Sets.Intake))
        {
            await new Koan.Flow.Model.Record
            {
                SourceId = "src1",
                OccurredAt = DateTimeOffset.UtcNow,
                Data = new Dictionary<string, object> { { Koan.Testing.Flow.FlowTestConstants.Keys.Email, new object[] { Koan.Testing.Flow.FlowTestConstants.Samples.EmailA, Koan.Testing.Flow.FlowTestConstants.Samples.EmailB } } }
            }.Save(ct);
        }
    ...
    var rejects = await RejectionReport.All(ct);
        ...
        (await Koan.Flow.Model.Record.All(Constants.Sets.Keyed, ct)).Should().BeEmpty();
    }
    ...
    var sp = await CreateHostAsync(dir, Koan.Testing.Flow.FlowTestConstants.UbiquitousAggregationTags);
        ...
        using (DataSetContext.With(Constants.Sets.Intake))
        {
            await new Koan.Flow.Model.Record
            {
                SourceId = "src1",
                OccurredAt = DateTimeOffset.UtcNow,
                Data = new Dictionary<string, object> { { Koan.Testing.Flow.FlowTestConstants.Keys.Email, Koan.Testing.Flow.FlowTestConstants.Samples.EmailA }, { "name", "Ann" } }
            }.Save(ct);
        }
    ...
    var allKeyed = await Koan.Flow.Model.Record.All(Constants.Sets.Keyed, ct);
