...
public class RolesAdminControllerTests
{
    ...
    services.AddSingleton<IRolePolicyBindingStore>(new InMemoryRolePolicyBindingStore());
        ...
        services.AddSingleton<IRoleConfigSnapshotProvider, Sora.Web.Auth.Roles.Services.DefaultRoleConfigSnapshotProvider>();
    ...
    {
        private readonly Dictionary<string, ISoraAuthRole> _items = new(StringComparer.OrdinalIgnoreCase);
        public Task<IReadOnlyList<ISoraAuthRole>> All(CancellationToken ct = default) => Task.FromResult<IReadOnlyList<ISoraAuthRole>>(_items.Values.ToList());
        public Task UpsertMany(IEnumerable<ISoraAuthRole> items, CancellationToken ct = default)
        {
    ...
    {
        private readonly Dictionary<string, ISoraAuthRoleAlias> _items = new(StringComparer.OrdinalIgnoreCase);
        public Task<IReadOnlyList<ISoraAuthRoleAlias>> All(CancellationToken ct = default) => Task.FromResult<IReadOnlyList<ISoraAuthRoleAlias>>(_items.Values.ToList());
        public Task UpsertMany(IEnumerable<ISoraAuthRoleAlias> items, CancellationToken ct = default)
        {
            foreach (var i in items) _items[i.Id] = i;
            return Task.CompletedTask;
        }
    ...
    {
        private readonly Dictionary<string, ISoraAuthRolePolicyBinding> _items = new(StringComparer.OrdinalIgnoreCase);
        public Task<IReadOnlyList<ISoraAuthRolePolicyBinding>> All(CancellationToken ct = default) => Task.FromResult<IReadOnlyList<ISoraAuthRolePolicyBinding>>(_items.Values.ToList());
        public Task UpsertMany(IEnumerable<ISoraAuthRolePolicyBinding> items, CancellationToken ct = default)
        {
            foreach (var i in items) _items[i.Id] = i;
            return Task.CompletedTask;
        }
