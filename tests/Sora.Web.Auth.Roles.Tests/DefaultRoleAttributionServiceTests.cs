...
using Sora.Web.Auth.Roles.Services;
...
public class DefaultRoleAttributionServiceTests
{
    ...
    public async Task Alias_mapping_prefers_snapshot_over_options()
    {
        ...
        var aliasStore = new InMemoryAliasStore(new Dictionary<string, ISoraAuthRoleAlias>
        {
    ...
    private sealed class Alias(string id, string target) : ISoraAuthRoleAlias
    {
        public string Id { get; set; } = id;
        public string TargetRole { get; set; } = target;
        public byte[]? RowVersion { get; set; }
    }
    private sealed class InMemoryAliasStore(Dictionary<string, ISoraAuthRoleAlias>? seed = null) : IRoleAliasStore
    {
        private readonly Dictionary<string, ISoraAuthRoleAlias> _items = seed ?? new(StringComparer.OrdinalIgnoreCase);
        ...
        public Task UpsertMany(IEnumerable<ISoraAuthRoleAlias> items, CancellationToken ct = default) { foreach (var i in items) _items[i.Id] = i; return Task.CompletedTask; }
    ...
    private sealed class InMemoryBindingStore : IRolePolicyBindingStore
    {
        ...
        public Task UpsertMany(IEnumerable<ISoraAuthRolePolicyBinding> items, CancellationToken ct = default) { foreach (var i in items) _items[i.Id] = i; return Task.CompletedTask; }
        public Task<bool> Delete(string id, CancellationToken ct = default) => Task.FromResult(_items.Remove(id));
    }
}
