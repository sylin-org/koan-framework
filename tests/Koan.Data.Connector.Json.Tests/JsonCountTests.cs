...
/// <summary>
/// Comprehensive count tests for JSON adapter.
/// Verifies CountRequest/CountResult contract, CountStrategy behavior,
/// IsEstimate flag (always false for JSON), and edge cases.
/// JSON adapter always performs exact counts (file-based, no metadata).
/// </summary>
public class JsonCountTests : KoanTestBase
{
    ...
    private IServiceProvider BuildJsonServices(string dir)
    {
        return BuildServices(services =>
        {
            ...
            services.AddJsonData<CountTestEntity, string>(o => { o.DirectoryPath = dir; });
    ...
    {
        var d = Path.Combine(Path.GetTempPath(), "Koan-json-count-tests", Guid.NewGuid().ToString("n"));
    ...
    [Fact]
    public async Task EntityCount_Fast_FallbacksToExactInJson()
    {
        // Arrange
        ...
        // Assert
        count.Should().Be(10, "Entity.Count.Fast() in JSON fallbacks to exact count");
    }
    ...
    #region P0: Critical - IsEstimate Flag Tests (JSON always exact)
    ...
    [Fact]
    public async Task FastCount_SetsIsEstimateFalse_JsonHasNoMetadata()
    {
        // Arrange
        ...
        // Assert
        result.IsEstimate.Should().BeFalse("JSON has no metadata, Fast fallbacks to exact with IsEstimate = false");
    ...
    public async Task OptimizedCount_SetsIsEstimateFalse()
    {
        ...
        // Assert
        result.IsEstimate.Should().BeFalse("JSON always returns exact counts");
        result.Value.Should().Be(1);
    }
    ...
    [Fact]
    public async Task CountStrategy_Fast_FallbacksToExactInJson()
    {
        // Arrange
        ...
        // Assert
        result.Value.Should().Be(10, "Fast strategy fallbacks to exact in JSON");
        result.IsEstimate.Should().BeFalse("JSON has no metadata, always exact");
    }

    [Fact]
    public async Task CountStrategy_Optimized_UsesExactInJson()
    {
        // Arrange
        ...
        // Assert
        result.Value.Should().Be(1);
        result.IsEstimate.Should().BeFalse("JSON optimized uses exact counting");
    }
    ...
    [Fact]
    public async Task Count_EmptyFile_ReturnsZero()
    {
        // Arrange
        ...
        // Assert
        exactResult.Value.Should().Be(0, "Exact count on empty file should be 0");
        fastResult.Value.Should().Be(0, "Fast count on empty file should also be 0 in JSON");
