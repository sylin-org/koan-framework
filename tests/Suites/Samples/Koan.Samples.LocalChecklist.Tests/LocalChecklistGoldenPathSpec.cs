...
namespace Koan.Samples.LocalChecklist.Tests;

public sealed class LocalChecklistGoldenPathSpec
{
    [Fact]
    public async Task Console_process_persists_explains_and_exits_cleanly()
    {
        var projectDirectory = ProjectDirectory();
        var dataDirectory = Path.Combine(Path.GetTempPath(), "koan-local-checklist", Guid.NewGuid().ToString("N"));
