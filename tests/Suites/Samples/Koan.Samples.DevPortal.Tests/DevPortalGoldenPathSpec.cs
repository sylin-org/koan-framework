...
public sealed class DevPortalGoldenPathSpec(DevPortalFixture fixture) : IClassFixture<DevPortalFixture>
{
    [Fact]
    public async Task Fresh_host_publishes_the_approved_set_idempotently_and_explains_its_default()
    {
        ...
        using var navigation = new HttpRequestMessage(HttpMethod.Get, "/articles/article-composition");
        navigation.Headers.Accept.ParseAdd("text/html");
        var socialCard = await client.SendAsync(navigation);
        socialCard.StatusCode.Should().Be(HttpStatusCode.OK);
        var socialHtml = await socialCard.Content.ReadAsStringAsync();
        socialHtml.Should().Contain("<meta property=\"og:title\" content=\"Composition that explains itself\"");
        socialHtml.Should().Contain("<meta property=\"og:description\" content=\"How references, startup decisions, and facts describe one runtime.\"");
