...
namespace Koan.Tests.Cache.Unit.Specs.Decorators;

public sealed class CacheRepositoryDecoratorSpec
{
    private readonly ITestOutputHelper _output;

    public CacheRepositoryDecoratorSpec(ITestOutputHelper output)
    {
        _output = output;
    }

    [Fact]
    public Task TryDecorate_with_entity_policy_wraps_repository()
        => Spec(nameof(TryDecorate_with_entity_policy_wraps_repository), () =>
        {
    ...
    private sealed class CacheTestContext : IDisposable
    {
        ...
        public static CacheTestContext Create()
        {
            ...
            var client = new CacheClient(
                ...
                instrumentation,
                new Support.TestOptionsMonitor<CacheOptions>(new CacheOptions()),
    ...
    public bool TryGetPolicy(Type targetType, [NotNullWhen(true)] out CachePolicyDescriptor? descriptor)
        {
            if (targetType == typeof(TestEntity))
            {
