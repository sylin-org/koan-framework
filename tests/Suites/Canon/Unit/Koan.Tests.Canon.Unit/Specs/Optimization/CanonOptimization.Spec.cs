...
{
    [Fact]
    public void Production_preset_prioritizes_throughput()
    {
    ...
    [Fact]
    public void Development_preset_remains_cautious()
    {
    ...
    [Fact]
    public async Task Adaptive_batch_processor_adjusts_within_bounds()
    {
        ...
        await processor.RecordMetrics(TimeSpan.FromMilliseconds(200), 500, 50 * 1024 * 1024);
        var increased = await processor.GetOptimalBatchSize();
        increased.Should().BeInRange(100, options.Performance.MaxBatchSize);

        await processor.RecordMetrics(TimeSpan.FromSeconds(6), 100, 800 * 1024 * 1024);
        var decreased = await processor.GetOptimalBatchSize();
        decreased.Should().BeInRange(100, options.Performance.MaxBatchSize);
    }

    [Fact]
    public async Task Performance_monitor_records_and_reports()
    {
        using var loggerFactory = LoggerFactory.Create(builder => builder.SetMinimumLevel(LogLevel.Debug));
    ...
    [Fact]
    public async Task Service_registration_registers_core_components()
    {
    ...
    [Fact]
    public async Task Production_registration_applies_preset()
    {
        var services = new ServiceCollection();
        services.AddLogging();
    ...
    [Fact]
    public async Task Development_registration_uses_conservative_defaults()
    {
        var services = new ServiceCollection();
        services.AddLogging();
