...
namespace Koan.Data.Connector.SqlServer.Tests.Support;

public sealed class SqlServerAutoFixture : IRelationalTestFixture<Specs.Count.SqlServerCountTests.CountTestEntity, string>, IAsyncLifetime, IDisposable
{
    ...
    {
        if (ReferenceEquals(AppHost.Current, ServiceProvider))
        {
        ...
        if (_localDbName is not null)
        {
            try
            {
                const string master = "Server=(localdb)\\MSSQLLocalDB;Integrated Security=True;TrustServerCertificate=True;";
    ...
    private async Task<bool> TryUseLocalDbAsync()
    {
        try
        {
            ...
            const string master = "Server=(localdb)\\MSSQLLocalDB;Integrated Security=True;TrustServerCertificate=True;Connection Timeout=5;";
            await using var masterConn = new SqlConnection(master);
            await masterConn.OpenAsync();
