...
using Koan.Data.Vector.Connector.PgVector;
...
/// <summary>Live conformance suite for the PgVector network reference adapter.</summary>
public sealed class PgVectorVectorAodbConformanceSpec(PgVectorTestFactory fixture)
    : VectorAodbConformanceSpecsBase
{
    protected override async Task<(IntegrationHost? host, string? skip)> BootHostAsync()
    {
        ...
        var settings = new Dictionary<string, string?>(StringComparer.Ordinal)
        {
            ...
            ["Koan:Data:PgVector:DisableAutoDetection"] = "true",
            ["Koan:Data:Postgres:ConnectionString"] = fixture.ConnectionString,
            ["Koan:Data:Sources:Default:Adapter"] = "postgres",
            ["Koan:Data:Sources:VectorConformance:Adapter"] = "pgvector",
            [$"Koan:Data:Sources:{SourceA}:Adapter"] = "postgres",
            [$"Koan:Data:Sources:{SourceA}:ConnectionString"] = fixture.ConnectionString,
            [$"Koan:Data:Sources:{SourceB}:Adapter"] = "postgres",
            [$"Koan:Data:Sources:{SourceB}:ConnectionString"] = fixture.ConnectionString,
            ["Koan:Data:Sources:VectorReadOnly:Adapter"] = "pgvector",
            ["Koan:Data:Sources:VectorReadOnly:Access"] = "ReadOnly",
            ["Koan:Data:Sources:VectorExternal:Adapter"] = "pgvector",
        ...
        var host = await KoanIntegrationHost.Configure()
            ...
            .ConfigureServices(services => services.AddKoan(koan =>
            {
                ...
                shared.Vector<PgVectorEuclideanDoc>(space => Space(space, "euclidean", VectorMetric.Euclidean));
                shared.Vector<PgVectorDotProductDoc>(space => Space(space, "dot", VectorMetric.DotProduct));
                koan.Data.Source("Default").Vector<PgVectorCoexistDoc>(space => Space(space, "coexist"));
                koan.Data.Source("VectorReadOnly").Vector<PgVectorReadOnlyDoc>(space => Space(space, "readonly"));
                koan.Data.Source("VectorExternal").Vector<PgVectorExternalDoc>(space => Space(space, "external"));
    ...
    private IVectorSearchRepository<VectorConformancePartitionDoc, string> Repository() =>
        ...
        ?? throw new InvalidOperationException("PgVector repository did not resolve.");
    ...
    private async Task SpacePlan()
    {
        ...
        var factory = Host!.Services.GetRequiredService<PgVectorVectorAdapterFactory>();
        var plan = new VectorSpacePlan("Default", "wrong", 8, VectorMetric.Cosine, VectorVisibility.Session);
        var table = StorageNameGenerator.Generate(
            typeof(PgVectorWrongShapeDoc),
            partition: null,
            factory.GetNamingCapability(Host.Services));
        await fixture.CreateWrongShapeTable(table, dimensions: 4);
        var repository = factory.Create<PgVectorWrongShapeDoc, string>(Host.Services, plan);
    ...
    private async Task Upsert()
    {
        ...
        await new PgVectorCoexistDoc { Id = "coexist", Title = "record payload" }.Save();
        await Vector<PgVectorCoexistDoc>.Save("coexist", Point(1), new { Title = "vector payload" });
        Assert.Equal("record payload", (await PgVectorCoexistDoc.Get("coexist"))?.Title);
        Assert.Equal(
            "vector payload",
            Value(Assert.IsType<DataObject>((await Vector<PgVectorCoexistDoc>.Get("coexist"))?.Metadata), "Title"));
    }
    ...
    private async Task Similarity()
    {
        ...
        await Vector<PgVectorEuclideanDoc>.Save("same", Point(0));
        await Vector<PgVectorEuclideanDoc>.Save("five-away", Point(3, 4));
        var euclidean = await Vector<PgVectorEuclideanDoc>.Search(Point(0), query => query.Top(2));
        ...
        await Vector<PgVectorDotProductDoc>.Save("positive", Point(2));
        await Vector<PgVectorDotProductDoc>.Save("zero", Point(0, 1));
        await Vector<PgVectorDotProductDoc>.Save("negative", Point(-1));
        var dot = await Vector<PgVectorDotProductDoc>.Search(Point(1), query => query.Top(3));
    ...
    private async Task ExecutionTruth()
    {
        await Vector<VectorConformancePartitionDoc>.Save("one", Point(1));
        var factory = Host!.Services.GetRequiredService<PgVectorVectorAdapterFactory>();
        var table = StorageNameGenerator.Resolve(
            factory.Provider,
            ...
            () => factory.GetNamingCapability(Host.Services),
            Host.Services);
        await fixture.CreateCosineHnswIndex(table);
    ...
    {
        await fixture.DropVectorExtension();
        Assert.False(await fixture.HasVectorExtension());
        await Task.WhenAll(
            Vector<VectorConformancePartitionDoc>.EnsureCreated(),
            Vector<PgVectorEuclideanDoc>.EnsureCreated());
        Assert.True(await fixture.HasVectorExtension());
        ...
        {
            await Vector<PgVectorReadOnlyDoc>.Sync();
            await Assert.ThrowsAsync<DataSourcePolicyException>(() => Vector<PgVectorReadOnlyDoc>.Save("blocked", Point(1)));
            await Assert.ThrowsAsync<DataSourcePolicyException>(() => Vector<PgVectorReadOnlyDoc>.Clear());
            await Assert.ThrowsAsync<DataSourcePolicyException>(() => Vector<PgVectorReadOnlyDoc>.EnsureCreated());
        }
        using (EntityContext.Source("VectorExternal"))
        {
            Assert.Null(await Vector<PgVectorExternalDoc>.Get("missing"));
            await Assert.ThrowsAsync<DataSourcePolicyException>(() => Vector<PgVectorExternalDoc>.EnsureCreated());
        }
    }
...
[HostScoped]
public sealed class PgVectorReadOnlyDoc : Koan.Data.Core.Model.Entity<PgVectorReadOnlyDoc>;

[HostScoped]
public sealed class PgVectorExternalDoc : Koan.Data.Core.Model.Entity<PgVectorExternalDoc>;

public sealed class PgVectorWrongShapeDoc : Koan.Data.Core.Model.Entity<PgVectorWrongShapeDoc>;

[HostScoped]
public sealed class PgVectorCoexistDoc : Koan.Data.Core.Model.Entity<PgVectorCoexistDoc>
{
    public string Title { get; set; } = string.Empty;
}

[HostScoped]
public sealed class PgVectorEuclideanDoc : Koan.Data.Core.Model.Entity<PgVectorEuclideanDoc>;
[HostScoped]
public sealed class PgVectorDotProductDoc : Koan.Data.Core.Model.Entity<PgVectorDotProductDoc>;
