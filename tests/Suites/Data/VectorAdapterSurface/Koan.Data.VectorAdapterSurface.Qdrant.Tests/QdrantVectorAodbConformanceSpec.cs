    ...
    : VectorAodbConformanceSpecsBase
{
    protected override async Task<(IntegrationHost? host, string? skip)> BootHostAsync()
    {
        ...
        var settings = new Dictionary<string, string?>(StringComparer.Ordinal)
        {
            ...
            ["Koan:Data:Qdrant:Endpoint"] = fixture.Endpoint,
            ["Koan:Data:Qdrant:DisableAutoDetection"] = "true",
            ["Koan:Data:Sources:VectorConformance:Adapter"] = "qdrant",
            [$"Koan:Data:Sources:{SourceA}:Adapter"] = "qdrant",
            [$"Koan:Data:Sources:{SourceB}:Adapter"] = "qdrant",
            ["Koan:Data:Sources:VectorReadOnly:Adapter"] = "qdrant",
            ["Koan:Data:Sources:VectorReadOnly:Access"] = "ReadOnly",
            ["Koan:Data:Sources:VectorExternal:Adapter"] = "qdrant",
        ...
        var host = await KoanIntegrationHost.Configure()
            ...
            .ConfigureServices(services => services.AddKoan(koan =>
            {
                ...
                shared.Vector<QdrantEuclideanDoc>(space => Space(space, "euclidean", VectorMetric.Euclidean));
                shared.Vector<QdrantDotProductDoc>(space => Space(space, "dot", VectorMetric.DotProduct));
                koan.Data.Source("VectorReadOnly").Vector<QdrantReadOnlyDoc>(space => Space(space, "readonly"));
                koan.Data.Source("VectorExternal").Vector<QdrantExternalDoc>(space => Space(space, "external"));
    ...
    private IVectorSearchRepository<VectorConformancePartitionDoc, string> Repository() =>
        ...
        ?? throw new InvalidOperationException("Qdrant repository did not resolve.");
    ...
    private async Task SpacePlan()
    {
        ...
        await fixture.PutCollection("QdrantWrongShapeDoc", """
            {"vectors":{"wrong":{"size":4,"distance":"Cosine"}}}
            """);
        var factory = Host!.Services.GetRequiredService<QdrantVectorAdapterFactory>();
        var repository = factory.Create<QdrantWrongShapeDoc, string>(Host.Services,
    ...
    private async Task Similarity()
    {
        ...
        await Vector<QdrantEuclideanDoc>.Save("same", Point(0));
        await Vector<QdrantEuclideanDoc>.Save("five-away", Point(3, 4));
        var euclidean = await Vector<QdrantEuclideanDoc>.Search(Point(0), query => query.Top(2));
        ...
        await Vector<QdrantDotProductDoc>.Save("positive", Point(2));
        await Vector<QdrantDotProductDoc>.Save("zero", Point(0, 1));
        await Vector<QdrantDotProductDoc>.Save("negative", Point(-1));
        var dot = await Vector<QdrantDotProductDoc>.Search(Point(1), query => query.Top(3));
    ...
    private async Task LifecyclePolicy()
    {
        ...
        {
            await Vector<QdrantReadOnlyDoc>.Sync();
            await Assert.ThrowsAsync<DataSourcePolicyException>(() => Vector<QdrantReadOnlyDoc>.Save("blocked", Point(1)));
            await Assert.ThrowsAsync<DataSourcePolicyException>(() => Vector<QdrantReadOnlyDoc>.Clear());
            await Assert.ThrowsAsync<DataSourcePolicyException>(() => Vector<QdrantReadOnlyDoc>.EnsureCreated());
        }
        using (EntityContext.Source("VectorExternal"))
        {
            Assert.Null(await Vector<QdrantExternalDoc>.Get("missing"));
            await Assert.ThrowsAsync<DataSourcePolicyException>(() => Vector<QdrantExternalDoc>.EnsureCreated());
        }
    }
...
[HostScoped]
public sealed class QdrantReadOnlyDoc : Koan.Data.Core.Model.Entity<QdrantReadOnlyDoc>;

[HostScoped]
public sealed class QdrantExternalDoc : Koan.Data.Core.Model.Entity<QdrantExternalDoc>;

public sealed class QdrantWrongShapeDoc : Koan.Data.Core.Model.Entity<QdrantWrongShapeDoc>;
[HostScoped]
public sealed class QdrantEuclideanDoc : Koan.Data.Core.Model.Entity<QdrantEuclideanDoc>;
[HostScoped]
public sealed class QdrantDotProductDoc : Koan.Data.Core.Model.Entity<QdrantDotProductDoc>;
