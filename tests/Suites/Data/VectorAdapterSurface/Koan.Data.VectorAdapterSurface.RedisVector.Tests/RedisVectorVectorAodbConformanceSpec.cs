...
using Koan.Data.Vector.Connector.RedisVector;
...
/// <summary>Live conformance suite for Redis Search on the shared Redis backend.</summary>
public sealed class RedisVectorVectorAodbConformanceSpec(RedisVectorTestFactory fixture)
    : VectorAodbConformanceSpecsBase
{
    protected override async Task<(IntegrationHost? host, string? skip)> BootHostAsync()
    {
        ...
        var settings = new Dictionary<string, string?>(StringComparer.Ordinal)
        {
            ...
            ["Koan:Data:Sources:Default:Database"] = "1",
            ["Koan:Data:Sources:VectorConformance:Adapter"] = "redis-vector",
            ["Koan:Data:Sources:VectorConformance:ConnectionString"] = fixture.ConnectionString,
            [$"Koan:Data:Sources:{SourceA}:Adapter"] = "redis",
            [$"Koan:Data:Sources:{SourceA}:redis:ConnectionString"] = fixture.ConnectionString,
            [$"Koan:Data:Sources:{SourceB}:Adapter"] = "redis-vector",
            [$"Koan:Data:Sources:{SourceB}:ConnectionString"] = fixture.ConnectionString,
            ["Koan:Data:Sources:VectorReadOnly:Adapter"] = "redis-vector",
            ["Koan:Data:Sources:VectorReadOnly:ConnectionString"] = fixture.ConnectionString,
            ["Koan:Data:Sources:VectorReadOnly:Access"] = "ReadOnly",
            ["Koan:Data:Sources:VectorExternal:Adapter"] = "redis-vector",
            ["Koan:Data:Sources:VectorExternal:ConnectionString"] = fixture.ConnectionString,
        ...
        var host = await KoanIntegrationHost.Configure()
            ...
            .ConfigureServices(services => services.AddKoan(koan =>
            {
                ...
                shared.Vector<RedisVectorEuclideanDoc>(space => Space(space, "euclidean", VectorMetric.Euclidean));
                shared.Vector<RedisVectorDotProductDoc>(space => Space(space, "dot", VectorMetric.DotProduct));
                koan.Data.Source("Default").Vector<RedisVectorCoexistDoc>(space => Space(space, "coexist"));
                koan.Data.Source("VectorReadOnly").Vector<RedisVectorReadOnlyDoc>(space => Space(space, "readonly"));
                koan.Data.Source("VectorExternal").Vector<RedisVectorExternalDoc>(space => Space(space, "external"));
    ...
    private IVectorSearchRepository<VectorConformancePartitionDoc, string> Repository() =>
        ...
        ?? throw new InvalidOperationException("RedisVector repository did not resolve.");
    ...
    private async Task SpacePlan()
    {
        ...
        var factory = Host!.Services.GetRequiredService<RedisVectorVectorAdapterFactory>();
        var plan = new VectorSpacePlan("Default", "wrong", 8, VectorMetric.Cosine, VectorVisibility.Session);
        var physical = StorageNameGenerator.Resolve(
            factory.Provider,
            typeof(RedisVectorWrongShapeDoc),
            partition: null,
            source: "Default",
            () => factory.GetNamingCapability(Host.Services),
            Host.Services);
        var indexName = $"koan_vector_{physical}";
        await fixture.CreateWrongShapeIndex(indexName, dimensions: 4, "__koan_embedding");
        var repository = factory.Create<RedisVectorWrongShapeDoc, string>(Host.Services, plan);
    ...
    {
        await Vector<VectorConformancePartitionDoc>.Save("same", Point(2), new { Version = 1, Stale = "remove" });
        ...
        var metadata = Assert.IsType<DataObject>(point.Metadata);
        Assert.Equal(2L, Convert.ToInt64(Value(metadata, "Version")));
        Assert.DoesNotContain(metadata.Properties, property => property.Name == "Stale");
        Assert.Empty((await Vector<VectorConformancePartitionDoc>.Search(
            Point(0, 3), query => query.Top(10).Where(Filter.Eq("Stale", "remove")))).Items);
        Assert.Single((await Vector<VectorConformancePartitionDoc>.Search(Point(0, 1), query => query.Top(10))).Items);

        await new RedisVectorCoexistDoc { Id = "coexist", Title = "record payload" }.Save();
        await Vector<RedisVectorCoexistDoc>.Save("coexist", Point(1), new { Title = "vector payload" });
        Assert.Equal("record payload", (await RedisVectorCoexistDoc.Get("coexist"))?.Title);
        Assert.Equal(
            "vector payload",
            Value(Assert.IsType<DataObject>((await Vector<RedisVectorCoexistDoc>.Get("coexist"))?.Metadata), "Title"));

        var provider = Host!.Services.GetRequiredService<IRedisConnectionProvider>();
        var standard = Host.Services.GetRequiredService<IConnectionMultiplexer>();
        Assert.Same(provider.GetDefault(), standard);
    }
    ...
    private async Task Metadata()
    {
        ...
        Assert.Null(await Vector<VectorConformancePartitionDoc>.Get("reserved"));

        await Vector<VectorConformancePartitionDoc>.Save("wide-number", Point(1),
            new Dictionary<string, object?> { ["Wide"] = double.MaxValue });
        var wide = Assert.IsType<DataObject>((await Vector<VectorConformancePartitionDoc>.Get("wide-number"))!.Metadata);
        Assert.Equal(double.MaxValue, Assert.IsType<double>(Value(wide, "Wide")));
    }
    ...
    private async Task Similarity()
    {
        ...
        await Vector<RedisVectorEuclideanDoc>.Save("same", Point(0));
        await Vector<RedisVectorEuclideanDoc>.Save("five-away", Point(3, 4));
        var euclidean = await Vector<RedisVectorEuclideanDoc>.Search(Point(0), query => query.Top(2));
        ...
        await Vector<RedisVectorDotProductDoc>.Save("positive", Point(2));
        await Vector<RedisVectorDotProductDoc>.Save("zero", Point(0, 1));
        await Vector<RedisVectorDotProductDoc>.Save("negative", Point(-1));
        var dot = await Vector<RedisVectorDotProductDoc>.Search(Point(1), query => query.Top(3));
        ...
        Assert.All(result.Items.Concat(euclidean.Items).Concat(dot.Items), item =>
        {
    ...
    private async Task ExecutionTruth()
    {
        ...
        var physical = StorageNameGenerator.Resolve(
            factory.Provider,
            typeof(VectorConformancePartitionDoc),
            partition: null,
            source: "VectorConformance",
            () => factory.GetNamingCapability(Host.Services),
            Host.Services);
        var result = await Vector<VectorConformancePartitionDoc>.Search(Point(1), query => query.Top(1));
        await fixture.AssertFlatIndex($"koan_vector_{physical}", dimensions: 8, metric: "COSINE");
    ...
    private async Task SessionVisibility()
    {
        ...
        Assert.Equal("visible", Assert.Single((await Vector<VectorConformancePartitionDoc>
        ...
        Assert.Empty((await Vector<VectorConformancePartitionDoc>.Search(Point(1), query => query.Top(10))).Items);
    }
    ...
    private async Task NativeFilter()
    {
        ...
        var prefiltered = await Vector<VectorConformancePartitionDoc>.Search(
        ...
        await Assert.ThrowsAsync<VectorFilterUnsupportedException>(() =>
            Vector<VectorConformancePartitionDoc>.Search(
                Point(1), query => query.Top(10).Where(Leaf("Wide", FilterOperator.Gt, 0d))));
    ...
    private async Task Bulk()
    {
        ...
        Assert.Equal(Point(0, 1), (await repository.Get("a", VectorScope.Unscoped))!.Embedding.ToArray(),
            new FloatToleranceComparer(0.00001f));
        Assert.NotNull(await repository.Get("b", VectorScope.Unscoped));
        ...
        Assert.Null(await repository.Get("a", VectorScope.Unscoped));
        Assert.Null(await repository.Get("b", VectorScope.Unscoped));
        Assert.Null(await repository.Get("missing", VectorScope.Unscoped));
    }
    ...
    {
        Assert.Equal(RedisVectorTestFactory.ExpectedSearchModuleVersion, fixture.SearchModuleVersion);
        await Task.WhenAll(
            Vector<VectorConformancePartitionDoc>.EnsureCreated(),
            Vector<RedisVectorEuclideanDoc>.EnsureCreated());
        ...
        {
            await Vector<RedisVectorReadOnlyDoc>.Sync();
            await Assert.ThrowsAsync<DataSourcePolicyException>(() => Vector<RedisVectorReadOnlyDoc>.Save("blocked", Point(1)));
            await Assert.ThrowsAsync<DataSourcePolicyException>(() => Vector<RedisVectorReadOnlyDoc>.Clear());
            await Assert.ThrowsAsync<DataSourcePolicyException>(() => Vector<RedisVectorReadOnlyDoc>.EnsureCreated());
        }
        using (EntityContext.Source("VectorExternal"))
        {
            Assert.Null(await Vector<RedisVectorExternalDoc>.Get("missing"));
            await Assert.ThrowsAsync<DataSourcePolicyException>(() => Vector<RedisVectorExternalDoc>.EnsureCreated());
        }
    }
    ...
    private async Task Isolation()
    {
        ...
        using (EntityContext.Partition("alpha"))
        {
            ...
            Assert.Null(await Vector<VectorConformancePartitionDoc>.Get("same"));
        }
        ...
        using (EntityContext.Source(SourceA))
        {
            ...
            Assert.Null(await Vector<VectorConformanceShardedDoc>.Get("same"));
        }
        using (EntityContext.Source(SourceB))
        ...
        using (EntityContext.Source(SourceA))
            Assert.Null(await Vector<VectorConformanceShardedDoc>.Get("clear"));
        using (EntityContext.Source(SourceB))
    ...
    private async Task FailureLifecycle()
    {
        ...
        await fixture.Restart(Host!.Services.GetRequiredService<IConnectionMultiplexer>());
        Assert.NotNull(await Vector<VectorConformancePartitionDoc>.Get("durable"));
        Assert.Equal("durable", Assert.Single((await Vector<VectorConformancePartitionDoc>
            .Search(Point(1), query => query.Top(1))).Items).Id);
        ...
        await Assert.ThrowsAsync<ObjectDisposedException>(() =>
            repository.Save(Array.Empty<VectorPoint<string>>(), VectorScope.Unscoped));
    }
...
[HostScoped]
public sealed class RedisVectorReadOnlyDoc : Koan.Data.Core.Model.Entity<RedisVectorReadOnlyDoc>;

[HostScoped]
public sealed class RedisVectorExternalDoc : Koan.Data.Core.Model.Entity<RedisVectorExternalDoc>;

public sealed class RedisVectorWrongShapeDoc : Koan.Data.Core.Model.Entity<RedisVectorWrongShapeDoc>;

[HostScoped]
public sealed class RedisVectorCoexistDoc : Koan.Data.Core.Model.Entity<RedisVectorCoexistDoc>
{
    public string Title { get; set; } = string.Empty;
}

[HostScoped]
public sealed class RedisVectorEuclideanDoc : Koan.Data.Core.Model.Entity<RedisVectorEuclideanDoc>;
[HostScoped]
public sealed class RedisVectorDotProductDoc : Koan.Data.Core.Model.Entity<RedisVectorDotProductDoc>;
