    ...
    : VectorAodbConformanceSpecsBase
{
    protected override async Task<(IntegrationHost? host, string? skip)> BootHostAsync()
    {
        ...
        var settings = new Dictionary<string, string?>(StringComparer.Ordinal)
        {
            ...
            ["Koan:Data:Milvus:Endpoint"] = fixture.Endpoint,
            ["Koan:Data:Milvus:DisableAutoDetection"] = "true",
            ["Koan:Data:Sources:VectorConformance:Adapter"] = "milvus",
            [$"Koan:Data:Sources:{SourceA}:Adapter"] = "milvus",
            [$"Koan:Data:Sources:{SourceB}:Adapter"] = "milvus",
            ["Koan:Data:Sources:VectorReadOnly:Adapter"] = "milvus",
            ["Koan:Data:Sources:VectorReadOnly:Access"] = "ReadOnly",
            ["Koan:Data:Sources:VectorExternal:Adapter"] = "milvus",
        ...
        var host = await KoanIntegrationHost.Configure()
            ...
            .ConfigureServices(services => services.AddKoan(koan =>
            {
                ...
                shared.Vector<MilvusEuclideanDoc>(space => Space(space, "euclidean", VectorMetric.Euclidean));
                shared.Vector<MilvusDotProductDoc>(space => Space(space, "dot", VectorMetric.DotProduct));
                koan.Data.Source("VectorReadOnly").Vector<MilvusReadOnlyDoc>(space => Space(space, "readonly"));
                koan.Data.Source("VectorExternal").Vector<MilvusExternalDoc>(space => Space(space, "external"));
    ...
    private IVectorSearchRepository<VectorConformancePartitionDoc, string> Repository() =>
        ...
        ?? throw new InvalidOperationException("Milvus repository did not resolve.");
    ...
    private async Task SpacePlan()
    {
        ...
        var factory = Host!.Services.GetRequiredService<MilvusVectorAdapterFactory>();
        var logical = VectorAdapterNaming.GetOrCompute<MilvusWrongShapeDoc>(Host.Services, factory, "Default");
        await fixture.PutCollection(MilvusRepository<MilvusWrongShapeDoc, string>.PhysicalName(logical));
        var repository = factory.Create<MilvusWrongShapeDoc, string>(Host.Services,
        ...
        Assert.Contains("cannot realize space 'wrong'", shape.Message, StringComparison.Ordinal);
    }
    ...
    private async Task Similarity()
    {
        ...
        await Vector<MilvusEuclideanDoc>.Save("same", Point(0));
        await Vector<MilvusEuclideanDoc>.Save("five-away", Point(3, 4));
        var euclidean = await Vector<MilvusEuclideanDoc>.Search(Point(0), query => query.Top(2));
        Assert.Equal(1d, euclidean.Items[0].Similarity, 10);
        Assert.Equal(1d / 6d, euclidean.Items[1].Similarity, 5);

        await Vector<MilvusDotProductDoc>.Save("positive", Point(2));
        await Vector<MilvusDotProductDoc>.Save("zero", Point(0, 1));
        await Vector<MilvusDotProductDoc>.Save("negative", Point(-1));
        var dot = await Vector<MilvusDotProductDoc>.Search(Point(1), query => query.Top(3));
    ...
    {
        Assert.True(Vector<VectorConformancePartitionDoc>.GetCapabilities().Has(VectorCaps.BulkUpsert));
        Assert.True(Vector<VectorConformancePartitionDoc>.GetCapabilities().Has(VectorCaps.BulkDelete));
    ...
    private async Task LifecyclePolicy()
    {
        ...
        {
            await Vector<MilvusReadOnlyDoc>.Sync();
            await Assert.ThrowsAsync<DataSourcePolicyException>(() => Vector<MilvusReadOnlyDoc>.Save("blocked", Point(1)));
            await Assert.ThrowsAsync<DataSourcePolicyException>(() => Vector<MilvusReadOnlyDoc>.Clear());
            await Assert.ThrowsAsync<DataSourcePolicyException>(() => Vector<MilvusReadOnlyDoc>.EnsureCreated());
        }
        using (EntityContext.Source("VectorExternal"))
        {
            Assert.Null(await Vector<MilvusExternalDoc>.Get("missing"));
            await Assert.ThrowsAsync<DataSourcePolicyException>(() => Vector<MilvusExternalDoc>.EnsureCreated());
        }
    }
    ...
    private async Task WarmPath()
    {
        ...
        Assert.True(timer.Elapsed < TimeSpan.FromSeconds(30), $"Eight network warm cycles took {timer.Elapsed}.");
    }
...
[HostScoped]
public sealed class MilvusReadOnlyDoc : Koan.Data.Core.Model.Entity<MilvusReadOnlyDoc>;
[HostScoped]
public sealed class MilvusExternalDoc : Koan.Data.Core.Model.Entity<MilvusExternalDoc>;
public sealed class MilvusWrongShapeDoc : Koan.Data.Core.Model.Entity<MilvusWrongShapeDoc>;
[HostScoped]
public sealed class MilvusEuclideanDoc : Koan.Data.Core.Model.Entity<MilvusEuclideanDoc>;
[HostScoped]
public sealed class MilvusDotProductDoc : Koan.Data.Core.Model.Entity<MilvusDotProductDoc>;
