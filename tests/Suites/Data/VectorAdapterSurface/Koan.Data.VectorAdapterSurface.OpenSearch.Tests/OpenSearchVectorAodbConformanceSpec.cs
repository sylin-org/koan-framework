    ...
    : VectorAodbConformanceSpecsBase
{
    protected override async Task<(IntegrationHost? host, string? skip)> BootHostAsync()
    {
        ...
        var settings = new Dictionary<string, string?>(StringComparer.Ordinal)
        {
            ...
            ["Koan:Data:OpenSearch:Endpoint"] = fixture.Endpoint,
            ["Koan:Data:OpenSearch:DisableAutoDetection"] = "true",
            ["Koan:Data:Sources:VectorConformance:Adapter"] = "opensearch",
            [$"Koan:Data:Sources:{SourceA}:Adapter"] = "opensearch",
            [$"Koan:Data:Sources:{SourceB}:Adapter"] = "opensearch",
            ["Koan:Data:Sources:VectorReadOnly:Adapter"] = "opensearch",
            ["Koan:Data:Sources:VectorReadOnly:Access"] = "ReadOnly",
            ["Koan:Data:Sources:VectorExternal:Adapter"] = "opensearch",
        ...
        var host = await KoanIntegrationHost.Configure()
            ...
            .ConfigureServices(services => services.AddKoan(koan =>
            {
                ...
                shared.Vector<OpenSearchEuclideanDoc>(space => Space(space, "euclidean", VectorMetric.Euclidean));
                shared.Vector<OpenSearchDotProductDoc>(space => Space(space, "dot", VectorMetric.DotProduct));
                koan.Data.Source("VectorReadOnly").Vector<OpenSearchReadOnlyDoc>(space => Space(space, "readonly"));
                koan.Data.Source("VectorExternal").Vector<OpenSearchExternalDoc>(space => Space(space, "external"));
    ...
    private IVectorSearchRepository<VectorConformancePartitionDoc, string> Repository() =>
        ...
        ?? throw new InvalidOperationException("OpenSearch repository did not resolve.");
    ...
    private async Task SpacePlan()
    {
        ...
        var factory = Host!.Services.GetRequiredService<OpenSearchVectorAdapterFactory>();
        var small = factory.Create<OpenSearchWrongShapeDoc, string>(Host.Services,
        ...
        var wrong = factory.Create<OpenSearchWrongShapeDoc, string>(Host.Services,
        ...
        Assert.Contains("knn_vector mapping", shape.Message, StringComparison.OrdinalIgnoreCase);
    ...
    private async Task DeleteOutcome()
    {
    ...
    private async Task Similarity()
    {
        ...
        await Vector<OpenSearchEuclideanDoc>.Save("same", Point(0));
        await Vector<OpenSearchEuclideanDoc>.Save("five-away", Point(3, 4));
        var euclidean = await Vector<OpenSearchEuclideanDoc>.Search(Point(0), query => query.Top(2));
        ...
        await Vector<OpenSearchDotProductDoc>.Save("positive", Point(2));
        await Vector<OpenSearchDotProductDoc>.Save("zero", Point(0, 1));
        await Vector<OpenSearchDotProductDoc>.Save("negative", Point(-1));
        var dot = await Vector<OpenSearchDotProductDoc>.Search(Point(1), query => query.Top(3));
    ...
    private async Task NativeFilter()
    {
        ...
        foreach (var filter in cases)
        {
            ...
            Assert.True(expected.SetEquals(result.Items.Select(item => item.Id)),
                $"Filter {filter} did not converge with the neutral oracle.");
        }
    ...
    private async Task LifecyclePolicy()
    {
        ...
        {
            await Vector<OpenSearchReadOnlyDoc>.Sync();
            await Assert.ThrowsAsync<DataSourcePolicyException>(() =>
                Vector<OpenSearchReadOnlyDoc>.Save("blocked", Point(1)));
            await Assert.ThrowsAsync<DataSourcePolicyException>(() => Vector<OpenSearchReadOnlyDoc>.Clear());
            await Assert.ThrowsAsync<DataSourcePolicyException>(() => Vector<OpenSearchReadOnlyDoc>.EnsureCreated());
        }
        using (EntityContext.Source("VectorExternal"))
        {
            Assert.Null(await Vector<OpenSearchExternalDoc>.Get("missing"));
            await Assert.ThrowsAsync<DataSourcePolicyException>(() => Vector<OpenSearchExternalDoc>.EnsureCreated());
        }
    }
...
[HostScoped]
public sealed class OpenSearchReadOnlyDoc : Koan.Data.Core.Model.Entity<OpenSearchReadOnlyDoc>;
[HostScoped]
public sealed class OpenSearchExternalDoc : Koan.Data.Core.Model.Entity<OpenSearchExternalDoc>;
public sealed class OpenSearchWrongShapeDoc : Koan.Data.Core.Model.Entity<OpenSearchWrongShapeDoc>;
[HostScoped]
public sealed class OpenSearchEuclideanDoc : Koan.Data.Core.Model.Entity<OpenSearchEuclideanDoc>;
[HostScoped]
public sealed class OpenSearchDotProductDoc : Koan.Data.Core.Model.Entity<OpenSearchDotProductDoc>;
