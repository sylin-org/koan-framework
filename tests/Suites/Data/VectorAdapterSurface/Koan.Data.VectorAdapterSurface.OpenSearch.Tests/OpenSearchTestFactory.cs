...
public sealed class OpenSearchTestFactory : IAsyncLifetime
{
    ...
    public async ValueTask InitializeAsync()
    {
        try
        {
            ...
            _container = new ContainerBuilder("opensearchproject/opensearch:3.7.0")
        ...
        {
            UnavailableReason = $"OpenSearch/Docker unavailable: {error.GetType().Name}: {error.Message}";
        }
    }
    ...
    public async Task Restart(CancellationToken ct = default)
    {
        ...
        if (!string.Equals(Endpoint, restarted, StringComparison.Ordinal))
            throw new InvalidOperationException(
                $"OpenSearch restart changed its endpoint from '{Endpoint}' to '{restarted}'.");
        _http?.Dispose();
        _http = Client(Endpoint);
        ...
        throw new TimeoutException("OpenSearch did not become ready after restart.");
    }
