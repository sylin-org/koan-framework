...
/// <summary>
/// ARCH-0103 — the production-adapter AODB isolation proof on a LIVE OpenSearch, mirroring
...
/// <list type="bullet">
/// <item><b>Shared</b> — the <c>ScopedVectorRepository</c> stamps <c>__koan_tenant</c> into the OpenSearch
/// <c>metadata</c> sub-document (a nested JSON key OpenSearch accepts faithfully) and ANDs it into the kNN
/// pre-filter, so a tenant's search returns only its own vectors even when the OTHER tenant's point is
...
/// <item><b>Database</b> — a Database-mode <c>[Sharded]</c> axis folds the routed source into the index
/// name (the source-fold floor shipped in <c>e196c97c</c>), so each shard is a distinct physical index.</item>
/// </list>
/// Skips when Docker / OpenSearch is unreachable.
/// </summary>
public sealed class OpenSearchVectorIsolationSpec : IAsyncLifetime
{
    [VectorAdapter("opensearch")]
    ...
    public async ValueTask InitializeAsync()
    {
        try
        {
            ...
            // Testcontainers 4.11+). Env + wait strategy mirror OpenSearchTestFactory.
            _container = new ContainerBuilder("opensearchproject/opensearch:2.13.0")
        ...
        {
            _skip = $"OpenSearch/Docker unavailable: {ex.GetType().Name}: {ex.Message}";
            return;
        }
        ...
        var settings = new System.Collections.Generic.Dictionary<string, string?>(StringComparer.Ordinal)
        {
            ...
            ["Koan:Data:OpenSearch:RefreshMode"] = "true",            // read-your-writes: synchronous refresh
            ["Koan:Data:OpenSearch:DisableAutoDetection"] = "true",   // never let discovery find another local OpenSearch
        };
        ...
        _host = await KoanIntegrationHost.Configure()
            ...
            // stray local OpenSearch can never be discovered/used instead of THIS container.
            .ConfigureServices(s =>
            {
                s.AddKoan();
        ...
        _resolvedEndpoint = _host.Services
            .GetRequiredService<Microsoft.Extensions.Options.IOptions<OpenSearchOptions>>().Value.Endpoint;
    }
    ...
    [Fact(DisplayName = "OpenSearch Shared: the __koan_tenant metadata write-stamp round-trips + isolates a kNN (closes the overlay 'live breach' flag empirically)")]
    [Trait("Category", "Integration")]
    public async Task Shared_tenant_overlay_roundtrips_and_isolates()
    {
        Assert.SkipWhen(_skip is not null, _skip ?? "");
        _resolvedEndpoint.Should().Be(_endpoint, "the adapter must target THIS Testcontainers OpenSearch, not a stray local one");
        ...
        // stamp round-tripping faithfully through OpenSearch's metadata, the filter excludes g1 — only acme's a1 comes back.
    ...
    [Fact(DisplayName = "OpenSearch Container: a distinct ambient partition resolves to a distinct physical index (no cross-partition leak)")]
    [Trait("Category", "Integration")]
    public async Task Container_partition_isolates()
    {
        Assert.SkipWhen(_skip is not null, _skip ?? "");

        var pA = "ospc-" + Guid.CreateVersion7().ToString("n");
        var pB = "ospc-" + Guid.CreateVersion7().ToString("n");
    ...
    [Fact(DisplayName = "OpenSearch Database: a Database-mode axis folds the routed source into the index name → distinct physical index per shard")]
    [Trait("Category", "Integration")]
    public async Task Database_shard_isolates()
    {
        ...
        using (OpenSearchShardAmbient.Use("alpha")) { await Vector<ShardVec>.EnsureCreated(); await Vector<ShardVec>.Save("s1", PointA); }
        using (OpenSearchShardAmbient.Use("beta")) { await Vector<ShardVec>.EnsureCreated(); await Vector<ShardVec>.Save("s2", PointB); }

        using (OpenSearchShardAmbient.Use("alpha"))
            (await Vector<ShardVec>.Search(new VectorQueryOptions(Query: PointB, TopK: 10)))
                .Matches.Select(m => m.Id).Should().Equal("s1");
        using (OpenSearchShardAmbient.Use("beta"))
...
public sealed class OpenSearchShardedAttribute : Attribute;

internal static class OpenSearchShardMetadata
{
    private static readonly ConcurrentDictionary<Type, bool> Cache = new();
    public static bool IsSharded(Type t)
        => Cache.GetOrAdd(t, static x => x.GetCustomAttribute<OpenSearchShardedAttribute>(inherit: true) is not null);
}

public static class OpenSearchShardAmbient
{
...
public sealed class OpenSearchShardCarrier : IAmbientSliceCarrier
{
    public string AxisKey => "koan:opensearch-shard";
    public string? Capture() => OpenSearchShardAmbient.Current is { } s ? "v1:" + s : null;
    public IDisposable Restore(string captured)
        => captured.StartsWith("v1:", StringComparison.Ordinal)
            ? OpenSearchShardAmbient.Use(captured[3..])
            : throw new InvalidOperationException($"OpenSearchShardCarrier cannot restore '{captured}'.");
    public IDisposable Suppress() => OpenSearchShardAmbient.Use(null);
}

public sealed class OpenSearchShardAxis : IDataAxis
{
    public void Declare(Axis axis) => axis
        .Named("opensearch-shard")
        .Mode(AxisMode.Database)
        .AppliesTo(OpenSearchShardMetadata.IsSharded)
        .Field("shard", static () => OpenSearchShardAmbient.Current, typeof(string))
        .Carries(new OpenSearchShardCarrier());
}
