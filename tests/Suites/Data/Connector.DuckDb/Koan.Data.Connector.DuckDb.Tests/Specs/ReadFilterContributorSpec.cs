...
/// <summary>
/// DATA-0106 §2/§4 — the read-filter contributor seam proven predicate-generic on the real DuckDB adapter
...
/// </summary>
public sealed class ReadFilterContributorSpec(DuckDbFixture fixture, ITestOutputHelper output)
    : KoanDataSpec<DuckDbFixture>(fixture, output), IDisposable
{
    ...
    [Fact(DisplayName = "DuckDb: a non-equality read-filter contributor scopes Query/Count and adds no auto-equality for the moderator")]
    ...
    [Fact(DisplayName = "DuckDb: a non-equality read-filter contributor IDOR-protects get-by-id and delete-by-id")]
    ...
    [Fact(DisplayName = "DuckDb: a non-equality read-filter contributor scopes the mass-delete paths (DeleteMany/DeleteAll/RemoveAll)")]
    ...
    [Fact(DisplayName = "DuckDb: a read-filter contributor whose predicate the adapter cannot push fails closed (no in-memory residual leak)")]
    ...
    [Fact(DisplayName = "DuckDb: a PURE predicate read-filter axis (no managed field) fails closed on raw query")]
