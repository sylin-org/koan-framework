...
/// <summary>DuckDB readiness follows runtime participation and probes the same routed source as Entity operations.</summary>
public sealed class DuckDbHealthContributorSpec
{
    ...
    [Fact]
    public async Task Available_but_unelected_duckdb_is_non_critical_and_does_not_touch_disk()
    {
        var databasePath = TempDatabase("inactive");
        await using var host = await KoanIntegrationHost.Configure()
            .WithSetting("Koan:Environment", "Test")
            .WithSetting("Koan:Data:DuckDb:ConnectionString", Connection(databasePath))
        ...
        var contributor = DuckDbHealth(host.Services);
    ...
    [Fact]
    public async Task Runtime_repository_use_activates_duckdb_but_entity_description_does_not()
    {
        ...
        try
        {
                             ...
                             .StartAsync())
            {
                ...
                _ = AggregateConfigs.Get<ExplicitDuckDbRecord, string>(host.Services);
                var describedOnly = await contributor.Check();
                describedOnly.State.Should().Be(HealthState.Unknown);
                contributor.IsCritical.Should().BeFalse();
                File.Exists(databasePath).Should().BeFalse();

                var saved = await new ExplicitDuckDbRecord { Value = "selected-at-runtime" }.Save();
                (await ExplicitDuckDbRecord.Get(saved.Id))!.Value.Should().Be("selected-at-runtime");
    ...
    [Fact]
    public async Task Direct_execution_activates_duckdb_but_session_description_does_not()
    {
    ...
    [Fact]
    public async Task Elected_duckdb_health_is_non_creating_and_then_probes_the_provisioned_default_source()
    {
        ...
        try
        {
            await using (var host = await KoanIntegrationHost.Configure()
                             ...
                             .WithSetting("Koan:Data:DuckDb:ConnectionString", Connection(fallbackPath))
    ...
    [Fact]
    public async Task Configured_named_duckdb_source_stays_inactive_until_selected_then_probes_its_own_database()
    {
        ...
        try
        {
            await using (var host = await KoanIntegrationHost.Configure()
                             .WithSetting("Koan:Environment", "Test")
                             .WithSetting("Koan:Data:Sources:Archive:Adapter", "duckdb")
    ...
    [Fact]
    public async Task Selected_duckdb_reports_unhealthy_when_its_database_directory_is_a_file()
    {
    ...
    public async Task Readiness_probes_every_active_source_and_attributes_the_failure()
    {
        ...
        try
        {
            await using var host = await KoanIntegrationHost.Configure()
                ...
                .WithSetting("Koan:Data:Sources:ZArchive:Adapter", "duckdb")
            ...
            var report = await DuckDbHealth(host.Services).Check();
    ...
    private static IHealthContributor DuckDbHealth(IServiceProvider services)
        => services.GetServices<IHealthContributor>().Single(contributor => contributor.Name == "data:duckdb");
    ...
    private static string TempDatabase(string label)
        => Path.Combine(Path.GetTempPath(), $"koan-duckdb-health-{label}-{Guid.CreateVersion7():n}.db");
