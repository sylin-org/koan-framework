...
public sealed class DuckDbLegacyMappingSpec(DuckDbFixture fixture)
{
    [Fact]
    public async Task Missing_external_database_rejects_without_creating_a_file()
    {
        var path = Path.Combine(Path.GetTempPath(), $"koan-duckdb-external-{Guid.CreateVersion7():N}.db");
        var settings = new Dictionary<string, string?>(StringComparer.Ordinal)
        {
            ["Koan:Environment"] = "Test",
            ["Koan:Data:Sources:Missing:Adapter"] = "duckdb",
    ...
    public async Task Flat_and_structured_legacy_shape_uses_the_ordinary_entity_surface()
    {
        ...
        {
            ["Koan:Data:Sources:Legacy:Adapter"] = "duckdb",
        ...
        await using var connection = new DuckDBConnection(fixture.ConnectionString);
    ...
    public async Task Managed_maps_support_identity_object_composite_and_generated_shapes()
    {
        var settings = new Dictionary<string, string?>(fixture.SettingsForBoot(), StringComparer.Ordinal)
        {
            ["Koan:Data:Sources:Mapped:Adapter"] = "duckdb",
    ...
    public async Task Read_only_external_map_reads_normally_and_rejects_write_before_mutation()
    {
        ...
        var settings = new Dictionary<string, string?>(fixture.SettingsForBoot(), StringComparer.Ordinal)
        {
            ["Koan:Data:Sources:ReadOnlyLegacy:Adapter"] = "duckdb",
