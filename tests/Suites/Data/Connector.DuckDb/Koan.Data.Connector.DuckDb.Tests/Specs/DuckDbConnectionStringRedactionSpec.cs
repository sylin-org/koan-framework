...
/// <summary>
/// Security regression guard (F2-duckdb). A malformed DuckDB connection string that carries a secret
...
/// Reverting <c>DuckDBConnectionLifecycle</c>'s <c>Redaction.DeIdentify(cs)</c> back to <c>cs</c> makes this test
/// fail (the raw secret appears in the captured log). ARCH-0079: exercised through real <c>AddKoan()</c>.
/// </summary>
public sealed class DuckDBConnectionStringRedactionSpec
{
    [Fact(DisplayName = "DuckDb: a malformed connection string is redacted (no secret leak) in the parse-failed warning")]
    public async Task Malformed_connection_string_secret_is_redacted_in_logs()
    {
        const string secret = "Super Secret;DoNotLeak_9f3a";
        // 'Password' is a valid DuckDB keyword (the SQLCipher key); 'Mode=NotARealMode' is an invalid
        // value for the typed Mode keyword, which forces DuckDBConnectionStringBuilder to throw
        ...
        var configuration = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                // The DuckDB adapter reads its connection string from this provider-specific key
                // (Constants.Configuration.Keys.ConnectionString) and assigns it WITHOUT validation,
                // so a malformed value reaches DuckDBConnectionLifecycle's parse-failed path.
                ["Koan:Data:DuckDb:ConnectionString"] = malformedCs,
            })
            .Build();
