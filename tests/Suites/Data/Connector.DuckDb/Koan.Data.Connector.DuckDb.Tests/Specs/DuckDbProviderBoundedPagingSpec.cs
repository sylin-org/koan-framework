...
public sealed class DuckDbProviderBoundedPagingSpec(DuckDbFixture fixture, ITestOutputHelper output)
    : KoanDataSpec<DuckDbFixture>(fixture, output)
{
    [Fact(DisplayName = "DuckDb: raw predicates stay unbounded unless shaping requests a page")]
    ...
    [Fact(DisplayName = "DuckDb: provider paging is bounded, count-free by default, and gated by total sort handling")]
    public async Task Provider_paging_reports_only_the_work_it_performed()
    {
        ...
        nestedSort.PaginationHandled.Should().BeTrue("the compiled mapping lowers nested structured paths to DuckDB JSON");
