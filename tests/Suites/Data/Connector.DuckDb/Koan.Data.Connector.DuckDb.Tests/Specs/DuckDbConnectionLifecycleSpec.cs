...
/// <summary>
/// DuckDB connection ownership contracts. A Koan host owns the connections it opens: disposing that host must
...
/// </summary>
public sealed class DuckDBConnectionLifecycleSpec(DuckDbFixture fixture, ITestOutputHelper output)
    : KoanDataSpec<DuckDbFixture>(fixture, output)
{
    ...
    {
        RequireBackingStore();
        var databasePath = Path.Combine(Path.GetTempPath(), $"koan-duckdb-lifecycle-{Guid.CreateVersion7():n}.db");
    ...
    public async Task Private_memory_database_is_isolated_per_routed_source()
    {
        ...
        {
            ["Koan:Data:Sources:Archive:Adapter"] = "duckdb",
    ...
    public async Task Identical_private_memory_configuration_is_isolated_between_live_hosts()
    {
        ...
        await using var first = await BootAsync(settings);
        await using var second = await BootAsync(settings);
        var firstDirect = first.Services.GetRequiredService<IDataService>().Direct(adapter: "duckdb");
        var secondDirect = second.Services.GetRequiredService<IDataService>().Direct(adapter: "duckdb");
    ...
    public async Task Concurrent_sources_resolve_independent_memory_targets()
    {
        ...
        {
            settings[$"Koan:Data:Sources:Source{index}:Adapter"] = "duckdb";
