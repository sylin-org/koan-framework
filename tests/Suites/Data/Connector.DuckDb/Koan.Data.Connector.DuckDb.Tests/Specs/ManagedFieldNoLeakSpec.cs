...
/// <summary>
/// DATA-0105 §3b — the managed-field seam proven end-to-end on the real DuckDB adapter (no-Docker, real
...
/// </summary>
public sealed class ManagedFieldNoLeakSpec(DuckDbFixture fixture, ITestOutputHelper output)
    : KoanDataSpec<DuckDbFixture>(fixture, output), IDisposable
{
    ...
    [Fact(DisplayName = "DuckDb: a managed scope isolates reads, key-gets (IDOR), and RemoveAll; no scope sees all")]
    ...
    [Fact(DisplayName = "DuckDb: a cross-scope id-keyed upsert is rejected (no row takeover)")]
