...
/// gate falls through to the legacy toggle / Reference=Intent default. Verified through a real TestServer
/// host: the Swagger UI is served at <c>/swagger/index.html</c> iff the gate is on.
/// </summary>
public sealed class SwaggerEnableUiGateSpec : IClassFixture<SwaggerWebApplicationFactory>
{
    ...
    {
        using var client = _factory.CreateClient(("Koan:OpenApi:EnableUi", "false"));
        var resp = await client.GetAsync(SwaggerUi);
    ...
    public async Task Legacy_swagger_enabled_false_disables_when_EnableUi_unset()
    {
        ...
        using var client = _factory.CreateClient(("Koan:Web:Swagger:Enabled", "false"));
        var resp = await client.GetAsync(SwaggerUi);
        resp.StatusCode.Should().Be(HttpStatusCode.NotFound);
    }
}
