...
/// partition, raw-path fail-closed, the non-isolating-adapter fail-closed, and tenancy-Off zero regression.
...
public sealed class AssertNoTenantLeakSpec
{
    ...
    {
        await using var runtime = await TenancyRuntimeFixture.CreateAsync(extraSettings: Mode("Enforce"), adapter: "json");
        runtime.ResetEntityCaches();
        using var _iso = Isolate();
    ...
    [Fact(DisplayName = "no tenant leak: tenancy-Off is zero regression — writes are unstamped, reads are unfiltered")]
    public async Task Tenancy_off_is_zero_regression()
    {
        await using var runtime = await TenancyRuntimeFixture.CreateAsync(); // Mode defaults to Off
        runtime.ResetEntityCaches();
        using var _iso = Isolate();
