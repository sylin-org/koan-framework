...
namespace Koan.Testing.Infrastructure;
...
public static class DockerEnvironment
{
    ...
    public static async Task<ProbeResult> ProbeAsync()
    {
        ...
        if (!string.IsNullOrWhiteSpace(cliEndpoint))
        {
            ...
            var fallbackMessage = $"{message}; ping failed, continuing with CLI-derived endpoint";
            Log("Probe: CLI endpoint ping failed after detection, falling back to CLI availability.");
            return new(true, cliEndpoint, fallbackMessage);
        }
    ...
    private static string NormalizeEndpoint(string endpoint)
    {
            ...
            && endpoint.StartsWith("npipe:", StringComparison.OrdinalIgnoreCase))
        {
            ...
            while (endpoint.Contains("///", StringComparison.Ordinal))
            {
                endpoint = endpoint.Replace("///", "//", StringComparison.Ordinal);
            }
        }
