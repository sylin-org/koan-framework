...
public sealed class ElasticSearchContainerFixture : IAsyncDisposable, IInitializableFixture
{
    ...
    private const int ElasticHttpPort = 9200;
    private const string ImageName = "docker.elastic.co/elasticsearch/elasticsearch:8.17.0";

    private TestcontainersContainer? _container;
    private string? _cliContainerId;
    private string? _dockerEndpoint;

    public ElasticSearchContainerFixture(string dockerFixtureKey = DockerFixtureDefaultKey)
    {
    ...
    public async ValueTask InitializeAsync(TestContext context)
    {
        ...
        context.Diagnostics.Info("elasticsearch.fixture.initialize", new { dockerKey = DockerFixtureKey });
        ...
        if (TryGetExplicitEndpoint(out var explicitEndpoint) && await CanPing(explicitEndpoint!, context.Cancellation).ConfigureAwait(false))
        {
            ...
            IsAvailable = true;
            context.Diagnostics.Info("elasticsearch.fixture.explicit", new { source = "env" });
            return;
        }
        ...
        if (await CanPing(DefaultEndpoint, context.Cancellation).ConfigureAwait(false))
        {
            ...
            IsAvailable = true;
            context.Diagnostics.Info("elasticsearch.fixture.local", new { host = "localhost", port = ElasticHttpPort });
            return;
        }

        if (!context.TryGetItem(DockerFixtureKey, out DockerDaemonFixture? dockerFixture))
        {
            UnavailableReason = $"Docker fixture '{DockerFixtureKey}' is not registered. Call UsingDocker() before UsingElasticSearchContainer().";
            context.Diagnostics.Warn("elasticsearch.fixture.docker.missing", new { DockerFixtureKey, reason = UnavailableReason });
            return;
        }
        ...
        if (!dockerFixture!.IsAvailable)
        {
            ...
            context.Diagnostics.Warn("elasticsearch.fixture.docker.unavailable", new { DockerFixtureKey, reason = UnavailableReason });
            return;
        }
        ...
        var builder = new TestcontainersBuilder<TestcontainersContainer>()
            .WithImage(ImageName)
            .WithCleanUp(true)
            .WithPortBinding(ElasticHttpPort, assignRandomHostPort: true)
            .WithWaitStrategy(Wait.ForUnixContainer().UntilPortIsAvailable(ElasticHttpPort))
        ...
        try
        {
            ...
            context.Diagnostics.Info("elasticsearch.fixture.container.create", new { image = ImageName, endpoint });

            await container.StartAsync(context.Cancellation).ConfigureAwait(false);
            var mappedPort = container.GetMappedPublicPort(ElasticHttpPort);
            var connection = $"http://localhost:{mappedPort}";

            if (!await WaitForReady(connection, context.Cancellation).ConfigureAwait(false))
            {
                throw new InvalidOperationException("Unable to confirm Elasticsearch readiness.");
            }

            Endpoint = connection;
            IsAvailable = true;
            UnavailableReason = null;
            context.Diagnostics.Info("elasticsearch.fixture.container.started", new { host = "localhost", port = mappedPort });
            return;
        }
        catch (Exception ex) when (IsTestcontainersMissingMethod(ex, out var mmex))
        {
            var missingMessage = mmex?.Message ?? ex.Message;
            context.Diagnostics.Warn("elasticsearch.fixture.testcontainers.missingmethod", new { message = missingMessage });
            ...
            UnavailableReason = failureReason ?? "Failed to start Elasticsearch container via Docker CLI fallback.";
            return;
        }
        catch (Exception ex)
        {
            UnavailableReason = $"Failed to start Elasticsearch container: {ex.GetType().Name}: {ex.Message}";
            context.Diagnostics.Error("elasticsearch.fixture.container.failed", new { message = ex.Message }, ex);
            await DisposeContainerSilently(container).ConfigureAwait(false);
            return;
        }
    }
    ...
    private static bool TryGetExplicitEndpoint(out string? endpoint)
    {
        var candidates = new[]
        {
            Environment.GetEnvironmentVariable("Koan_TESTS_ELASTICSEARCH"),
            Environment.GetEnvironmentVariable("Koan_ELASTICSEARCH__ENDPOINT"),
            Environment.GetEnvironmentVariable("ELASTICSEARCH_ENDPOINT"),
            Environment.GetEnvironmentVariable("ElasticSearch__Endpoint")
        };
    ...
    {
        var containerName = $"koan-elasticsearch-{Guid.NewGuid():N}";
        var runArgs = $"run --rm -d --name {containerName} -p 127.0.0.1::{ElasticHttpPort} --env discovery.type=single-node --env xpack.security.enabled=false --env xpack.security.http.ssl.enabled=false --env ES_JAVA_OPTS=\"-Xms512m -Xmx512m\" {ImageName}";
        var (runOk, runStdout, runStderr, runExitCode) = await RunDockerCommand(runArgs, context.Cancellation).ConfigureAwait(false);

        if (!runOk)
        {
            context.Diagnostics.Warn("elasticsearch.fixture.dockercli.run.failed", new { exitCode = runExitCode, stdout = Truncate(runStdout), stderr = Truncate(runStderr) });
        ...
        {
            portResult = await RunDockerCommand($"port {containerName} {ElasticHttpPort}/tcp", context.Cancellation).ConfigureAwait(false);
        ...
        if (!portResult.ok || string.IsNullOrWhiteSpace(portResult.stdout))
        {
            context.Diagnostics.Warn("elasticsearch.fixture.dockercli.port.failed", new { exitCode = portResult.exitCode, stdout = Truncate(portResult.stdout), stderr = Truncate(portResult.stderr) });
            await StopCliContainer().ConfigureAwait(false);
            return (false, null, "Failed to determine published Elasticsearch port from docker CLI");
        }

        var hostPort = ParseDockerPortOutput(portResult.stdout);
        if (hostPort == 0)
        {
            context.Diagnostics.Warn("elasticsearch.fixture.dockercli.port.parse", new { stdout = Truncate(portResult.stdout) });
            await StopCliContainer().ConfigureAwait(false);
            return (false, null, "Unable to parse published Elasticsearch port from docker CLI output");
        }

        var connection = $"http://127.0.0.1:{hostPort}";
        if (!await WaitForReady(connection, context.Cancellation).ConfigureAwait(false))
        {
            context.Diagnostics.Warn("elasticsearch.fixture.dockercli.connect.timeout", new { port = hostPort });
            await StopCliContainer().ConfigureAwait(false);
            return (false, null, $"Elasticsearch container did not respond on localhost:{hostPort} within timeout");
        }

        Endpoint = connection;
        IsAvailable = true;
        UnavailableReason = null;
        context.Diagnostics.Info("elasticsearch.fixture.dockercli.started", new { container = containerName, host = "localhost", port = hostPort });
