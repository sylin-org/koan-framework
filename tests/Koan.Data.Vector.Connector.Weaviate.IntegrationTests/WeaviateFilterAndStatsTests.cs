...
public sealed class WeaviateFilterAndStatsTests : IClassFixture<WeaviateAutoFixture>
{
    ...
    public async Task Index_Clear_And_Stats()
    {
        ...
        // Stats should be >= 1
    var exec = (IInstructionExecutor<TestEntity>)repo;
    var count = await exec.ExecuteAsync<int>(new Instruction(VectorInstructions.IndexStats));
        Assert.True(count >= 1);

        // Clear (destructive) with explicit opt-in
    await exec.ExecuteAsync<object>(new Instruction(VectorInstructions.IndexClear, Options: new Dictionary<string, object?> { ["AllowDestructive"] = true }));
