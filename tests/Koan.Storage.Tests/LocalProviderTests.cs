...
public class LocalProviderTests
{
    ...
    public async Task DefaultProfile_Is_Used_When_Profile_Not_Specified()
    {
        ...
        var dict = new Dictionary<string, string?>
        {
            ["Koan:Storage:Profiles:main:Provider"] = "local",
            ["Koan:Storage:Profiles:main:Container"] = "bucket",
    ...
    public async Task SingleProfileOnly_Fallback_Works_With_One_Profile()
    {
        ...
        var dict = new Dictionary<string, string?>
        {
            ["Koan:Storage:Profiles:solo:Provider"] = "local",
            ["Koan:Storage:Profiles:solo:Container"] = "only",
            ["Koan:Storage:Providers:Local:BasePath"] = temp.Path
        };
        var config = new ConfigurationBuilder().AddInMemoryCollection(dict!).Build();
        var services = new ServiceCollection();
