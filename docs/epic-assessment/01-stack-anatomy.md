...
| K3 | Crypto constants | The HKDF **domain-separation byte strings** originally carried ZG vocabulary (`b"pond-unlock-slot-totp-v1"`, `b"pond-fido2-storage-key-v1"`); renamed once to the neutral `b"koi-…-v1"` namespace in the pre-1.0 greenfield window (no production vault existed) and frozen there | `koi/crates/koi-crypto/src/unlock_slots.rs`, `key_agreement.rs` | ✅ neutral + frozen at `koi-*` (a new algorithm gets a new label, never a rename) |
