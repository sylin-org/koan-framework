...
- **Inputs**: Koan application bootstrapped with `builder.Services.AddKoan()`, Canon packages installed, and at least one storage adapter for entities and vectors when required.
- **Outputs**: Repeatable canonization pipelines, semantic processing flows, controllers, and adapters that keep intake, enrichment, and projection concerns separate.
- **Error Modes**: Misaligned stage configuration, unbounded pipelines that ignore cancellation, provider limitations (no vector/search support), or missing adapter registration preventing canonization from running.
- **Success Criteria**: Data reliably progresses from intake to projections, semantic pipelines scale with streaming, Canon controllers expose predictable APIs, and telemetry captures throughput plus failures.

### Edge Cases

- **Multi-source ingestion** – make the default adapter explicit when multiple Canon intake providers exist; annotate flow entities with `[DataAdapter]` when routing to specialized stores.
...
Canon (formerly the Flow pillar) orchestrates inbound data, enrichment, and publication through orderly stages. Entities derived from `FlowEntity<T>` move through intake ➜ processing ➜ projections while semantic pipelines (`Pipeline()`) offer declarative transformations. Canon integrates directly with Data, AI, and Messaging pillars to stream entities, enrich with AI, emit events, and surface rich APIs without custom repositories. Existing type names (`FlowEntity`, `FlowSets`, etc.) remain for now; the namespace, packages, and documentation now reflect Canon.
...
Install the Canon runtime and supporting adapters:

```powershell
pwsh -c "dotnet add package Koan.Canon"
```
...
## Canon Entities & Stages

### Modeling Canon Entities
...
```
        ...
        Canon’s runtime still reads configuration from the existing `Koan:Canon` section; rename to `Koan:Canon` is tracked in [ARCH-0056](../../decisions/ARCH-0056-koan-canon-pillar-renaming.md). Override adapter lists (`Include`/`Exclude`) when multiple sources are active and prefer secrets from `IConfiguration` providers instead of embedding credentials.
...
`FlowEntityController<T>` exposes Canon-specific CRUD and projection endpoints without custom routing.
...
- `GET /api/flow/device` – list canonical entities (route prefix retains `flow` for backward compatibility).
- `GET /api/flow/device/{id}` – fetch the canonical projection.
...
Canon events published via `FlowEvent<T>` connect pipelines with external systems, enabling replay, alerting, and cross-pillar workflows.
...
```csharp
public static class DeviceCanonInterceptors
{
...
```csharp
public sealed class CanonHealthCheck : IHealthContributor
{
    public string Name => "canon-pipeline";
...
```csharp
public sealed class CanonDeviceRetryPolicy : IFlowRetryPolicy<Device>
{
...
- [AI & Vector How-To](../../guides/ai-vector-howto.md)
