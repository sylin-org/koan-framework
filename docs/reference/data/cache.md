...
When code mutates state outside `Entity<T>.Save/Remove` (raw SQL via `IDataService.Direct(...)`, batch jobs, external workers), the cache decorator never sees the change. Use the canonical evict surface:
