...
Control responders (API-side, opt-out gates)
- Announce responder
  - Key: `Sora:Flow:Control:AutoResponse:Announce:Enabled` (bool). Default: `true`.
  - Behavior: Replies to `ControlCommand { Verb = "announce" }` with `ControlResponse<AdapterAnnouncement>` from the in-memory registry. If `Target` is set, it validates the target (`system:adapter[:instance|*]`) and returns `NotFound` when a specific instance isn’t present.
- Ping responder
  - Key: `Sora:Flow:Control:AutoResponse:Ping:Enabled` (bool). Default: `true`.
  - Behavior: Replies to `ControlCommand { Verb = "ping" }` with `FlowAck.Ok`. When `Target` is specific (e.g., `system:adapter:instance`), it validates presence in the registry and returns `NotFound` if missing.

Example (disable one or both responders)
{
  "Sora": {
    "Flow": {
      "Control": {
