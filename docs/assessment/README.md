...
| 7 | [07-strategic-prompt-stash.md](07-strategic-prompt-stash.md) | Self-contained, maturity-ordered build cards for the second-act capabilities (05 §3.1): target shapes + reference usage embedded, full research→implement→verify sessions runnable by lesser models, phased to compose (self-description → verification → trust → domain power → reach). |
| 8 | [08-agyo-reorganization.md](08-agyo-reorganization.md) | The three-way disposition (delete / split / migrate-to-agyo) that reclassified the Track-C cut cards. |
| 9 | [09-agent-native-projection.md](09-agent-native-projection.md) | What "agent-native" means (one projection, two faces), grounded + adversarially verified against the codebase: the opportunity catalog, the design-charter harvest, and a confirmed read-path security finding (§10). |
