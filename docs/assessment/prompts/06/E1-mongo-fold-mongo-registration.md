# E1-mongo · Fold Mongo manual registration into the registrar
...
_Instantiated from E1-TEMPLATE · adapter Mongo._

```text
TASK: In src/Connectors/Data/Mongo, fold the manual MongoRegistration static class into
...
2. Find consumers of the manual AddMongoAdapter() extension (grep across src/samples/tests —
   ...
   retained thin AddMongoAdapter() that now just delegates to the registrar's Register
   method — choose the FIRST option unless tests fail.
3. Delete the manual class.
VERIFY: build green; that adapter's test suite green (tests/Suites/Data/Mongo).
ADAPTER (this session): Mongo.
```
