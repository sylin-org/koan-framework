# E1-json · Fold Json manual registration into the registrar
...
_Instantiated from E1-TEMPLATE · adapter Json._

```text
TASK: In src/Connectors/Data/Json, fold the manual JsonRegistration static class into
...
2. Find consumers of the manual AddJsonAdapter() extension (grep across src/samples/tests —
   ...
   retained thin AddJsonAdapter() that now just delegates to the registrar's Register
   method — choose the FIRST option unless tests fail.
3. Delete the manual class.
VERIFY: build green; that adapter's test suite green (tests/Suites/Data/Json).
ADAPTER (this session): Json.
