# E1-sqlite · Fold Sqlite manual registration into the registrar
...
_Instantiated from E1-TEMPLATE · adapter Sqlite._

```text
TASK: In src/Connectors/Data/Sqlite, fold the manual SqliteRegistration static class into
...
2. Find consumers of the manual AddSqliteAdapter() extension (grep across src/samples/tests —
   ...
   retained thin AddSqliteAdapter() that now just delegates to the registrar's Register
   method — choose the FIRST option unless tests fail.
3. Delete the manual class.
VERIFY: build green; that adapter's test suite green (tests/Suites/Data/Sqlite).
ADAPTER (this session): Sqlite.
```
