# C13 · Cut PGVector connector + vector filter surface
...
_Instantiated from CUT-TEMPLATE · row C13._

```text
TASK: cut the project(s) src/Connectors/Data/Vector/PGVector + its orphan test project.
JUSTIFICATION (verify, then cite in your summary): Does not compile (csproj's own comment); out of sln; fix parked on branch trusting-mccarthy. Tag attic/pgvector first.
PRECHECKS (all must hold, else STOP):
1. grep each project name across **/*.csproj — inbound ProjectReferences must match none in sln.
2. grep the key public type names across src/ samples/ tests/ — no live consumers beyond none in sln.
...
3. then delete IVectorFilterTranslator<TNative> + VectorEmbeddingAttribute from Koan.Data.Vector.Abstractions (their only implementor/consumer was PGVector — verify with grep).
