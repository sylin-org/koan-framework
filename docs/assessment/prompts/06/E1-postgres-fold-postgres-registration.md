# E1-postgres · Fold Postgres manual registration into the registrar
...
_Instantiated from E1-TEMPLATE · adapter Postgres._

```text
TASK: In src/Connectors/Data/Postgres, fold the manual PostgresRegistration static class into
...
2. Find consumers of the manual AddPostgresAdapter() extension (grep across src/samples/tests —
   ...
   retained thin AddPostgresAdapter() that now just delegates to the registrar's Register
   method — choose the FIRST option unless tests fail.
3. Delete the manual class.
VERIFY: build green; that adapter's test suite green (tests/Suites/Data/Postgres).
ADAPTER (this session): Postgres.
```
