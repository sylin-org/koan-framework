# E1-couchbase · Fold Couchbase manual registration into the registrar
...
_Instantiated from E1-TEMPLATE · adapter Couchbase._

```text
TASK: In src/Connectors/Data/Couchbase, fold the manual CouchbaseRegistration static class into
...
2. Find consumers of the manual AddCouchbaseAdapter() extension (grep across src/samples/tests —
   ...
   retained thin AddCouchbaseAdapter() that now just delegates to the registrar's Register
   method — choose the FIRST option unless tests fail.
3. Delete the manual class.
VERIFY: build green; that adapter's test suite green (tests/Suites/Data/Couchbase).
ADAPTER (this session): Couchbase.
```
