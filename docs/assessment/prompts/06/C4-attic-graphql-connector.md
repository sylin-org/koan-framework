# C4 · Attic-tag Koan.Web.Connector.GraphQl
...
_Instantiated from CUT-TEMPLATE · row C4._

```text
TASK: attic-tag the project(s) Koan.Web.Connector.GraphQl.
JUSTIFICATION (verify, then cite in your summary): Sole consumer archived; HotChocolate CVE treadmill for nobody (WEB-0041/0042 note)..
PRECHECKS (all must hold, else STOP):
1. grep each project name across **/*.csproj — inbound ProjectReferences must match archived sample S4 only.
2. grep the key public type names across src/ samples/ tests/ — no live consumers beyond archived sample S4 only.
