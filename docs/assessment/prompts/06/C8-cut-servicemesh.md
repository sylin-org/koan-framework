# C8 · Cut Koan.ServiceMesh + Translation service
...
_Instantiated from CUT-TEMPLATE · row C8._

```text
TASK: cut the project(s) Koan.ServiceMesh + Koan.ServiceMesh.Abstractions + src/Services/Translation (+ .Container).
JUSTIFICATION (verify, then cite in your summary): No ADR, no tests, experimental..
PRECHECKS (all must hold, else STOP):
1. grep each project name across **/*.csproj — inbound ProjectReferences must match each other + Koan.Web.Admin (defensive) + S8.PolyglotShop (broken).
2. grep the key public type names across src/ samples/ tests/ — no live consumers beyond each other + Koan.Web.Admin (defensive) + S8.PolyglotShop (broken).
...
3. in Koan.Web.Admin remove the defensive ServiceMesh surface (KoanAdminServiceMeshSurfaceFactory + its GetService call) — read it first; it null-checks, so removal is a small excision. S8.PolyglotShop is already broken/out-of-sln: move to attic in the same pass..
