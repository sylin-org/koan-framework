# C19 · Execute MEDIA-0008's overdue deletion
...
> **Reorg (2026-06-14)**: confirmed DELETE — not an agyo candidate (superseded by storage-backed derivations (MEDIA-0007, IMediaSource.TryStoreDerivationAsync) that participate in backup/quota/GC; the [Obsolete] output-cache deletion is already scheduled (MEDIA-0008)) — see docs/assessment/08-agyo-reorganization.md + agyo-tools docs/decisions/AGYO-0001.
...
> **Reorg disposition: confirmed DELETE (08-agyo-reorganization).** Not an agyo candidate — it is a strictly-inferior predecessor of a live core capability (storage-backed derivations (MEDIA-0007, IMediaSource.TryStoreDerivationAsync) that participate in backup/quota/GC; the [Obsolete] output-cache deletion is already scheduled (MEDIA-0008)), with no distinct app-building value. Nothing to preserve. Proceed with the cut below.
