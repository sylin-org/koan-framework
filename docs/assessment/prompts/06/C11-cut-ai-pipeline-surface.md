...
> **Reorg (2026-06-14)**: confirmed DELETE — not an agyo candidate (superseded by the live EntityAi API (Embed/Chat/Ocr) — the pipeline surface is inert (internal ctors, no entry point, ImagePipeline.ToImage throws unconditionally)). — see docs/assessment/08-agyo-reorganization.md + agyo-tools docs/decisions/AGYO-0001.
...
> **Reorg disposition: confirmed DELETE (08-agyo-reorganization).** Not an agyo candidate — it is a strictly-inferior predecessor of a live core capability (the live EntityAi API (Embed/Chat/Ocr) — the pipeline surface is inert (internal ctors, no entry point, ImagePipeline.ToImage throws unconditionally)), with no distinct app-building value. Nothing to preserve. Proceed with the cut below.
