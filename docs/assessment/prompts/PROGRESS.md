...
| D2 | T2 | — | done | 2026-06-15 | opus-4.8 | 61c7eaa9 (+ deletions in 6174b79b — see Divergence) | **Partial by mandate** ("delete only the *verified-dead* surface"). DONE: item 1 (dead markers IServiceAdapter/IKoanService/IDevServiceDescriptor + Koan0049A diagnostic) + item 5 (tombstone IVT). MOOT: item 4 (PGVector/KoanContext already gone). **NOT dead → excluded** (under architect review for ARCH-0077 routing): item 2 (legacy attrs still read by CLI ProjectDependencyAnalyzer + ComposeExporter), item 3 (SelfOrchestration is Development-default-live), item 6 (Cli.Core is a published periphery package, not all-internal). Build green; Bootstrap 23/23. |
