...
> renames). So the doc **misnamed** renamed fields (probe: `[JsonProperty("renamedField")]` showed as
> `original` in OpenAPI, but the wire is `renamedField`) and emitted **opaque integer enums** (the wire is
> already string — a contract violation, not just drift). That is the honest "one schema projector" gap.
> **→ FIXED via follow-up card `X-openapi-newtonsoft-fidelity` (commit `6018757b`, 2026-06-19).** Approach
> changed from the card's per-schema `IOpenApiSchemaTransformer` to fixing the SOURCE globally:
> `NewtonsoftSchemaMirror` mirrors the Newtonsoft contract onto the System.Text.Json options the generator
> reads (empirically `Http.Json.JsonOptions`, not `Mvc`) — string enums + `[JsonProperty]` renames +
> `[JsonIgnore]` omission, faithful by construction. Guard `SchemaProjectionConformanceSpec` flipped
> drift→fidelity (3 mutation-checked teeth + keeps-[McpIgnore]).
