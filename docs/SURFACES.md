...
| MCP server | MCP suites + AS-edge specs | 2026-06-20 | MCP suites + Koan.Web.Auth.Server.IntegrationTests (local) | Real and consumed (6.4k LOC); SEC-0006: the HTTP/SSE edge is now an OAuth resource server (Koan.bearer ES256 validation, RFC 9728 WWW-Authenticate, RFC 8707 audience via McpEdgeAuth/McpResourceIdentity), guarded by McpAuthRampSpec/McpConfiguredResourceSpec |
