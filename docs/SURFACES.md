...
| AODB adapter isolation conformance (RowScoped/ContainerScoped/DatabaseScoped, ARCH-0103 §6 P5) | `AodbConformanceSpecsBase<TFixture>` × 8 adapter cells | 2026-06-26 | conformance kit (local; 8 surfaces × 4 facts: Declares/Shared/Container/Database) | The AODB three-mode ledger: every conformant adapter declares all three isolation tokens (`DataCaps.Isolation.*`), each **co-defined with its proof** (ARCH-0094 — declare-but-not-realize ⇒ red; realize-but-not-declare ⇒ red). Cells: Mongo + Couchbase (document), InMemory/Json/Redis (KV), SQLite/Postgres/SQL Server (relational). Postgres/SQL Server Database cells provision a fresh physical DB per routed source; the **Couchbase** Database cell provisions a per-source **bucket** (Container = native scope per partition, Database = native bucket per source). The shared discoverable shard axis lives in the TestKit (adapter-test-only — never data-core/tenancy, so off-proofs unaffected). Each adapter's boot report notes its declared modes. **Couchbase FOLDED onto `DocumentStore` (2026-06-26, ARCH-0103 P4)** — now declares all three tokens + passes all four cells (the document family is complete) |
| Vector AODB isolation (Shared/Container/Database, ARCH-0103) | `QdrantVectorIsolationSpec` (live Qdrant) + `VectorTenantIsolationSpec` (InMemory) + `VectorDatabaseRoutingSpec` (InMemory/SqliteVec) | 2026-06-26 | local; Qdrant Testcontainers (3/3) + Docker-free in-proc | Container realized fleet-wide via the name-mangling floor (partition→collection); Database via the source-fold (routed source→collection, `e196c97c`, fail-closed injective guard); Shared via `ScopedVectorRepository` (`__koan_tenant` stamp+filter). **Overlay-naming security flag TEST-VERIFIED on live Qdrant** (`__koan_tenant` nested-payload round-trip isolates a kNN) — only Weaviate's GraphQL reserves `__` (declares `IOverlayNamingAware`); Qdrant/Milvus/ES/OpenSearch use faithful nested JSON keys. The Qdrant spec surfaced + fixed a real empty-`CollectionName` bug (full-AddKoan configurator binds absent key to `""`; Milvus had the twin, fixed). **Follow-on**: Weaviate/ES/OpenSearch/Milvus production specs + vector `DataCaps.Isolation.*` token declarations |
