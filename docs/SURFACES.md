...
| AODB adapter isolation conformance (RowScoped/ContainerScoped/DatabaseScoped, ARCH-0103 §6 P5) | `AodbConformanceSpecsBase<TFixture>` × 7 adapter cells | 2026-06-26 | conformance kit (local; 7 surfaces × 4 facts: Declares/Shared/Container/Database) | The AODB three-mode ledger: every conformant adapter declares all three isolation tokens (`DataCaps.Isolation.*`), each **co-defined with its proof** (ARCH-0094 — declare-but-not-realize ⇒ red; realize-but-not-declare ⇒ red). Cells: Mongo (document), InMemory/Json/Redis (KV), SQLite/Postgres/SQL Server (relational). Postgres/SQL Server Database cells provision a fresh physical DB per routed source. The shared discoverable shard axis lives in the TestKit (adapter-test-only — never data-core/tenancy, so off-proofs unaffected). Each adapter's boot report notes its declared modes. **Couchbase excluded** (not yet folded onto `DocumentStore`; declares **no** isolation tokens — so it fails closed on managed-scoped routing and would fail `Declares_all_three`) |
