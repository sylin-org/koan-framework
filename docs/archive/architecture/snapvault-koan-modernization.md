...
| **Real-time progress** | `PhotoProcessingHub` (SignalR) + hub group mgmt + scattered `EmitProgress`/`SendAsync` calls + the SignalR client ref | `Koan.Web.Sse` — a controller endpoint that returns `Sse.Stream(...)` over progress read **from the ledger**. Progress is reported once, durably, via `ctx.Progress`; the stream projects it. | the `PhotoProgressEvent`/`JobCompletionEvent` payload shapes (now SSE payloads) |
