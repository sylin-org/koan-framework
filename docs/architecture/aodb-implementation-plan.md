...
- Date: 2026-06-25
- Status: **PHASES 1 + 2 SHIPPED** on `dev` (all green, unpushed); Database-mode auto-routing is live. **Remaining: Tier-2 visibility surfaces** (`.Explain` over the full Aodb, boot-report per-entity) + the two Phase-3 follow-ons pinned below.
...
>
> **▶▶ Phase 2 SHIPPED** (`AxisMode.Database` auto-routing). The carrier-only contract was relaxed: a Database-mode axis now declares `.Field` (the per-operation **source-key provider**) + `.Carries` (durable across the async hop), forbids `.Reads`/`.OnDelete`. The expander registers a `DatabaseRouteDescriptor` into the new **`DatabaseRouteRegistry`** (copy-on-write, lock-free reads); `AdapterResolver.ResolveForEntity` consults it at **Priority 1.5** — *after* an explicit `EntityContext.Source` (the caller's override always wins), gated by `IsEmpty` so **off = byte-identical** (FC-5). Unconfigured-source routing **fails closed, self-explaining** (FC-7) — the realized `ProvisioningPosture.ExternalOnly`. Proven by a **Docker-free multi-DB SQLite gate** (`MultiDatabaseRoutingSpec`): observed RED (no hook ⇒ both shards in Default, *no isolation, no error* — the footgun) → GREEN (hook ⇒ physical isolation across two DB files) + the fail-closed case. Adversarial routing-core review: **0 CRITICAL**; the under-lock-delegate + per-call-lock findings were fixed by the copy-on-write registry.
> *Green throughout: axes unit 58 (+2 Database contract) · axes integration 14 (+2) · data-core off-proof 273 (FC-5 byte-identity) · tenancy flagship 104 (no perturbation).*
> **Phase 3 follow-ons (pinned, not blocking):** (1) a declarative **`NullKeyBehavior.FailClosed`** opt-in so a strict-isolation Database axis throws on an unset ambient rather than falling through to Default (today the provider must throw itself; documented on `DatabaseRouteDescriptor.SourceKeyProvider`); (2) **boot-time detection of overlapping Database-axis predicates** (today "first applicable wins" is documented behavior — a full predicate-overlap check needs entity-type enumeration, deferable to the schema stage). Per-tenant *placement* (A isolated, C shared) remains the P6 broker.
