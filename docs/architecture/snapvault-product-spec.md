...
- **Media-derivation eviction (from the step-3b review) — DONE IN 5c.** The delete path now evicts the recipe cache for the deleted source via the structural `PhotoAsset.AfterRemove` hook (`Initialization/PhotoAssetCleanup.cs`, registered once in `SnapVaultModule.Start`): `MediaDerivation.Query(d => d.SourceMediaId == photoId)` → `Delete` (blob) + `Remove` (record), and the §3 collection dead-id pruning rides the same hook (`AfterRemove`, not `BeforeRemove` — cleanup should only run after the delete actually succeeds). It fires on **any** delete path (bulk delete today, deprovisioning later), each leg guarded per-item so one failure can't strand the rest. This is **cleanup, not correctness**: an un-evicted render is unreachable because `MediaEntitySource.OpenAsync` (the access gate) 404s the deleted source *before* the controller's derivation-cache lookup — so an orphan never serves, it only wastes storage. Targeted eviction (by `SourceMediaId`) is the primary path precisely because `MediaEntitySource<T>` leaves the framework `SweepOrphanedDerivationsAsync` a no-op (a context-free cross-tenant probe is unsafe) — and targeting also can't false-positive a still-live, access-gated source into deletion. Proven by `SnapVaultMutationSpec.Delete_evicts_renders_and_prunes_collections`.
...
- **Unify event-click browse with the #5 windowing (from the 5b review) — DO IN 5f.** The SPA calls `by-event` (#6) with no pagination and renders `response.photos`, so an event with >50 photos silently shows only the first 50 (the server paginates correctly; the SPA ignores it). Either add an `"event"` context to `PhotoSetDefinition`/`PhotoSetService` (query by `EventId`, windowed like the grid) and repoint the UI, or paginate SPA-side. Functional gap, sample-scale.
- **Original-blob keying collision (from the 5d review) — DO IN STEP 6 (media/AI cleanup).** 5a ingest stores the original via `PhotoAsset.Upload(stream, fileName)`, so the blob key is the (sanitized) upload **filename**, not uniquified. Two same-named uploads in a tenant (routine in a photo library — `IMG_0001.jpg`) collide on the key: the second overwrites the first's bytes and **both records then serve the same image** — a real correctness bug. It also makes a *partial* delete's blob-eviction unsafe (a surviving same-named photo may share the blob), which is why 5c bulk-delete + the `AfterRemove` hook delete only the record, leaving the original blob orphaned (storage-waste, never serves — the record is gone). Fix at ingest: key the blob uniquely per photo (by the entity id, or content-addressed via `MediaEntity.Store` which dedups identical bytes) — then blob-delete on any delete path becomes safe and the orphan goes away. The 5d **wipe** sidesteps this safely (it deletes *all* photos → no survivor can share a to-be-deleted blob, so it deletes blobs too).
