...
- **Media-derivation eviction (from the step-3b review)** — when the studio-side photo/set **delete** path lands (step 5), evict the recipe cache for the deleted source: `MediaDerivation.Query(d => d.SourceMediaId == photoId)` → `Remove` (the `BeforeRemove` that already does collection dead-id cleanup, §3). This is **cleanup, not correctness**: an un-evicted render is unreachable because `MediaEntitySource.OpenAsync` (the access gate) 404s the deleted source *before* the controller's derivation-cache lookup — so an orphan never serves, it only wastes storage until swept. The framework `MediaDerivationSweepService` (a background orphan-sweep, disabled by default) is the backstop; `MediaEntitySource<T>` leaves `SweepOrphanedDerivationsAsync` as the framework no-op (a context-free cross-tenant probe is unsafe), so delete-time eviction is the primary path.
...
- **Seal the raw `EntityController` write verbs (from the 5b review) — DO IN 5c.** `PhotosController : EntityController<PhotoAsset>` inherits `POST /api/photos` (Upsert) + `DELETE /api/photos/{id}`, which **bypass the ingest pipeline** — a crafted body creates a blob-less, embedding-less `PhotoAsset` stuck at `Pending`, `EventId=""`. When the 5c mutation surface lands, override `Upsert`/`UpsertMany` (and the raw single `Delete`, since the UI uses the custom bulk-delete #12) to `405` so photos only enter via `/upload`. Data-integrity, not an isolation hole (auth is separate).
