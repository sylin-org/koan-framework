...
- **🔧 Media URLs (#30–33) — CHANGE (D3):** these are the only routes slated to change. The framework recipe controller serves `/media/{id}/{recipe}`; update the 4 UI call sites (`grid.js`, `lightbox.js`, `ImagePreloader.js`) and drop the `masonryThumbnailMediaId`/`retinaThumbnailMediaId` fields from the photo object. Variants map: `gallery`→`gallery`, `original`→`/media/{id}` (no recipe), masonry→`masonry`, and retina→`retina`. Recipes render on demand and persist through the selected source when supported.
