...
| PMC-050 | AOT / nothing re-runs the proof | The NativeAOT result is measured by publishing and running a sample by hand. Nothing in any suite or ratchet re-runs it, so it decays silently — and it already has: ARCH-0093 certified the single binary on 2026-07-17, the mapping compiler began using `MemberInfo.MetadataToken` on 2026-08-06, and from that day every AOT-published Koan application failed on the first entity it mapped. Five weeks passed with the claim standing in an accepted ADR. PMC-049 found it only because it went looking for something else. | An AOT publish is minutes, needs a specific toolchain (vcvars64, `IlcUseEnvironmentalTools`), and is far too slow for an ordinary suite — which is exactly why it was left as a manual milestone and exactly why it rotted. Wiring it into the wrong gate makes every developer pay for it; leaving it out means the next decay is also found by accident. | Decide where it belongs: the certification ratchet (`scripts/green-ratchet.ps1`), which is already a deliberate manual boundary, or a CI lane that runs on a schedule rather than per-PR. The check has to be publish **and run**: of the three defects PMC-049 found, only the reference manifest fails at publish time. `MetadataToken` throws when the first entity is mapped and `Assembly.GetName()` throws during boot discovery, so an ILC compile — the obvious cheap proxy, and the one this entry first proposed — would have caught one of three and certified a binary that dies on startup. That rules out a per-PR gate on cost and argues for a scheduled lane that runs the real thing. | A change that reintroduces `MetadataToken` on a mapping path, or breaks the reference manifest, fails something automatically instead of being discovered five weeks later by an unrelated investigation. |
| PMC-051 | AOT / only win-x64 is measured | PMC-049 proved all five relational backends publish and run under NativeAOT on win-x64. Linux and the linux-arm64 edge RID — the appliance story the sovereign tier is justified by — are unmeasured for the server adapters. | The tempting inference is that the framework is RID-agnostic, so Linux follows. ARCH-0093 made exactly that argument in 2026-07 and it has since been shown to rest on a result that decayed: the win-x64 proof it cited stopped being true three weeks later and nobody noticed. An inference from a proof that expired is not evidence, and the cross-ILC toolchain for arm64 is genuinely different work from a native win-x64 publish. | Decide whether Linux x64 is measured first as the cheap confirmation, or arm64 directly as the case that actually matters for the edge claim — and whether a Linux failure would be a framework defect or a provider one, since `Microsoft.Data.SqlClient`'s globalization constraint may behave differently against a container's ICU. | A publish-and-run per connector on linux-x64 following the same procedure as PMC-049, and a named blocker with its provider and error for anything that fails. Only then is the RID-agnostic claim evidence rather than inference. |
