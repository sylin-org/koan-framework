...
| PMC-049 | Relational / does the single binary now reach the servers? | Removing Dapper (PMC-047) removed the blocker ARCH-0093 named: nothing in the relational tier emits IL at runtime any more, and all four adapters execute through the AOT-clean `AdoCommands`/`SqlParameters` surface. Whether that makes a PostgreSQL, SQL Server or MySQL application *publishable* under NativeAOT is a different question, and depends on the provider libraries — `Npgsql`, `Microsoft.Data.SqlClient`, `MySqlConnector` — which have not been examined at all. ARCH-0093's measured proof covers SQLite only. | The claim is attractive enough to be repeated before it is tested, which is how an unproven capability becomes inherited fact. It was briefly written into DATA-0120's consequences and has been demoted to a hypothesis pending this. | Decide nothing until it is measured. The two outcomes are different work: if the providers publish, the single-binary story extends from the embedded floor to the server adapters and wants a sample and a guide entry; if one of them does not, DATA-0120's benefit list loses a line and the reason belongs in ARCH-0093 so the next reader does not retry it. | A NativeAOT publish of a minimal app per server adapter, run against a real container, following `docs/guides/nativeaot-howto.md` and the `-p:KoanAot=true` gate — either a working binary that serves a read and a write, or the named blocker with its provider and its error. |
