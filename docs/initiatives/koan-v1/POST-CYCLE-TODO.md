...
| PMC-043 | Relational / indexable text | SQL Server and MySQL decline to index a mapped `string` or `byte[]`, because neither can key what it holds. SQL Server reads a mapped scalar out of the document as `nvarchar(4000)` and rejects an index entry over 1700 bytes; MySQL holds one as `longtext` and refuses a key over TEXT without a prefix length. Both are declined loudly — the claim is recorded as unproved and validation reports Degraded — rather than built into an index that accepts short rows and rejects long ones. Measured 2026-08-21: before the decline existed, a 2000-character label failed its insert with "index entry of length 4000 bytes exceeds the maximum length of 1700 bytes". | The fix is not in the index. Both stores could key a bounded text column, and the column is bounded only if the projection is rendered with an explicit cast — which the dialect's own reads must then spell identically, or the optimizer stops substituting the column and the index goes unused. That makes it a change to every filter and order this adapter emits, which does not belong inside an index slice. | Decide whether a mapped text value gets a bounded projected column (and what bound), or whether text stays unindexable on these two stores and applications reach for a store that indexes it. A bound also has to be reconciled with DATA-0100 comparable encoding, since a truncating cast would silently reorder. | Indexing a text property on SQL Server and MySQL builds an index the planner uses, and a value longer than the bound still round-trips through the entity unchanged. |
