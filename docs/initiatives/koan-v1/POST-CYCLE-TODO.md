...
| PMC-047 | Relational / the AOT-clean ADO surface | `Koan.Data.Relational/Ado` — `AdoCommands` and `SqlParameters` — is tracked, AOT-clean, and called by nobody. It was built for ARCH-0093 so SQLite could avoid Dapper's runtime IL emit under NativeAOT, and SQLite then hand-rolled its commands instead: 13 `CreateCommand()` calls in `SqliteRepository`, none of them through this surface. Its documentation still describes a "Dapper-backed twin with the same surface", which R11-02 deliberately retired, so the surface has outlived both its twin and its only intended caller. `QueryIdJsonAsync` also pins the retired `(Id, Json)` storage shape the mapping model has since outgrown. | Deleting it is not obviously right and neither is adopting it. It is the only AOT-clean relational execution surface in the tree, and the NativeAOT proof that justifies it is shipped and measured — so removing it would leave the single-binary floor with no shared primitive if SQLite's private helpers ever need company. Adopting it means rewriting SQLite's read path against a surface whose central method assumes a storage shape that no longer exists. | Decide whether SQLite adopts it (and `QueryIdJsonAsync` is replaced by something that reads the current mapped column set), or it is retired the way its twin was and SQLite's private helpers become the acknowledged AOT path. Either way the stale reference to the retired twin in its own documentation goes. | Whichever is chosen, `git ls-files` shows no tracked file describing a surface nothing calls, and the SQLite suite plus the NativeAOT publish both stay green. |
