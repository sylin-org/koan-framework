...
| PMC-056 | Jobs / SQL Server deadlocks a claim, and the seam that should absorb it was never wired | Eight concurrent claimers against SQL Server deadlock: `SqlException` 1205, "chosen as the deadlock victim", thrown from a scalar read inside `DataJobLedger.ClaimNext` and reproduced 2026-08-22 at roughly one occurrence in five full-suite runs. `JobOrchestrator.DrainAsync` wraps its loop in `try`/`finally` with no `catch`, so the exception ends the drain. Deadlock is the one SQL Server failure whose documented remedy is literally "rerun the transaction", and Koan has nowhere to put that fact: **`DataFailure` and `DataRetryDisposition` exist in `Koan.Data.Abstractions.Failures` with zero producers and zero consumers across `src/`** — a classification seam designed and never wired, the third found this cycle after the schema orchestrator (PMC-040) and the ADO surface (PMC-047). | Catching `SqlException` in the ledger would put a store's error numbers inside framework policy, which is the boundary DATA-0119 draws: the adapter knows a deadlock is transient, the framework decides what transient earns. Wiring the seam is the correct fix and it is larger than this defect — every adapter would need a translator, and `DataFailure` carries commit-outcome and replay dispositions that need answering per failure, not just retry. Bolting a retry onto the claim path would work and would leave the seam dead for the next caller to rediscover. | Decide whether this cycle wires `DataFailure` for real — starting with the relational adapters, which is where the classification is well understood — or whether the claim path gets a bounded retry now with the seam carried separately. If the seam: decide who translates (the adapter, or a shared relational translator over SQLSTATE/error numbers), what a caller does with a disposition, and whether an unclassified failure stays raw. If the retry: decide where it lives so it is not store knowledge leaking into `Koan.Jobs`. | A claim that meets a deadlock is retried and succeeds rather than ending the drain; the jobs suites stay green across repeated full runs on SQL Server; and whatever classifies the failure is proven by a spec that fails when the classification is removed. |
