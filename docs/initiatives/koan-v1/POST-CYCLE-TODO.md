...
| PMC-035 | Identity × Tenancy invitations — **SHIPPED 2026-08-27** (the invitation row is the contested resource: `TenantInvite` carries only the token's SHA-256 hash, and the claim is a capability-REQUIRED conditional write (`Status == Pending && ClaimedBy == null`) — adapters without `write.conditionalReplace` refuse correctively rather than resurrect the retired check-then-write race. State machine `Pending → Claimed → Accepted` with `Revoked` racing only the claim and `Expired` checked inside the claim guard; same-claimant re-drive is the idempotent recovery for interrupted runs (deterministic `Membership.KeyFor` seat, never consumed without a seat); verified-`IdentityEmail` inbox ownership, reserved-host-role refusal, tenant-audit entries on issue/revoke/accept; `InviteIssuanceService` returns the raw token exactly once; HTTP ceremony in Koan.Tenancy.Web (`api/tenancy/invitations[/accept]`). Proof: two identities racing one token yield exactly one seat and one claimant; interrupted-claim recovery succeeds for the claimant and is refused for peers; revocation/expiry/unverified-email/wrong-owner/second-accept all fail closed without consuming; specs in Koan.Identity.Tests (6) — suite 96 green. | The retired flow's only concurrency defense (person-derived deterministic seat id) converged same-person double-accepts but let two different racing identities mint two seats; R11-05 retired it rather than ship that guarantee unproven. | A process-local keyed lease would hide rather than solve the distributed claim problem; fail-closed pre-claim without recovery could consume an invitation without creating its seat. | Retired 2026-07-18 (R11-05) — see PROGRESS.md; prior art in 2d9957e7e (verified-email bind, deterministic seat id retained; plaintext token + check-then-write accept rejected). | Two identities racing one token across hosts yield exactly one seat and one durable claimant; retries recover every injected failure boundary without double grant or lost invitation; token/revocation/expiry/assurance/role cases fail correctly; package reference plus `AddKoan()` exposes the ceremony with startup/runtime explanation. |
...
| PMC-062 | Koan / conformance-kit manifest rejects real application project names — **SHIPPED 2026-08-27** (parser half: Core 1.0.25/1.0.26 — ProjectReference raw identity is provenance, canonical always enforced, regression spec in Koan.Core.Tests; release-model half: `stamp-dependents` rewrites each package's `dependency-versions.json` when direct or transitive dependency versions move — the stamp commit advances NBGV height and re-stamps compat-band floors — and plan-release refuses any release stranding direct dependents behind new floors; verified by the full 99-package train flood release and a fresh-restore probe resolving App 1.0.14 → Core 1.0.26, after which the proof-and-observability cold run PASSED with the batteries 4 passed / 0 failed / 2 capability-gated) — retained as the record; | The documented test-your-app flow is the framework's own proof contract; every real user app has a non-Koan assembly name, and a shared-package fix was unreachable through unchanged dependents' frozen floors. | The batteries failed at boot with a malformed-manifest correction — loud, not silent. | Dependent-advance policy: when a shared package advances, direct dependents re-stamp and advance in the same release, enforced by a planner guard. | A cold agent follows testing-your-app.md against published packages and the inherited batteries run green. |
...
- **PMC-062 — the release train learned to carry its own fixes forward.** Opened and closed 2026-08-27.
  The proof-and-observability cold runs surfaced it in two halves. First, a parser strictness bug: the embedded
  composition manifest records each ProjectReference as (kind, raw, canonical), and Parse demanded the RAW side of
  a project reference satisfy the Koan-identity rule — but a project's raw name is its own arbitrary assembly name
  (`TmplApp`, `MyApp`), so the documented flow of referencing an application project from a test project made every
  `EntityConformanceSpecs` battery fail at boot with `malformed at line 3/4`. Fixed in Core: package references
  still require a Koan raw id; project references carry provenance and only the canonical identity is enforced
  (regression spec included). Second, the release-model half the fix exposed: NBGV versions advance only on commits
  touching a package's own directory, and compat-band floors stamp at pack time — so a Core fix published as
  1.0.25 was unreachable through `Sylin.Koan.App 1.0.8`'s frozen floors. The tooling closes that loop:
  `Koan.Packaging stamp-dependents` rewrites each package's `dependency-versions.json` (direct versions plus a
  transitive fingerprint) whenever upstream moves, the stamp commit is what advances NBGV height, and plan-release
  now REFUSES any release that would strand a direct dependent behind a new floor. Run stamp-dependents until
  quiescent, commit, release. Verified end to end: the full 99-package train flood published green, a fresh
  restore resolved App 1.0.14 → Core 1.0.26, and the cold re-run passed with the batteries 4 passed / 0 failed /
