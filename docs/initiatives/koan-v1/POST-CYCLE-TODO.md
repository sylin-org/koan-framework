...
| PMC-038 | MySQL / shared filter corpus | The MySQL connector is the one relational adapter whose suite never ran `FilterConvergence`. Pointing it there on 2026-08-20 failed before a single filter was evaluated: seeding `ConvergenceWidget` throws `MySqlException: Truncated incorrect INTEGER value: 'null'`, which is a write-path defect, most likely a JSON null reaching a generated column typed as an integer. Every other relational adapter seeds and converges on the same corpus. | The failure is in seeding, so nothing downstream of it has ever been exercised on MySQL: whether its filters converge and whether they push down are both unknown rather than known-good. Guessing at the cast without reading the generated-column DDL would be repairing a symptom. | Read what MySqlSchema generates for a nullable numeric path and decide whether the column definition, the JSON extraction, or the null encoding is wrong; then decide whether existing tables need the column redefined. | `MySqlFilterConvergenceSpec` runs the shared corpus through `FilterConvergence.AssertPushesDownAsync` and passes, putting MySQL on the same footing as the other four relational adapters. |
| PMC-041 | Relational / declared indexes | Only SQLite builds an index an entity declares with `[Index]`. PostgreSQL, CockroachDB, SQL Server and MySQL have never created one — their retired `*Schema.cs` files contained no index grammar at all, and nothing reported the omission, so a declared index simply did not exist on four of the five relational stores. Adopting the schema seam made it visible: each of those stores answers `SupportsMappedIndexes = false`, so the plan records `Index:{name}` as an unproved claim and validation reports Degraded rather than Healthy. | The gap is old and the reporting is new; building four index grammars on the same pass that moved the decision is how a migration acquires unproven capability. Each store also needs its own proof that the planner actually uses what was built, which is a live-database test per adapter, not a unit test. | Decide whether a mapped index over a value inside the structured root is an expression index (SQLite's shape), an index over the persisted computed column SQL Server and MySQL already create, or a store-by-store choice — and whether PostgreSQL should start creating computed columns to have something to index. | Each store creates the declared index, and an `EXPLAIN`-equivalent proves the planner chooses it for the reads that value serves, as `SqliteMappedIndexSpec` already does for SQLite. Validation then reports Healthy rather than Degraded for an indexed entity. |
| PMC-042 | Jobs / PostgreSQL ledger isolation | `retired_work_types_are_dead_lettered_without_blocking_valid_work` fails on the PostgreSQL jobs adapter when the suite runs whole and passes when run alone: the retired row is still `Queued` where the shared behaviour suite expects `Dead`. The same `JobBehaviorSuite` passes 81/81 on SQLite and 68/68 on SQL Server, so the behaviour is right and something about this adapter's test isolation is not. Confirmed pre-existing on 2026-08-21 by running the suite against an unmodified worktree at `ffec8407b`, which fails identically. | It is one test out of sixty-eight, it is an isolation problem rather than a wrong answer, and diagnosing it means instrumenting the real claim loop rather than reading it — the same shape as the Mongo jobs flake, which turned out to be a genuine drain race and not a test defect. Guessing costs more than the failure does. | Establish first whether the retired row is left behind by a prior test in the same container, or whether the drain genuinely stops before reclaiming it under PostgreSQL's isolation level. Do not quiet the assertion until that is known; the Mongo precedent is that this shape of flake hides a real loop defect. | The full suite passes repeatedly with no ordering dependence, and if a loop defect is found, a focused spec that fails against the unfixed loop. |
