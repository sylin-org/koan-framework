...
| PMC-011 | Release automation | A future pre-1.0 minor bump must republish the complete reverse-dependent closure, but NBGV changes only Git-touched package owners and the release compiler does not yet mint or enforce that version wave. Data.Core currently has 80 reverse dependents. | R07-01 does not break the shipped Data.Core contract, so manufacturing an 81-package minor wave here would be noise. The missing automation is nevertheless a release-safety gap for the next real breaking tier. | Choose a Git-native automatic intent mechanism that gives every closure member a fresh immutable version without restoring lockstep or requiring an operator-maintained package list; make interrupted publication resumable. | A synthetic breaking package change automatically selects, versions, packs, clean-room restores, and verifies its full reverse closure; omitting any member fails before publish; ordinary leaf patches remain independent. |
