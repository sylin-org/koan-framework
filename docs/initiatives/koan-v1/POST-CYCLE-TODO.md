...
| PMC-040 | Relational / schema seam adoption | The four relational runtimes still hand-roll schema creation and validation in their own `*Schema.cs` (SQLite 201, MySQL 326, SQL Server 140, Npgsql 103 lines), while `RelationalSchemaOrchestrator` — which owns the same decisions, including the DDL consent gate — is registered in DI and resolved by nobody. DATA-0119 removed both reasons adoption was impossible: the contract is now asynchronous, and `NativeType`/`NativeTypeFor` let a store validate in its own vocabulary so MySQL's character-set checking and SQLite's `TEXT`-for-everything survive the move. | Adoption is now mechanical but not small: four `IRelationalDdlExecutor` + `IRelationalStoreFeatures` implementations, four schema paths deleted, and container-backed verification for three of them. Doing it on momentum at the end of a long cycle is how a migration quietly weakens a validation nobody re-reads. | Decide whether the connection is held by the executor or passed per call — SQLite's validate opens non-creating for an external lifecycle and reports a distinct absence, which is a connection concern rather than a schema one. Confirm the orchestrator's `Plan` produces the same column set each adapter's `CreateTable` produces today, especially SQLite's single-generated-identity `INTEGER PRIMARY KEY AUTOINCREMENT`. | Each adapter's existing suite green with its `*Schema.cs` deleted; MySQL still fails a character-set drift; SQL Server and PostgreSQL still refuse automatic DDL in Production, now because the path they take evaluates consent rather than because a call was remembered. |
