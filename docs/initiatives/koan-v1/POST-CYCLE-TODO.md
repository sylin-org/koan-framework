...
| PMC-039 | Data / projection pushdown | No adapter pushes a projection down: every one of the seven hardcodes `ProjectionHandled = false`, so a sparse-fieldset query materializes whole entities and trims them in memory. Verified on 2026-08-20 through the new fallback fact, which reports "Koan finished projection in memory" for a projected read on SQLite. It is the one query axis the no-fallback sweep does not cover, because neither oracle projects. | Unlike filter or sort, the win is not uniform. A relational entity is one JSON document in one column, so selecting fields changes little; MongoDB `$project` and a SQL++ field list genuinely cut transfer. Pushing it everywhere for symmetry would add per-adapter machinery for a benefit only some of them have. | Decide where a projection is worth expressing natively and where reporting it honestly is enough, and whether projection becomes a declared capability so the choice is visible rather than hardcoded false. Note that a pushed projection returns partial entities, which interacts with materialization and with Entity-family type resolution. | A projection oracle in the shared kit, mirroring `FilterConvergence.AssertPushesDownAsync`: identical results everywhere, and no fallback fact on the adapters that declare they push it down. |
