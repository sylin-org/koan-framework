...
| PMC-052 | Relational / drift that could be repaired is only reported | `EnsureCreatedAsync` adds columns a table is missing, and treats every other difference as something to report or refuse. Some drift is repairable under the consent the caller already gave: a stale MySQL generated column (PMC-045) is one `ALTER TABLE ... MODIFY COLUMN` away from correct, and Koan knows the expression it wants. Today an upgraded database reports Degraded forever until a human writes that statement by hand. | Repair is a mutation, and the difference between "add a column that is missing" and "replace a column that exists" is the difference between an additive change and one that can destroy data if the framework is wrong about what it is looking at. A projected column is the safe case — the store recomputes it from the document, so rebuilding loses nothing — but the general case is not safe, and a repair path that does not distinguish them would be a data-loss defect waiting for its first wrong classification. | Decide which findings are repairable and prove the boundary: almost certainly a projected column whose stamp is stale, and nothing else without its own justification. Decide too whether repair happens silently under `AutoCreate` or requires its own consent, since rebuilding a stored generated column on a large table rewrites it. | An upgraded MySQL table with a stale generated column is rebuilt on boot under `AutoCreate`, validates clean afterwards, and the index built on that column is used again; a drifted column that is *not* a projection is still refused rather than rewritten. |
...
| PMC-053 | SQLite / a recurring unexplained failure cluster | The SQLite connector suite has three times reported a cluster of failures — four to nine at once, always file-backed specs (`SqliteColdRestartSpec`, `SqliteMappedIndexSpec`, the paging and legacy-mapping specs) — and passed 49/49 on an immediate re-run every time. Twice the cause was established: a concurrent foreground build, and a worktree built at an older commit, both clobbering the shared test output the suite was running against. The third occurrence on 2026-08-21 had neither: the run built first and nothing else was in flight, so the pattern is **not** fully explained by build provenance. | Two of three occurrences had a real, identified cause, which makes the third easy to wave through as more of the same — and that is the reasoning that kept PMC-042 filed as "isolation" for a day while it was a genuine tie-order defect. The suite is also the one whose specs create, dispose and re-open file-backed databases and call `SqliteConnection.ClearAllPools()`, so a handle or pool-lifetime interaction is at least as plausible as an environment artifact. | Establish whether the failures share a cause with each other before looking for a cause outside the suite: capture the actual exception from a failing cluster rather than the assertion, and check whether the specs involved share a temp path, a pooled connection, or a disposal order. If it is a handle race, it belongs to the connection lifecycle; if it is leftover state from an aborted prior run, the fixture should be clearing it. | The exception behind a cluster is captured and named, and either the suite runs clean repeatedly under the condition that reproduces it, or the entry closes with the environment interaction identified so the next reader does not re-investigate. |
