...
| PMC-036 | Data / sort pushdown | An order key that reaches through a collection (`?sort=-Sightings.LastChangedAt`, meaning "by each widget's latest sighting") is pushed down only by Couchbase, which computes it as `ARRAY_MAX` over an array comprehension. Mongo and the relational trio decline it, so `InMemorySorter` finishes the ordering — correct, but it materializes the whole result set and gives up native paging for a query the application wrote in one line. | The answers are right and every surface suite is green, so this is a performance cliff rather than a defect, and it is invisible exactly where it bites: a large collection, in production. Fixing it well means one shared way to express the aggregate, not four private ones. | Decide whether `SortAggregation` becomes a declared capability adapters negotiate, so the fallback is chosen knowingly and appears in facts, and whether Mongo (`$map` + `$max`) and relational (lateral or correlated aggregate) translate it or keep declining. Settle First/Last, which depend on an element order a document store does not promise. | Cross-adapter convergence over the existing surface suite: identical orderings and page windows on every adapter, plus evidence that a pushed-down sort performs no in-memory fallback — the receipt reports the sort handled and pagination native. |
