...
The exact R12-05 candidate becomes one recoverable public 0.20 wave. A clean human and coding-agent
environment can install Koan from public NuGet, reach a useful Entity result, replace local persistence
...
recover one unavailable-adapter intent; run public package-only FirstUse and GoldenJourney; conduct independent
human and coding-agent journeys; record immutable identities, links, findings, and go/no-go for R12-07.
...
8. Have at least one independent human and one independent coding agent follow public guidance only. Record
   anonymous confusion, elapsed time, corrections, and their explanation of reference/application responsibility.
...
7. One independent human and one independent coding agent complete the journey and leave no unresolved contradiction
   or hidden prerequisite.
