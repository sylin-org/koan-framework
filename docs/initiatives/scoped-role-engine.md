...
The minimum HTTP surface covers capability descriptors; bounded role list/get/create/edit/disable/
retire; bounded assignment list/assign/revoke and an assignable-role picker; policy read/update/reset;
current-subject effective access; bounded proposed-change preview; and scope-authorized audit history.
Viewing definitions, inspecting assignments, assigning roles, editing definitions, editing policy,
previewing others and reading audit are separately authorizable. Core remains the only owner of
...
| 2026-09-13 | Slice 2 headless query boundary and first real-provider receipt green | Mutation authority is re-read through the selected contributor and captured scope versions at the one-shot lifecycle boundary. `Constrain`, `Query`, `QueryWithCount`, `Get` and `Count` share one immutable plan plus a provider-pushed direct scope-key filter; paginated access rejects adapters without provider-bounded paging. Identity is 112/112 and the isolated SQLite receipt is 1/1, proving pushed scope filtering, paging/count, insert-only and conditional-replace CAS. Not complete: Mongo parity, optional Web management routes/directories and W01-W18 remain open. |
