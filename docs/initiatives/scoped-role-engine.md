...
Reapproval lifecycle contexts must carry that same full effective grant snapshot, including grants
derived from replacement-policy audiences, so a lifecycle veto cannot inspect a weaker authority
set than the mutation actually approves.
...
Membership removal uses provider-atomic conditional delete by identity plus expected version; adapters
without that explicit capability reject rather than falling back to a read-then-delete race.
...
| 2026-09-13 | Red-team lifecycle, delete-CAS and input-bound corrections green | Reapproval now crosses veto-capable `MemberAdding` and committed-only `MemberAdded`, with both phases observing the full effective approval including replacement-policy grants. Data exposes explicit `Write.ConditionalDelete` / `DeleteIf`; InMemory, SQLite and Mongo prove that a revoke paused before dispatch loses to a concurrent reapproval without deleting its newer generation. Headless and Web management reject oversized names/descriptions/presentation metadata, excessive clauses/parameters and nested parameter graphs. Identity is 126/126; Data Core is 563/563; SQLite, Mongo and Web are 1/1; Identity and Identity.Web build warning-clean. |
