...
| 2026-09-13 | Red-team lifecycle, delete-CAS and input-bound corrections green | Reapproval now crosses veto-capable `MemberAdding` and committed-only `MemberAdded`. Data exposes explicit `Write.ConditionalDelete` / `DeleteIf`; InMemory, SQLite and Mongo prove that a revoke paused before dispatch loses to a concurrent reapproval without deleting its newer generation. Headless and Web management reject oversized names/descriptions/presentation metadata, excessive clauses/parameters and nested parameter graphs. Identity is 125/125; Data Core is 563/563; SQLite, Mongo and Web are 1/1; Identity and Identity.Web build warning-clean. |
