...
> **Status (dev):** Phase 1 (Gate) shipped as `711251b8`, Phase 2 (Constrain) as `83783b75` — the `[Access]` per-action gate (legacy `[Authorize]`/`[AllowAnonymous]`/`[RequireScope]` lowered as sugar, shipped floor tests unchanged), and `EntityAccess<T>` with `Owner` declared once + `Constrain` riding the WEB-0068 read rail + create-stamp / update-freeze / out-of-scope-404 / bounded mass-delete writes. Both TDD'd, mutation-checked, and adversarially reviewed (71 tests; the WEB-0068 and MCP regression suites stay byte-identical). Phase 3 (Project) is next; Phases 4–5 follow.
