...
- **(c) Opportunistic migration.** Convert registrars to `KoanModule` when a pillar is already being touched — collapse its bootstrap hosted service into `Start`, its `Initialize` into `Register`, its `Describe` into `Report`. Drive from dogfood; never a big-bang.
  - **Pass 1 ✅ DONE — the `IKoanInitializer`-only fold.** Investigation showed the "8 initializer-only types" weren't a uniform mechanical set; the clean folds were **5**, and 3 of those sat beside a no-op-`Initialize` `KoanAutoRegistrar` (a registration↔provenance split _within one assembly_). So the fold healed those splits rather than widening them — each became **one** `KoanModule` (Id = prior `ModuleName`, `Register` = the initializer body, `Report` = the registrar's `Describe` verbatim, so boot provenance is byte-identical):
    - `Koan.Data.Core` — merged `KoanDataCoreInitializer` + `KoanAutoRegistrar` → `KoanDataCoreModule`.
    - `Koan.Web.Transformers` — merged the nested `TransformerStartupInitializer` + `KoanAutoRegistrar` → `KoanWebTransformersModule`.
    - `Koan.Web.Connector.GraphQl` — the initializer and registrar **both** called `AddKoanGraphQl()` (a live double-registration); deleted the redundant `KoanGraphQlInitializer`, converted the registrar → `KoanGraphQlModule`.
    - `Koan.Data.Direct` — straight convert `KoanDataDirectInitializer` → `KoanDataDirectModule` (no sibling registrar).
    - `Koan.Context` (service) — absorbed `JobMaintenanceTaskRegistration` into the existing pipeline registrar; deleted the fragment.
    - **Deferred from pass 1 (intricate, not mechanical):** `RecipeInitializer` + `RecipeApplier` (dual source-gen-discovery **and** DI-registered deferral — a mechanical fold would double-apply recipes), and `MongoOptimizationAutoRegistrar` (registers no services — global driver config — and is manually invoked as `optimizer.Initialize(null!)` from Mongo's `KoanAutoRegistrar`, which the bridge's `Initialize` would NPE).
    - Verified: full solution build clean; Bootstrap integration 16/16, Core 179/179, Transformers 13/13. Also cleared ~2.5-month-old orphaned `artifacts/generated` cruft (10 dirs) that was being globbed into compilation and broke on the type deletions.
  - **Pass 2 — finding: the deferred items do NOT belong in `Start`.** The pass-1 note hypothesized that recipes and the Mongo driver-config would "collapse into `Start`." Investigation corrects that:
    - **Recipes apply by registering services** (`IKoanRecipe.Apply(IServiceCollection, cfg, env)`) — that mutates the service graph and therefore **must** run in the `Register`/`Initialize` phase, before the container is built. `Start` receives a built `IServiceProvider`, into which you cannot add services. So recipes are a legitimate Initialize-phase concern, not a `Start` candidate. (Their two-type split also encodes a real register-then-apply ordering; left intact.)
    - **`MongoOptimizationAutoRegistrar`** configures the global Mongo driver (serializers, conventions, class maps) and is **deliberately invoked early** (manually, during the registrar's `Initialize`) so the serializers exist before any Mongo operation during bootstrap. `Start` runs at host-start, after `Initialize` — too late. Initialize-phase by necessity.
    - **The genuine `Start` candidates are one-time bootstrap hosted services** — e.g. `AuthBootstrapHostedService`, `RoleBootstrapHostedService` (run their pillar's one-shot startup once, with DI available, no cleanup). Note `CachePolicyBootstrapper` is **not** one: it holds a `StopAsync` lifecycle (subscribes/unsubscribes from `AppDomain.AssemblyLoad`), so it stays a hosted service. These migrate opportunistically when their pillars are next touched; none is a completion gate.
- **(d) Settle. ✅ SETTLED.** The criterion — "a new pillar **can** be authored as one `KoanModule` with an ordered `Start`" — is met and proven: `KoanModuleTests.Host_runs_Start_on_every_module_in_Before_After_order` exercises a real `KoanModule` (Register + Start + `[After]` ordering + Report) discovered and run through the production pipeline (source-gen → `KoanRegistry` → `RegistrarOrdering` → `KoanModuleHost`). The authoring surface, the discovery path, and the ordered `Start` lifecycle are all in place; the auth off-registry split-brain is healed; the `IKoanInitializer`-only fragmentation is folded. Remaining registrar→`KoanModule` conversions are **opportunistic** (drive from dogfood when a pillar is touched), and `IKoanInitializer`/`IKoanAutoRegistrar` remain indefinitely as the internal discovered plumbing — deleting them was never a goal.
