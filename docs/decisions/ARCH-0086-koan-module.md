...
- **(c) Opportunistic migration.** Convert registrars to `KoanModule` when a pillar is already being touched — collapse its bootstrap hosted service into `Start`, its `Initialize` into `Register`, its `Describe` into `Report`. Drive from dogfood; never a big-bang.
  - **Pass 1 ✅ DONE — the `IKoanInitializer`-only fold.** Investigation showed the "8 initializer-only types" weren't a uniform mechanical set; the clean folds were **5**, and 3 of those sat beside a no-op-`Initialize` `KoanAutoRegistrar` (a registration↔provenance split _within one assembly_). So the fold healed those splits rather than widening them — each became **one** `KoanModule` (Id = prior `ModuleName`, `Register` = the initializer body, `Report` = the registrar's `Describe` verbatim, so boot provenance is byte-identical):
    ...
    - **Deferred (NOT mechanical — own pass):** `RecipeInitializer` + `RecipeApplier` (dual source-gen-discovery **and** DI-registered deferral — a mechanical fold would double-apply recipes; the real win is collapsing the deferral into `KoanModule.Start`, which has a live `IServiceProvider`), and `MongoOptimizationAutoRegistrar` (registers no services — global driver config — and is manually invoked as `optimizer.Initialize(null!)` from Mongo's `KoanAutoRegistrar`, which the bridge's `Initialize` would NPE).
