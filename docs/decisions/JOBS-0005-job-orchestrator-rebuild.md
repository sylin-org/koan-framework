...
- **Per-DB container matrix** — infrastructure is in place (the convergence suite + `JobsHarness.StartWithSettingsAsync` make each tier a subclass + connector reference). Enabling Postgres/Mongo/SqlServer is **blocked on a framework gap**: the durable Jobs ledger's framework-defined entities (`JobRecord`/`JobGateRecord`/`JobClaimTicket`) are not schema-ensured/created on the Postgres adapter (SQLite auto-creates on write; PG's `data.ensureCreated` for these entities does not materialize the table). That's a Koan data-layer/schema concern, not a Jobs bug — once it's closed, each tier is a one-class subclass. *Resume lead:* the PG adapter handles `data.ensureCreated` at `PostgresRepository.cs:756` (→ `orch.EnsureCreatedAsync<TEntity,TKey>`) and auto-ensures on write at `:186`; the table is JSON-storage `JobRecord_<hash>`. Yet `Data<JobRecord,string>.Execute<object?>(new Instruction(DataInstructions.EnsureCreated))` did not materialize it. Trace whether the static `Data<T>.Execute` routes the instruction to the adapter for a framework-defined entity, and whether `EnsureSchemaOnStart` covers entities in referenced framework assemblies (vs. the app assembly). Likely proper fix: `KoanJobsModule.Start` (or a lazy `DataJobLedger` init) explicitly ensures the three Jobs entities' schema at startup. The test harness (`JobsHarness.EnsureJobSchema`) already attempts this best-effort.
...
- **Distributed core — shipped** (2 specs): two orchestrators ("nodes") share one ledger — competing consumers never double-claim (50 jobs, 2 nodes, exactly 50 runs), and a resource gate set by one node is honored by the other. Remaining in this tier: the `+bus` push-transport package (`Koan.Jobs.Transport.Messaging` riding `Koan.Messaging`) and per-type `[JobPersistence]` two-ledger routing (the attribute + binding resolution exist; the in-memory-vs-durable routing is not yet wired).
