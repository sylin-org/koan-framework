...
**Full unification — no additive MCP-only bridge.** The SEC-0002 `IAuthorize` seam becomes the **canonical decision engine that every surface calls**: REST base CRUD is routed through it (replacing the inline `CanRead`/`CanWrite`/`CanRemove` path), and the MCP edge calls it (replacing the `McpToolAccessPolicy`-only path). One engine, two entry points (ASP.NET filter for REST, explicit seam call for the MCP edge).
...
- The precise **compat shim** for `CanRead`/`CanWrite`/`CanRemove` consumers — designed in Phase 3.
