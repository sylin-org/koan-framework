...
2. **Per-built-in-op access is the entity floor in v1, and the floor is entity-wide.** `[Authorize]`/`[AllowAnonymous]`/`[RequireScope]` on the entity gate read+write+remove *uniformly* — standard ASP.NET attributes are binary and don't express a per-verb split. The **read/mutate split AND per-op scope granularity are both deferred** (see "Explicitly deferred"); finer per-verb gating is a controller-action override (REST) or a custom toolset verb (MCP). Only *custom verbs* carry their own `[RequireScope]`. `[ToolHidden]` is absolute removal; per-grant visibility is the Wall.
...
3. **Access-floor unification (the consolidation — see Decision D "Phase 3 resolved design").** Four reviewable, individually-green slices:
   - ✅ **3.1** (`1188901c`) Relocated the seam abstraction `Koan.Web.Extensions` → `Koan.Web` (providers stay); added `[RequireScope]` + `EntityFloorAuthorizationProvider`; registered the seam by default (no provider → default Allow = backward-compat). **Folded 3.4 in** (the dormant hook was the `AuthorizeRequest` collision source).
   - ✅ **3.2** `EntityEndpointService.Gate` authorizes every operation (read/write/remove) through the seam; **hard-cut `CanRead/CanWrite/CanRemove`** (they gated nothing — zero overrides repo-wide); `Koan-Access-*` headers recomputed from the seam (`AnnotateAccess`). One gate, both surfaces. Migrated the broken canonical skill examples (koan-auth/koan-web) to the floor model.
   - **3.3** MCP edge → the SEC-0004 gate (the seam realization). ✅ **3.3a** (`0c34d910`) thread the MCP caller's `ClaimsPrincipal` into `EntityRequestContext.User` on both data paths (tools + code-mode); STDIO anonymous. ✅ **3.3b** (`4221d6e8`) retire `McpToolAccessPolicy`'s entity-tool enforcement + the bridge pre-check (denial → `meta.shortCircuit`); refactor `tools/list`/catalog visibility onto the *same* gate via `McpEntityGate.CoarseAllows`; demote `[McpEntity]` to pure exposure (`RequiredScopes`/`RequireAuthentication` dropped); migrate the Vault/Article conformance fixtures to `[Access]`. Custom `[McpTool]` verbs keep the policy. The full MCP surface is exercised by `tests/Suites/Mcp/Koan.Mcp.Conformance.Tests`. (The S16.PantryPal dogfood was **retired** — it was a single-user anonymous app with nothing to gate; a purpose-built MCP showcase sample is forthcoming.)
   - ✅ **3.4** *(folded into 3.1)* Deleted the dormant `IAuthorizeHook` path; kept `AuthorizeDecision`.
...
- **The read/mutate split and per-operation scope granularity** — the v1 floor is **entity-wide** (`[Authorize]`/`[RequireScope]` gate all of read/write/remove uniformly, because standard ASP.NET attributes are binary). The common "public read / authenticated write" shape is therefore NOT expressible by the floor alone in v1 — use a controller-action override (REST) or a custom toolset verb (MCP) until a richer floor lands. A per-verb floor would need a Koan-specific access vocabulary (the very thing §D retired) or a new attribute property; deferred pending a decision on whether the terseness justifies the concept.
