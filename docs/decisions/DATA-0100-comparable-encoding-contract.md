...
**Realization update (2026-08-21, PMC-037).** `ComparableScalarEncoding` now lives in `Koan.Data.Core` (namespace `Koan.Data.Core`), unchanged in behaviour. It moved because Couchbase — a document store, which cannot reference `Koan.Data.Relational` — is governed by this contract and was the one adapter it never reached: it stored a `TimeSpan` in .NET's default form and ordered `1.00:00:00` before `23:00:00`. Couchbase now writes the canonical form through the same converters, and `TimeSpan` is admitted to `TypeClassification.IsPortableStreamSortScalar`. The converters keep reading the legacy string form, so documents written before the change still materialize; they order correctly once rewritten. The decision above is unchanged — only where it is spelled and which stores honour it.
