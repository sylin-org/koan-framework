...
**Status**: **Accepted (2026-06-07); steps 0–4 implemented.** The unified seam + RBAC floor + `PolicyAuthorizationProvider` (WEB-0047 absorbed, parity-proven against characterization specs) are in place, and the capability gates are rewired through `IAuthorize` (`ICapabilityAuthorizer` retired; WEB-0047 marked *Superseded by SEC-0002*; the sync-over-async on the gates is gone). **Step 5 (folding the entity `IAuthorizeHook` into the seam) is DEFERRED** — the entity hook already shares the `AuthorizeDecision` vocabulary and is a legitimately separate per-entity pipeline extension point; full decision-engine routing is a future increment pending a real driver (e.g. wanting entity-pipeline authz to use PDP providers). The seam currently lives in `Koan.Web.Extensions` with an `HttpContext`-free contract; the physical hoist to a web-independent assembly is deferred until bus/jobs consume it. PDP/ReBAC adapters (§5 Tiers 2+) remain future, opt-in.
**Date**: 2026-06-07
