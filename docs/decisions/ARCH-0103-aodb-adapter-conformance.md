...
- **Remaining (Docker-heavy, explicit follow-on)**: production-adapter isolation specs (Qdrant first — lightest) proving the Shared overlay `__koan_tenant` round-trip (empirically closing the security flag) + Container + Database (source→collection); then Weaviate/ES/OpenSearch/Milvus; + declaring the `DataCaps.Isolation.*` tokens on the vector repos (needs a vector conformance kit + inner-vs-decorator token placement). The `CollectionName`/`IndexName` option footgun (pinning bypasses partition+source folding) is documented; warn/compose-on-pin is a follow-on.
