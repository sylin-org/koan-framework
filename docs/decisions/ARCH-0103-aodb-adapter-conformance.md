...
- **InMemory slice SHIPPED** (`dev`, the family base + the first fold). The base (`KeyValueStore<TEntity,TKey>` + `KvRecord` + the hybrid `KvFilterEvaluator`) lives in `Koan.Data.Core/KeyValue/`; `InMemoryRepository` is now a thin subclass supplying get/scan/write/remove/clear over `InMemoryDataStore` (which holds `KvRecord` envelopes, keyed by `(source, type, partition)`). **Shared** mode is proven by the new `InMemoryManagedFieldNoLeakSpec` (the `ManagedFieldNoLeak` oracle: read isolation + IDOR + cross-scope write-reject + scoped RemoveAll) — RED→GREEN. **Container** mode is proven by the existing `InMemoryPartitionSpec`. Gates green: InMemory suite 34, data-core 274, tenancy 104 (byte-identical). **Database** mode is realized (the store is source-keyed, the exact mechanism `VectorDatabaseRoutingSpec` already proves for the InMemoryVector twin); a dedicated InMemory data-plane Database routing proof (a 2-source host) is the one pending cell for this slice.
- **Json + Redis folds** are the remaining P2 work (the JSON-text family): introduce `ManagedFieldJsonInjector` (lift relational's `ManagedFieldContractResolver` into a shared home both relational and KV reach — likely `Koan.Data.Core`, adding Newtonsoft there, or a small shared assembly), serialize the `KvRecord` through it (entity + `__`-keys), and on read deserialize the entity + extract the `__`-keys back into the envelope's `Managed`. Json gains a per-source directory; Redis a per-source connection / DB-index. Then the conformance specs (Json Docker-free; Redis Testcontainers).
