...
- **Remaining (Docker-heavy, explicit follow-on)**: the HTTP-fleet production isolation specs for Weaviate/ES/OpenSearch/Milvus (mirror the Qdrant spec) — **DONE, see §9.10**; declaring the `DataCaps.Isolation.*` tokens on the vector repos (needs a vector conformance kit + inner-vs-decorator token placement). The `CollectionName`/`IndexName` option footgun (pinning a real name bypasses partition+source folding) is documented; warn/compose-on-pin is a follow-on.
...
- **Follow-on**: a SqliteVec conformance cell (the natural Docker-free non-filtering twin, exercising the Shared under-claim/fail-close branch); the HTTP adapters adopting the kit (consolidating the Task-1 production specs onto it).
