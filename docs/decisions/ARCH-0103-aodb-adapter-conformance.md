...
- **P5 COMPLETE — the conformance ledger** (`dev`, `c7d060af`..`35e910b8`).
  ...
  - **Documented follow-ons**: (a) Couchbase folds onto `DocumentStore` (then it declares the tokens + gains a conformance cell); (b) the vector fleet's Container/Database realization + the overlay-naming silent-drop fix (the ADR's separate vector phase); (c) optionally derive the boot note from the declared caps rather than a static string (drift-proofing — a `note`).
