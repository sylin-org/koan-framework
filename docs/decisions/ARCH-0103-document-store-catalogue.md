...
## 7. Conformance pulled forward (P5 → now) — **SHIPPED** (`c7d060af`..`35e910b8`)

Defined, with Mongo as the first green cell and then generalized to all SEVEN conformant surfaces: capability tokens **`DataCaps.Isolation.ContainerScoped`** + **`DatabaseScoped`** (co-defined with their checks — over-claim fails green, ARCH-0094); and **`AodbConformanceSpecsBase<TFixture>`** proving, through real `AddKoan()`: **Shared** (`ManagedFieldNoLeak` oracle), **Container** (distinct container per partition + concurrent no-leak), **Database** (per-source physical routing + fail-closed on unconfigured), plus a **Declares** cell that pins the token↔proof binding. Green on Mongo · InMemory/Json/Redis · Sqlite/Postgres/SqlServer (28 facts). The shared discoverable shard axis lives in the TestKit (adapter-test-only — off-proofs untouched). **Couchbase reuses the kit once it folds onto `DocumentStore`** (today it declares no isolation token → excluded). Full status: ARCH-0103-aodb-adapter-conformance §9.7.

## 8. Phasing & gates

> **Implementation status (2026-06-25, on `dev`, unpushed):** P0 catalogue `f5ed15c9` · P1 oracle-verified `8e59b2e7` · **P2 `DocumentStore` base + golden `MongoDocumentStore` + per-source pool `96e0e11e`** (debt left behind: GetDatabase reflection, `_=nameResolver`, 3 schema caches, dup ToCamelCase; **per-(entity,source) pool leak FIXED**; readiness deadlock FIXED — `EnsureReady` ungated) · P3 AODB Database-mode proof + console cleanup `ad547a62` · 4-lens adversarial review GO/0-must-fix + fixes `f3778519` (cancellation parity, factory sync-`IDisposable`, batch + memoization contracts documented). **GATES GREEN: Mongo connector 27/27, Web Mongo 49/49, data-core 274/274 + tenancy 104/104 byte-identical, full-solution build clean.** Then the **`host.docker.internal` discovery candidate `2bb342fb`** (P4 §below) and the **cross-cutting promotion `0bcec884`**: `AdapterHealthContributorBase` (Mongo health fresh-client-per-probe leak FIXED) + `AdapterReadinessForwardingBase` (DocumentStore inherits) + `AdapterActivity` (DocumentStore composes) into `Koan.Core(.Adapters)`. **P4 NOW COMPLETE — the two deferred items shipped (focused pass, both 4-lens-review GO / 0 must-fix):** (1) **`KoanModule` registrar merge `b878851a`** — `KoanAutoRegistrar` + `MongoOptimizationAutoRegistrar` folded into one `MongoModule : KoanModule` + one once-guarded `MongoDriverConfiguration` (both static-config bodies harvested verbatim, single lock, no `Initialize(null!)`); dead `MongoDbZenGardenOfferingBinding` + the available-providers scan dropped; boot-report connection display converged on the fleet-shared `AdapterBootReporting.ResolveConnectionString`. Gate: Mongo 27/27 + data-core 274 + tenancy 104 byte-identical. (2) **Connection-string consolidation `d0ea898e`** — the 4 copies folded into one `MongoConnectionString` helper (verbatim; `MergeOverrides`/`ApplyParameters` kept as two methods — divergent parse+policy, not force-unified) + the non-Default `"auto"` quirk fixed via `ResolveRoutedConnection` (revives the dead `baseOptions` fallback); tests-first via `MongoConnectionStringSpec` (34 characterization tests). Gate: Mongo connector 61/61 (27 oracle + 34 char). **FLEET FOLLOW-UP (review-surfaced, NOT done): the sibling Postgres/SqlServer/Sqlite/SqliteVec factories share the same non-Default `"auto"` quirk and are WORSE (no fallback at all) — the proper fix is to hoist `ResolveRoutedConnection`-style normalization into the shared `AdapterConnectionResolver` so every factory's non-Default path collapses blank/`"auto"` uniformly (P5 / fleet mandate).**
...
- **P3** — **SHIPPED** (`c7d060af`..`35e910b8`; landed after P4 in wall-clock). `ContainerScoped`/`DatabaseScoped` tokens + `AodbConformanceSpecsBase` green on ALL seven conformant surfaces (28 facts) + per-adapter boot notes + SURFACES row; 5-lens adversarial review GO_WITH_FIXES (0 must-fix). See §7 + ARCH-0103-aodb-adapter-conformance §9.7.
- **P4** — orchestration/discovery/boot-report/connection-string cleanup (`KoanModule` + one helper); then **Couchbase** folds onto the proven base (gains the write-stamp I9/I10 + per-source).
  - **Couchbase fold COMPLETE (2026-06-26).** `CouchbaseRepository` (sealed, ~1000 LOC, on no family base, declared ZERO isolation tokens) deleted → **`CouchbaseDocumentStore<TEntity,TKey> : DocumentStore<TEntity,TKey>`** (thin dialect of native primitives; harvested INTACT: `CouchbaseN1qlFilterTranslator`, GUID-N key/storage encoding, CAS conditional-replace, the transaction batch, the raw-N1QL escape hatch `IRawQueryRepository`/`Query.String`, primary-index online-wait, RequestPlus). Now declares all three AODB modes mapped to Couchbase's 3-level keyspace: **Shared** = managed field injected into the document JSON via the shared `ManagedFieldJsonInjector` + a CAS conflict guard (I9/I10; injected/guard values normalized through the SAME `CouchbaseN1qlFilterTranslator.NormalizeValue` the read filter uses so Guid/enum/DateTime managed fields round-trip — strings unchanged); **Container** = distinct native **scope** per ambient partition (existing `CouchbaseClusterProvider` mapping, unchanged); **Database** = distinct native **bucket** per routed source (per-source `CouchbaseClusterProvider` pooled by connection|bucket in `CouchbaseAdapterFactory`, `IAsyncDisposable`/`IDisposable`). Couchbase-local fixes the fold surfaced: a `FormatCollectionName` charset sanitizer (the framework resolver leaves a nested-type `'+'` the collection manager rejects — `ManagedFieldNoLeak+ScopedDoc`) mirroring `FormatScope`; the CAS loop trips a clear framework contention error on exhaustion (never an unconditional fallback — a doc can transition scope, so the write stays guard-checked, fail-closed). **Gates GREEN:** Couchbase connector 13/13 (9 prior CRUD/filter/CAS/scope-naming + 4 new AODB conformance cells) + Web AdapterSurface Couchbase 49/49 + data-core 274 + tenancy 104 byte-identical + full-solution build clean. 4-lens adversarial review (Workflow): 6 confirmed (1 HIGH + 1 MEDIUM fixed pre-commit; 2 LOW per-source-pool findings = Mongo parity, deferred to the shared `AdapterConnectionResolver` hoist). **Conformance**: `CouchbaseAodbConformanceSpec : AodbConformanceSpecsBase<CouchbaseFixture>` (Database cell provisions two per-source buckets; the fixture raises the node data-quota to fit). The document family is now complete (Mongo + Couchbase).
- **Deferred items recorded here (P2 verification, 2026-06-25):**
  ...
  - **Pre-existing quirk — non-Default source with `ConnectionString="auto"`. FIXED `d0ea898e`** via `MongoConnectionString.ResolveRoutedConnection(sourceConn, resolvedDefault)` (collapses blank/`"auto"` → the discovery-resolved Default `baseOptions`, reviving the factory's intended-but-dead fallback; explicit per-source connections pass through unchanged). `AdapterConnectionResolver` returned the literal `"auto"` for a non-Default source relying on runtime discovery; it does **not** receive the discovery-resolved Default string, so the factory's `baseOptions.ConnectionString` fallback was near-dead-code and the per-source provider pool would key on an invalid string. **Fleet residual:** the sibling relational/SqliteVec factories share this quirk and are worse (no fallback) — hoist the normalization into the shared `AdapterConnectionResolver` (P5).
