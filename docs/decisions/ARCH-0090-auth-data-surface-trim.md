...
**Status — DONE (card X-data-direct-fold).** Executed as specified: the five impl files (`DirectSession`/`DirectTransaction`/`DirectDataService`/`JsonSettings`/`ConfigurationConstants`) moved to `src/Koan.Data.Core/Direct/` under namespace `Koan.Data.Core.Direct` (impls now `internal`); `IDirectDataService` registered by default in `ServiceCollectionExtensions.RegisterKoanDataCoreServices` next to `IDataService`; `DirectRegistration` + `KoanDataDirectModule` dropped; the `Koan.Data.Direct` project deleted from `Koan.sln` + disk. Empirical re-derivation found the premise *stronger* than framed — the package had **zero `ProjectReference`s repo-wide** (a true orphan), so `.Direct()` was dead-by-default before this; the three connector references to "Koan.Data.Direct" were DATA-0053 provenance **comments only** (no compile-time edge). Direct was entirely untested, so the fold ships its first guard: `DirectDataAccessSpec` (2 specs through real `AddKoan()` + SQLite — default-registration + SQL round-trip + tx commit/rollback), mutation-verified (disabling the registration fails both). Migration note also landed in `docs/reference/modules-overview.md`, `docs/architecture/capability-map.md`, `docs/reference/data/cache.md`, `CLAUDE.md`.
