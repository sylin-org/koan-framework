...
- Inconsistencies (AI surface `IAiPipeline` vs legacy names, `Get(id)` vs `ById(id)`, bootstrap examples)
...
- Standardize AI interface samples to `IAiPipeline` and data access samples to entity statics; eliminate repository-pattern examples.
...
- Consistent naming: `IAiPipeline` (not `IAiService`), `Get(id)` as preferred by-id static; note aliases only once.
...
- Fix TOC formatting; normalize bootstrap, `IAiPipeline`, and `Get(id)` across high-traffic docs.
