#!/usr/bin/env bash
# Staged composite — opencode harness, local 12GB-tier model. usage: run-opencode.sh <koan|plain>
# Mirrors the other runners: identical stage bodies, gates, graders — only harness/model differ.
set -u
cd "$(dirname "$0")"
ARM="${1:?usage: run-opencode.sh <koan|plain>}"
REPO="$(cd ../../../../.. && pwd)"
G="$PWD"
MODEL="${OCC_MODEL:-opencode-qwen35-9b-local}"
MREF="${OCC_MREF:-ollama/local-code:qwen35-9b-q4}"
...
if [ "$ARM" = "koan" ]; then
  ...
  NEUTRAL="$(dirname "$REPO")/_agent-race-neutral/staged-opencode"
  [ -d "$NEUTRAL" ] && { echo "neutral folder exists — move it aside first." >&2; exit 1; }
  mkdir -p "$NEUTRAL"
  WORK="$NEUTRAL"
fi
OFLAGS=(-m "$MREF" --format json)
...
run_stage() { # $1 stage no; $2 prompt file; $3 session id ("" for fresh)
  ...
  S=$(date +%s)
  if [ -n "$3" ]; then
    (cd "$WORK" && timeout 1800 opencode run -s "$3" "${OFLAGS[@]}" "$PROMPT") > "$ART/events-s$1.jsonl" 2>&1
  else
    (cd "$WORK" && timeout 1800 opencode run "${OFLAGS[@]}" "$PROMPT") > "$ART/events-s$1.jsonl" 2>&1
  fi
  E=$(date +%s)
...
SID=$(grep -o '"sessionID":"[^"]*"' "$ART/events-s1.jsonl" | head -1 | cut -d'"' -f4)
