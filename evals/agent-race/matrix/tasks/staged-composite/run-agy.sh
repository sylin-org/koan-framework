#!/usr/bin/env bash
# Staged composite — Antigravity agy harness (Gemini tiers). usage: run-agy.sh <koan|plain>
# Mirrors run.sh / run-claude.sh: identical stage bodies, gates, graders — only the harness differs.
set -u
cd "$(dirname "$0")"
ARM="${1:?usage: run-agy.sh <koan|plain>}"
REPO="$(cd ../../../../.. && pwd)"
G="$PWD"
AGY="/c/Users/onose/AppData/Local/agy/bin/agy.exe"
MODEL="agy-gemini"
...
if [ "$ARM" = "koan" ]; then
  ...
  NEUTRAL="$(dirname "$REPO")/_agent-race-neutral/staged-agy"
  [ -d "$NEUTRAL" ] && { echo "neutral folder exists — move it aside first." >&2; exit 1; }
  mkdir -p "$NEUTRAL"
  WORK="$NEUTRAL"
fi
AFLAGS=(--dangerously-skip-permissions --print-timeout 30m --output-format stream-json)

run_stage() { # $1 stage no; $2 prompt file; $3 conversation id ("" for fresh)
  local S E PROMPT
  PROMPT="$(cat "$2")"
  S=$(date +%s)
  if [ -n "$3" ]; then
    (cd "$WORK" && timeout 1800 "$AGY" --conversation "$3" --add-dir "$WORK" -p "$PROMPT" "${AFLAGS[@]}") > "$ART/events-s$1.jsonl" 2>&1
  else
    (cd "$WORK" && timeout 1800 "$AGY" --add-dir "$WORK" -p "$PROMPT" "${AFLAGS[@]}") > "$ART/events-s$1.jsonl" 2>&1
  fi
  E=$(date +%s)
...
SID=$(grep -o '"conversation_id":"[^"]*"' "$ART/events-s1.jsonl" | head -1 | cut -d'"' -f4)
echo "conversation: ${SID:-<none>}"
...
for n in 1 2 3; do
  echo "stage$n wallclock: $(cat "$ART/wallclock-stage$n.txt")s"
done
