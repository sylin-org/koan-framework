...
if [ "$ARM" = "koan" ]; then
  ...
  NEUTRAL="$(dirname "$REPO")/_agent-race-neutral/test03-rel"
...
gate() {
  bash "$G/grade-test03.sh" "$WORK" 2>&1 | tee "$ART/grade-stage1.txt"
  local s n t
...
(cd "$WORK" && timeout 2700 codex exec "${FLAGS[@]}" --json \
  -o "$ART/lastmsg-s1.txt" - < <(cat "$P1" "$G/task-rel.txt")) > "$ART/events-s1.jsonl" 2>&1
E=$(date +%s)
